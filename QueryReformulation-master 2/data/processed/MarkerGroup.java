...
*/
        ...
        public MarkerGroupingEntry testAttribute(IMarker marker) {
            Object value;
            if (!marker.exists()) {
                // If the marker was deleted during the update drop it
                return null;
            }
            try {
...
*/
    class GroupMarkerField extends MarkerField {
        ...
        public int compare(MarkerItem item1, MarkerItem item2) {
            ...
            try {
                return (findGroupValue(item2.getMarker().getType(), item2.getMarker()).getPriority() - findGroupValue(item1.getMarker().getType(), item1.getMarker()).getPriority());
...
*/
    private void processEntries() {
        ...
        IConfigurationElement[] markerEntryElements = configurationElement.getChildren(MarkerSupportRegistry.MARKER_GROUPING_ENTRY);
        IConfigurationElement[] attributeGroupingElements = configurationElement.getChildren(MarkerSupportRegistry.MARKER_ATTRIBUTE_GROUPING);
        ...
        for (int i = 0; i < attributeGroupingElements.length; i++) {
            ...
            IConfigurationElement[] mappings = attributeGrouping.getElement().getChildren(MarkerSupportRegistry.ATTRIBUTE_MAPPING);
            for (int mappingIndex = 0; mappingIndex < mappings.length; mappingIndex++) {
                ...
                if (idsToEntries.containsKey(entryId)) {
                    ...
                    entry.getMarkerGroup().mapAttribute(attributeGrouping, entry, mappings[mappingIndex].getAttribute(MarkerSupportRegistry.VALUE));
                } else {
                    IDEWorkbenchPlugin.log(NLS.bind(//$NON-NLS-1$
...
*/
    public void mapAttribute(AttributeMarkerGrouping attributeGrouping, MarkerGroupingEntry entry, String attributeValue) {
        addEntry(attributeGrouping.getMarkerType(), new AttributeMapping(entry, attributeGrouping.getAttribute(), attributeValue, attributeGrouping));
...
*/
    public void unmap(AttributeMarkerGrouping attributeMarkerGrouping) {
        ...
        while (mappings.hasNext()) {
            EntryMapping mapping = mappings.next();
            if (mapping instanceof AttributeMapping && (((AttributeMapping) mapping).grouping == attributeMarkerGrouping)) {
                removed.add(mapping);
            }
        }
