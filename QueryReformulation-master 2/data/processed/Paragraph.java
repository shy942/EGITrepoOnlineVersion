...
public class Paragraph {
    ...
    public void parseRegularText(String text, boolean expandURLs, boolean wrapAllowed, HyperlinkSettings settings, String fontId, String colorId) {
        ...
        if (expandURLs) {
            ...
            if (loc == -1)
                ...
                int textLoc = 0;
                while (loc != -1) {
                    addSegment(new TextSegment(text.substring(textLoc, loc), fontId, colorId, wrapAllowed));
                    boolean added = false;
                ...
                if (textLoc < text.length()) {
                    addSegment(new TextSegment(text.substring(textLoc), fontId, colorId, wrapAllowed));
                }
            }
        } else {
            addSegment(new TextSegment(text, fontId, colorId, wrapAllowed));
    ...
    private int findUrl(String text, int startIndex) {
        ...
        Arrays.sort(locs);
        for (int i = 0; i < PROTOCOLS.length; i++) if (locs[i] != -1)
            return locs[i];
        return -1;
    }
