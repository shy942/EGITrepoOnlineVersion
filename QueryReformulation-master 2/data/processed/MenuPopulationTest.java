...
*/
    ...
    public void testMenuServicePopupContribution() throws Exception {
        ...
        try {
            ...
            popupMenuExtender = new PopupMenuExtender(IPageLayout.ID_PROBLEM_VIEW, manager, problemsView.getSite().getSelectionProvider(), problemsView, ((PartSite) problemsView.getSite()).getContext(), false);
    ...
    public void testMenuServiceContribution() {
        ...
        AbstractContributionFactory factory = new AbstractContributionFactory("menu:org.eclipse.ui.main.menu?after=file", "205747") {
            ...
            public void createContributionItems(IServiceLocator serviceLocator, IContributionRoot additions) {
                ...
                CommandContributionItem cci = new CommandContributionItem(new CommandContributionItemParameter(serviceLocator, "my.about", IWorkbenchCommandConstants.HELP_ABOUT, CommandContributionItem.STYLE_PUSH));
                manager.add(cci);
...
*/
    private boolean[] addLogger() {
        ...
        Platform.addLogListener(new ILogListener() {
            ...
            public void logging(IStatus status, String plugin) {
                if ("org.eclipse.ui.workbench".equals(status.getPlugin()) && status.getSeverity() == IStatus.ERROR && status.getException() instanceof IndexOutOfBoundsException) {
                    errorLogged[0] = true;
                }
            }
        });
    ...
    public void testFactoryScopePopulation() throws Exception {
        AbstractContributionFactory factory = new AbstractContributionFactory("menu:the.population.menu?after=additions", "org.eclipse.ui.tests") {
        ...
        assertNotNull(view);
        IMenuService service = view.getSite().getService(IMenuService.class);
    ...
    protected void doSetUp() throws Exception {
        super.doSetUp();
        afterOne = new AbstractContributionFactory("menu:after.menu?after=after.one", "org.eclipse.ui.tests") {

            @Override
            public void createContributionItems(IServiceLocator serviceLocator, IContributionRoot additions) {
                additions.addContributionItem(new GroupMarker("after.insert"), null);
            }
        };
        menuService.addContributionFactory(afterOne);
        beforeOne = new AbstractContributionFactory("menu:before.menu?before=before.one", "org.eclipse.ui.tests") {

            @Override
            public void createContributionItems(IServiceLocator serviceLocator, IContributionRoot additions) {
                additions.addContributionItem(new GroupMarker("before.insert"), null);
            }
        };
        menuService.addContributionFactory(beforeOne);
        endofOne = new AbstractContributionFactory("menu:endof.menu?endof=endof.one", "org.eclipse.ui.tests") {

            @Override
            public void createContributionItems(IServiceLocator serviceLocator, IContributionRoot additions) {
                additions.addContributionItem(new GroupMarker("endof.insert"), null);
            }
        };
        menuService.addContributionFactory(endofOne);
        usefulContribution = new CommandContributionItem(new CommandContributionItemParameter(window, null, IWorkbenchCommandConstants.HELP_ABOUT, 0));
        iconField = CommandContributionItem.class.getDeclaredField(FIELD_ICON);
    ...
    public void testPrivatePopup() throws Exception {
        ...
        try {
            ...
            manager.setRemoveAllWhenShown(true);
            popupMenuExtender = new PopupMenuExtender(activePart.getSite().getId(), manager, null, activePart, ((PartSite) activePart.getSite()).getContext());
    ...
    private void assertPrivatePopups(final MenuManager manager) {
        ...
        // allPopups = false. Should show only if additions is available
        assertTrue("allPopups = false for cmd3. Should show only if additions present", hasAdditions == cmd3Found);
    }
