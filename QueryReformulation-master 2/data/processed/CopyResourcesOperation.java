...
*/
    protected void copy(IProgressMonitor monitor, IAdaptable uiInfo) throws CoreException {
        SubMonitor subMonitor = SubMonitor.convert(monitor, resources.length + (resourceDescriptions != null ? resourceDescriptions.length : 0));
        ...
        for (int i = 0; i < resources.length; i++) {
            ...
            overwrites = WorkspaceUndoUtil.copy(new IResource[] { resources[i] }, getDestinationPath(resources[i], i), resourcesAtDestination, subMonitor.split(1), uiInfo, true, fCreateGroups, fCreateLinks, fRelativeToVariable);
        ...
        setResourceDescriptions((ResourceDescription[]) overwrittenResources.toArray(new ResourceDescription[overwrittenResources.size()]));
        // Reset the target resources to refer to the resources in their new
        // location.
        setTargetResources((IResource[]) resourcesAtDestination.toArray(new IResource[resourcesAtDestination.size()]));
    }
...
*/
    ...
    public IStatus computeUndoableStatus(IProgressMonitor monitor) {
        ...
        // recreate them?
        if (status.isOK() && resourceDescriptions != null && resourceDescriptions.length > 0) {
...
*/
    private void setOriginalResources(IResource[] originals) {
        ...
        for (int i = 0; i < originals.length; i++) {
            snapshotResourceDescriptions[i] = ResourceDescription.fromResource(originals[i]);
        }
    }
}
