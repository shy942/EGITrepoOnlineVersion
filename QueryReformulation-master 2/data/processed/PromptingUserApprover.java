...
*/
    ...
    public IStatus proceedRedoing(IUndoableOperation operation, IOperationHistory history, IAdaptable uiInfo) {
        ...
        boolean prompt = UndoPlugin.getDefault().getPreferenceStore().getBoolean(PreferenceConstants.PREF_CONFIRMUNDO);
        if (!prompt) {
            return Status.OK_STATUS;
        }
...
*/
    private IStatus prompt(boolean undoing, IUndoableOperation operation, IAdaptable uiInfo) {
        ...
        String message = NLS.bind(UndoExampleMessages.BoxView_ConfirmUndo, command, operation.getLabel());
        MessageDialogWithToggle dialog = MessageDialogWithToggle.openOkCancelConfirm(shell, UndoExampleMessages.BoxView_Title, message, UndoExampleMessages.UndoPreferences_DoNotConfirm, false, null, null);
