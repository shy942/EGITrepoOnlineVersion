    ...
    public Object execute(ExecutionEvent event) {
        ...
        try {
            PlatformUI.getWorkbench().getProgressService().run(true, true,  mon -> {
                ...
                IUndoableOperation op = new UpdateMarkersOperation(markers, attrs, MarkerMessages.markCompletedAction_title, true);
                subMonitor.worked(20);
                execute(op, MarkerMessages.markCompletedAction_title, subMonitor.split(80), null);
            });
        } catch (InvocationTargetException e) {
            StatusManager.getManager().handle(StatusUtil.newStatus(IStatus.ERROR, e.getLocalizedMessage(), e), StatusManager.LOG);
        } catch (InterruptedException e) {
            StatusManager.getManager().handle(StatusUtil.newStatus(IStatus.WARNING, e.getLocalizedMessage(), e), StatusManager.LOG);
        }
        return this;
    }
}
