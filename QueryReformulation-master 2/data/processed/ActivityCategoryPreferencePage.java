...
*/
        public  CategoryLabelProvider(boolean decorate) {
            ...
            lockDescriptor = AbstractUIPlugin.imageDescriptorFromPlugin(PlatformUI.PLUGIN_ID, //$NON-NLS-1$
            "icons/full/ovr16/lock_ovr.png");
        }
                ...
                try {
                    if (decorate) {
                        if (isLocked(category)) {
                            ...
                            OverlayIcon overlay = new OverlayIcon(descriptor, lockDescriptor, new Point(originalImageData.width, originalImageData.height));
...
*/
    private void createPromptButton(Composite composite) {
        ...
        activityPromptButton.setSelection(getPreferenceStore().getBoolean(IPreferenceConstants.SHOULD_PROMPT_FOR_ENABLEMENT));
    }
...
*/
    ...
    private ICategory[] getPartialCategories() {
        return WorkbenchActivityHelper.resolveCategories(workingCopy, InternalActivityHelper.getPartiallyEnabledCategories(workingCopy));
    }

    private ICategory[] getEnabledCategories() {
        return WorkbenchActivityHelper.resolveCategories(workingCopy, InternalActivityHelper.getEnabledCategories(workingCopy));
    }
...
*/
    ...
    public boolean performOk() {
        ...
        getPreferenceStore().setValue(IPreferenceConstants.SHOULD_PROMPT_FOR_ENABLEMENT, activityPromptButton.getSelection());
        return true;
    }
