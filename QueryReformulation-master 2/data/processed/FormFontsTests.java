...
public class FormFontsTests extends TestCase {
    ...
    public void testMultipleInstances() {
        ...
        for (count = 1; count < 20; count++) Assert.assertEquals("getBoldFont(...) returned a different font for the same params on iteration " + count, boldSystemFont, FormFonts.getInstance().getBoldFont(display, display.getSystemFont()));
        for (; count > 0; count--) {
            FormFonts.getInstance().markFinished(boldSystemFont, display);
            if (count != 1)
