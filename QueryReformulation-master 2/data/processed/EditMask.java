...
*/
    ...
    private VerifyListener verifyListener = new VerifyListener() {
        ...
        public void verifyText(VerifyEvent e) {
            // any new characters
            if (// should eventually be .isFull() to account for optional characters
            editMaskParser.isComplete() && e.start == e.end && e.text.length() > 0) {
                e.doit = false;
                return;
            }
    ...
    private Runnable updateTextField = new Runnable() {
        ...
        private void updateSelectionPosition(String newRawText) {
            ...
            if (!newRawText.equals(oldRawText)) {
                // yep
                ...
                if (selection > oldSelection) {
                    // typed an illegal character; backup
                    text.setSelection(new Point(selection - 1, selection - 1));
                } else {
                    // backspaced over a literal; don't interfere with selection position
                    text.setSelection(new Point(selection, selection));
                }
            }
