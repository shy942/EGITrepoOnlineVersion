...
public class SnippetSideEffectMigration {
    ...
    static class ObservableView {
        ...
        private void bindData() {
            ...
            IObservableValue personFirstNameObservable = BeanProperties.value(ObservableBeanPerson.PROPERTY_FIRST_NAME).observe(person);
            ISWTObservableValue personFirstNameTextObservable = WidgetProperties.text(SWT.Modify).observe(personFirstNameText);
            dbc.bindValue(personFirstNameTextObservable, personFirstNameObservable, new UpdateValueStrategy().setAfterConvertValidator( obj -> {
                if (obj instanceof String && ((String) obj).isEmpty()) {
                    return ValidationStatus.error("First Name may not be empty");
                }
                return Status.OK_STATUS;
            }), null);
            IObservableValue personLastNameObservable = BeanProperties.value(ObservableBeanPerson.PROPERTY_LAST_NAME).observe(person);
            ISWTObservableValue personLastNameTextObservable = WidgetProperties.text(SWT.Modify).observe(personLastNameText);
            dbc.bindValue(personLastNameTextObservable, personLastNameObservable, new UpdateValueStrategy().setAfterConvertValidator( obj -> {
                if (obj instanceof String && ((String) obj).isEmpty()) {
