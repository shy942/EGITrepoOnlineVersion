    ...
    public static void main(String[] args) {
        ...
        Realm.runWithDefault(DisplayRealm.getRealm(display), new Runnable() {
            ...
            public void run() {
                ...
                dbc.bindValue(WidgetProperties.text(SWT.Modify).observe(text), value, new UpdateValueStrategy().setAfterConvertValidator(new FiveValidator()), null);
                // Bind the error label to the validation error on the dbc.
                dbc.bindValue(WidgetProperties.text().observe(errorLabel), new AggregateValidationStatus(dbc.getBindings(), AggregateValidationStatus.MAX_SEVERITY));
                shell.pack();
                shell.open();
