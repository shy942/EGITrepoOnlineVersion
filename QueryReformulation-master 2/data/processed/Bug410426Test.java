...
public class Bug410426Test extends UITestCase {
    ...
    public void testNoClassCastExceptionForMenuManagerToolbarContribution() throws Exception {
        ...
        ExtendedLogReaderService log = window.getService(ExtendedLogReaderService.class);
        LogListener logListener = new SynchronousLogListener() {

            @Override
            public void logged(LogEntry entry) {
                if (entry.getLevel() == LogService.LOG_ERROR && entry.getException() instanceof ClassCastException && entry.getException().getMessage().contains("MenuManager cannot be cast to org.eclipse.jface.action.ContributionItem")) {
                    //$NON-NLS-N$
        ...
        LogFilter logFilter = new LogFilter() {
            ...
            public boolean isLoggable(Bundle bundle, String loggerName, int logLevel) {
                //$NON-NLS-N$
                return logLevel == LogService.LOG_ERROR && loggerName == null && "org.eclipse.equinox.event".equals(bundle.getSymbolicName());
            }
        };
