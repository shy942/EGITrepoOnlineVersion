            ...
            public void keyTraversed(TraverseEvent e) {
                if (e.detail == SWT.TRAVERSE_RETURN) {
                    ...
                    if (getTableViewer().getTable().getItemCount() == 0) {
                        ...
                        boolean textChanged = !initialFilterTextValue.equals(filterText.getText().trim());
                        if (hasFocus && textChanged && filterText.getText().trim().length() > 0) {
                            TableItem item = getFirstHighlightedItem(getTable().getItems());
                            if (item != null) {
                                getTable().setSelection(new TableItem[] { item });
