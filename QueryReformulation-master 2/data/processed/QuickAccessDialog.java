...
public class QuickAccessDialog extends PopupDialog {
    ...
    public  QuickAccessDialog(final IWorkbenchWindow window, final Command invokingCommand) {
        super(// persist
        ProgressManagerUtil.getDefaultParent(), // persist
        SWT.RESIZE, // persist
        true, // persist
        true, // but not location
        false, true, true, null, QuickAccessMessages.QuickAccess_StartTypingToFindMatches);
        this.window = window;
        ...
        BusyIndicator.showWhile(window.getShell() == null ? null : window.getShell().getDisplay(), new Runnable() {
            ...
            public void run() {
                ...
                QuickAccessProvider[] providers = new QuickAccessProvider[] { new PreviousPicksProvider(previousPicksList), new EditorProvider(), new ViewProvider(model.getContext().get(MApplication.class), model), new PerspectiveProvider(), commandProvider, new ActionProvider(), new WizardProvider(), new PreferenceProvider(), new PropertiesProvider() };
                    ...
                    protected void updateFeedback(boolean filterTextEmpty, boolean showAllMatches) {
                        if (filterTextEmpty) {
                            ...
                            if (showAllMatches || sequences == null || sequences.length == 0) {
                                ...
                                setInfoText(NLS.bind(QuickAccessMessages.QuickAccess_PressKeyToShowAllMatches, sequences[0].format()));
                            }
                        }
                    }
...
*/
                    ...
                    protected QuickAccessElement getPerfectMatch(String filter) {
                        QuickAccessElement perfectMatch = (QuickAccessElement) elementMap.get(filter);
                        return perfectMatch;
                    }
    ...
    private void restoreDialog() {
        ...
        if (dialogSettings != null) {
            ...
            if (orderedElements != null && orderedProviders != null && textEntries != null && textArray != null) {
                int arrayIndex = 0;
                for (int i = 0; i < orderedElements.length; i++) {
                    QuickAccessProvider quickAccessProvider = (QuickAccessProvider) providerMap.get(orderedProviders[i]);
