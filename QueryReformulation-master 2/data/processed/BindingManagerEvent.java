...
*/
    public  BindingManagerEvent(final BindingManager manager, final boolean activeBindingsChanged, final Map previousTriggersByParameterizedCommand, final boolean activeSchemeChanged, final Scheme scheme, final boolean schemeDefined, final boolean localeChanged, final boolean platformChanged) {
        if (manager == null) {
            throw new NullPointerException(//$NON-NLS-1$
            "A binding manager event needs a binding manager");
        }
        this.manager = manager;
        if (schemeDefined && (scheme == null)) {
            throw new NullPointerException(//$NON-NLS-1$
            "If a scheme changed defined state, then there should be a scheme identifier");
        }
        this.scheme = scheme;
...
*/
    public final boolean isActiveBindingsChangedFor(final ParameterizedCommand parameterizedCommand) {
        ...
        if (previousTriggersByParameterizedCommand != null) {
            final Collection previousBindingCollection = (Collection) previousTriggersByParameterizedCommand.get(parameterizedCommand);
            if (previousBindingCollection == null) {
                previousBindings = EMTPY_SEQUENCE;
            } else {
                previousBindings = (TriggerSequence[]) previousBindingCollection.toArray(new TriggerSequence[previousBindingCollection.size()]);
            }
        } else {
            previousBindings = EMTPY_SEQUENCE;
        }
