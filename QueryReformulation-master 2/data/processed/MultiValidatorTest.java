...
public class MultiValidatorTest extends AbstractDefaultRealmTestCase {
    ...
    public void testValidationStaleness() {
        ...
        assertTrue(validationStatus.isStale());
        // +1
        assertEquals(1, validationChangeCounter.count);
        assertEquals(1, validationStaleCounter.count);
        ...
        assertFalse(validationStatus.isStale());
        // +1
        assertEquals(2, validationChangeCounter.count);
        assertEquals(1, validationStaleCounter.count);
    }
    ...
    public void testValidationStatusBecomesStaleThroughNewDependency() {
        final DependencyObservableValue nonStaleDependency = new DependencyObservableValue(ValidationStatus.ok(), IStatus.class);
        nonStaleDependency.setStale(false);
        final DependencyObservableValue staleDependency = new DependencyObservableValue(ValidationStatus.ok(), IStatus.class);
    ...
    public void testBug251003_CompareDependenciesByIdentity() {
        ...
        validator = new MultiValidator() {
            ...
            @Override
            protected IStatus validate() {
                for (Iterator<DependencyObservable> it = dependencies.iterator(); it.hasNext(); ) ObservableTracker.getterCalled(it.next());
                return null;
            }
        };
