...
*/
    private void writeInstalledIUs(PrintWriter writer) {
        ...
        try {
            ...
            //$NON-NLS-1$
            writer.println(" " + NLS.bind(IDEWorkbenchMessages.ConfigurationLogUpdateSection_lastChangedOn, DateFormat.getDateInstance().format(new Date(profile.getTimestamp()))));
            //$NON-NLS-1$
            writer.println(" " + NLS.bind(IDEWorkbenchMessages.ConfigurationLogUpdateSection_location, profile.getProperty(IProfile.PROP_INSTALL_FOLDER)));
            //$NON-NLS-1$
            writer.println(" " + NLS.bind(IDEWorkbenchMessages.ConfigurationLogUpdateSection_timestamp, Long.toString(profile.getTimestamp())));
            writer.println();
            ...
            if (!sorted.isEmpty()) {
                ...
                writer.println();
                for (Iterator iter = sorted.iterator(); iter.hasNext(); ) writer.println(iter.next());
            }
        } finally {
