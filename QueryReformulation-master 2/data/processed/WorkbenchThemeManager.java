...
public class WorkbenchThemeManager extends EventManager implements IThemeManager {
    ...
    public static interface Events {
        ...
        //$NON-NLS-1$
        public static final String THEME_REGISTRY_RESTYLED = TOPIC + "/themeRegistryRestyled";

        //$NON-NLS-1$
        public static final String THEME_REGISTRY_MODIFIED = TOPIC + "/themeRegistryModified";
    }
    ...
    public ITheme getCurrentTheme() {
        ...
        if (currentTheme == null) {
            ...
            themeId == null)
                ...
                if (currentTheme == null) {
                    // still null - the preference
                    // didn't resolve to a proper theme
                    setCurrentTheme(IThemeManager.DEFAULT_THEME);
                    StatusManager.getManager().handle(StatusUtil.newStatus(PlatformUI.PLUGIN_ID, "Could not restore current theme: " + themeId, //$NON-NLS-1$
                    null));
                }
            }
        }
...
*/
    ...
    public void setCurrentTheme(String id) {
        ...
        if (WorkbenchThemeManager.getInstance().doSetCurrentTheme(id)) {
            ...
            if (!PrefUtil.getAPIPreferenceStore().getString(IWorkbenchPreferenceConstants.CURRENT_THEME_ID).equals(id)) {
            ...
            }
            {
                if (oldTheme != null && eventBroker != null) {
                    ...
                    eventBroker.send(UIEvents.UILifeCycle.THEME_DEFINITION_CHANGED, context.get(MApplication.class.getName()));
                }
            }
        }
    }
    ...
    public static class WorkbenchThemeChangedHandler implements EventHandler {
        ...
        protected void sendThemeRegistryRestyledEvent() {
            IEventBroker eventBroker = (IEventBroker) getContext().get(IEventBroker.class.getName());
        ...
        protected org.eclipse.e4.ui.css.swt.theme.ITheme getTheme(org.osgi.service.event.Event event) {
            org.eclipse.e4.ui.css.swt.theme.ITheme theme = (org.eclipse.e4.ui.css.swt.theme.ITheme) event.getProperty(IThemeEngine.Events.THEME);
            if (theme == null) {
                IThemeEngine themeEngine = (IThemeEngine) getContext().get(IThemeEngine.class.getName());
        ...
        protected void resetThemeRegistries(ThemeRegistry themeRegistry, FontRegistry fontRegistry, ColorRegistry colorRegistry) {
            for (FontDefinition def : themeRegistry.getFonts()) {
                if (def.isOverridden()) {
                    ...
                    fontRegistry.put(def.getId(), def.getValue() != null ? def.getValue() : PreferenceConverter.getFontDataArrayDefaultDefault());
                }
            }
        ...
        protected void populateDefinition(org.eclipse.e4.ui.css.swt.theme.ITheme cssTheme, ITheme theme, ColorRegistry registry, ColorDefinition definition, IPreferenceStore store) {
            ThemeElementHelper.populateDefinition(cssTheme, theme, registry, definition, store);
        }

        protected void populateDefinition(org.eclipse.e4.ui.css.swt.theme.ITheme cssTheme, ITheme theme, FontRegistry registry, FontDefinition definition, IPreferenceStore store) {
            ThemeElementHelper.populateDefinition(cssTheme, theme, registry, definition, store);
        }
    }
    ...
    public static class ThemeRegistryModifiedHandler implements EventHandler {
        ...
        protected void sendThemeDefinitionChangedEvent() {
            MApplication application = (MApplication) getContext().get(MApplication.class.getName());
            getInstance().eventBroker.send(UIEvents.UILifeCycle.THEME_DEFINITION_CHANGED, application);
        }

        protected void populateThemeRegistries(ThemeRegistry themeRegistry, FontRegistry fontRegistry, ColorRegistry colorRegistry, org.eclipse.e4.ui.css.swt.theme.ITheme cssTheme, ITheme theme) {
            IPreferenceStore store = PrefUtil.getInternalPreferenceStore();
