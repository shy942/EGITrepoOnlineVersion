...
public class Snippet025TableViewerWithPropertyDerivedColumns {
    ...
    static class View {
        ...
        protected void bindGUI(DataBindingContext dbc) {
            ...
            ViewerSupport.bind(peopleViewer, viewModel.getPeople(), BeanProperties.values(Person.class, new String[] { "name", "mother.name", "father.name", "mother.mother.name", "mother.father.name", "father.mother.name", "father.father.name" }));
            IObservableValue masterSelection = ViewerProperties.singleSelection().observe(peopleViewer);
            dbc.bindValue(WidgetProperties.text(SWT.Modify).observe(nameText), BeanProperties.value(Person.class, "name").observeDetail(masterSelection));
            ...
            dbc.bindValue(ViewerProperties.singleSelection().observe(mothercomboViewer), BeanProperties.value(Person.class, "mother").observeDetail(masterSelection));
            ...
            dbc.bindValue(ViewerProperties.singleSelection().observe(fatherComboViewer), BeanProperties.value(Person.class, "father").observeDetail(masterSelection));
        }
    }
}
