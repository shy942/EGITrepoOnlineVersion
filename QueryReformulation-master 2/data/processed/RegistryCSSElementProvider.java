...
public class RegistryCSSElementProvider implements IElementProvider {
    ...
    private Map<Class<?>, IElementProvider> providerCache = Collections.synchronizedMap(new WeakHashMap<Class<?>, IElementProvider>());
                    ...
                    if ("provider".equals(ce.getName())) {
                        for (IConfigurationElement ce2 : ce.getChildren()) {
                            if (typeName.equals(ce2.getAttribute("class"))) {
                                try {
                                    if (extpt.equals(DEPRECATED_ELEMENT_PROVIDER_EXTPOINT)) {
                                        System.err.println("Extension point " + DEPRECATED_ELEMENT_PROVIDER_EXTPOINT + " is deprecated; use " + ELEMENT_PROVIDER_EXTPOINT);
                                    }
