...
*/
    public LinkHelperDescriptor[] getLinkHelpersFor(Object anObject, INavigatorContentService aContentService) {
        ...
        for (Iterator<LinkHelperDescriptor> itr = getDescriptors().iterator(); itr.hasNext(); ) {
            descriptor = itr.next();
            if (aContentService.isVisible(descriptor.getId()) && descriptor.isEnabledFor(anObject)) {
...
*/
    public LinkHelperDescriptor[] getLinkHelpersFor(IEditorInput anInput, INavigatorContentService aContentService) {
        ...
        for (Iterator<LinkHelperDescriptor> itr = getDescriptors().iterator(); itr.hasNext(); ) {
            descriptor = itr.next();
            if (aContentService.isVisible(descriptor.getId()) && descriptor.isEnabledFor(anInput)) {
                helpersList.add(descriptor);
            }
        }
        if (helpersList.size() == 0) {
