...
*/
    public  HandlerProxy(final String commandId, final IConfigurationElement configurationElement, final String handlerAttributeName, final Expression enabledWhenExpression, final IEvaluationService evaluationService) {
...
*/
    ...
    public final Object execute(final ExecutionEvent event) throws ExecutionException {
        ...
        if (loadException != null)
            //$NON-NLS-1$
            throw new ExecutionException("Exception occured when loading the handler", loadException);
        return null;
    }
    ...
    private final boolean loadHandler() {
        if (handler == null) {
            ...
            try {
                ...
                final String message = "The proxied handler was the wrong class";
                final IStatus status = new Status(IStatus.ERROR, WorkbenchPlugin.PI_WORKBENCH, 0, message, e);
                WorkbenchPlugin.log(message, status);
                configurationElement = null;
                loadException = e;
            } catch (final CoreException e) {
                final String message = "The proxied handler for '" + configurationElement.getAttribute(handlerAttributeName) + "' could not be loaded";
...
*/
    private String getConfigurationElementAttribute() {
        ...
        if (attribute == null) {
            IConfigurationElement[] children = configurationElement.getChildren(handlerAttributeName);
            for (int i = 0; i < children.length; i++) {
                String childAttribute = children[i].getAttribute(IWorkbenchRegistryConstants.ATT_CLASS);
