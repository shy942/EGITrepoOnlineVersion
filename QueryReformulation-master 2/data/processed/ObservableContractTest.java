...
public class ObservableContractTest extends ObservableDelegateTest {
    ...
    public void testGetRealm_NotNull() throws Exception {
        assertNotNull(formatFail("The observable's realm should not be null."), observable.getRealm());
    }
    ...
    public void testChange_ChangeEvent() throws Exception {
        ...
        assertEquals(formatFail("A change in the observable should notify change listeners."), 1, listener.count);
    }
    ...
    public void testChange_EventObservable() throws Exception {
        ...
        assertSame(formatFail("In the change event the source of the change should be the observable."), observable, event.getObservable());
    }
    ...
    public void testChange_ObservableRealmIsTheCurrentRealm() throws Exception {
        ...
        observable.addChangeListener(listener);
        delegate.change(observable);
        assertTrue(formatFail("On change the current realm should be the realm of the observable."), listener.isCurrentRealm);
    }
    ...
    public void testRemoveChangeListener_RemovesListener() throws Exception {
        ...
        delegate.change(observable);
        assertEquals(formatFail("When a change listener is removed it should not still receive change events."), 1, listener.count);
    }

    public void testIsStale_NotStale() throws Exception {
        delegate.setStale(observable, false);
        assertFalse(formatFail("When an observable is not stale isStale() should return false."), observable.isStale());
    }
