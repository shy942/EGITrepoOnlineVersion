...
*/
    private EventHandler widgetSetHandler = new EventHandler() {
        ...
        public void handleEvent(Event event) {
            // being unset
            if (event.getProperty(UIEvents.EventTags.ELEMENT) == part && event.getProperty(UIEvents.EventTags.NEW_VALUE) == null) {
                Assert.isTrue(!composite.isDisposed(), //$NON-NLS-1$
                "The widget should not have been disposed at this point");
                beingDisposed = true;
...
*/
    ...
    protected boolean createPartControl(final IWorkbenchPart legacyPart, Composite parent) {
        ...
        try {
            ...
            IStatus status = new Status(IStatus.ERROR, WorkbenchPlugin.PI_WORKBENCH, "Failed to create the part's controls", e);
            WorkbenchPartReference reference = getReference();
    ...
    void delegateSetFocus() {
        try {
            ...
            if (logger != null) {
                String msg = "Error setting focus to : " + part.getClass().getName();
