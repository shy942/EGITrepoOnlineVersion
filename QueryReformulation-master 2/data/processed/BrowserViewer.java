...
*/
    public  BrowserViewer(Composite parent, int style) {
        ...
        if (browser != null) {
            ...
            PlatformUI.getWorkbench().getHelpSystem().setHelp(browser, ContextIds.WEB_BROWSER);
        } else
...
*/
    private boolean navigate(String url) {
        ...
        if (browser != null)
            //$NON-NLS-1$
            return browser.setUrl(url, null, new String[] { "Cache-Control: no-cache" });
...
*/
    ...
    private void setURL(String url, boolean browse) {
        ...
        if (//$NON-NLS-1$
        "eclipse".equalsIgnoreCase(url))
            //$NON-NLS-1$
            url = "http://www.eclipse.org";
        else if (//$NON-NLS-1$
        "wtp".equalsIgnoreCase(url))
            //$NON-NLS-1$
...
*/
    ...
    protected void addSynchronizationListener() {
        if (fileListenerThread != null)
            return;
        fileListenerThread = new //$NON-NLS-1$
        Thread(//$NON-NLS-1$
        "Browser file synchronization") {
