...
public class FormImages {
    ...
    private abstract class AbstractImageDescriptor extends ImageDescriptor {
        ...
        public int hashCode() {
            int hash = 0;
            for (RGB fRGB : fRGBs) hash = hash * 7 + fRGB.hashCode();
            hash = hash * 7 + fLength;
            return hash;
        }
    }
    ...
    private class ComplexImageDescriptor extends AbstractImageDescriptor {
        ...
        public boolean equals(Object obj) {
            if (obj instanceof ComplexImageDescriptor) {
                ...
                if (super.equals(obj) && id.fVertical == fVertical && Arrays.equals(id.fPercents, fPercents)) {
                    if ((id.fBgRGB == null && fBgRGB == null) || (id.fBgRGB != null && id.fBgRGB.equals(fBgRGB)))
                        return true;
                    ...
                    int sum = 0;
                    for (int fPercent : fPercents) sum += fPercent;
                    if (sum >= 100)
                        return true;
                }
            }
        ...
        public int hashCode() {
            ...
            for (int fPercent : fPercents) hash = hash * 7 + Integer.valueOf(fPercent).hashCode();
            return hash;
        }
        ...
        public Image createImage(boolean returnMissingImageOnError, Device device) {
            ...
            for (int i = 0; i < colors.length; i++) colors[i] = new Color(device, fRGBs[i]);
    ...
    public Image getSectionGradientImage(Color color1, Color color2, int realtheight, int theight, int marginHeight, Display display) {
        if (color1 == null || color1.isDisposed())
            return null;
        AbstractImageDescriptor desc = new SimpleSectionGradientImageDescriptor(color1, color2, realtheight, theight, marginHeight);
    ...
    public Image getGradient(Color[] colors, int[] percents, int length, boolean vertical, Color bg, Display display) {
        if (colors.length == 0)
            return null;
        for (Color color : colors) if (color == null || color.isDisposed())
            return null;
