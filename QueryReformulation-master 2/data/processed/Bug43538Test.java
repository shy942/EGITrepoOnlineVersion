...
*/
    public void testCtrlSpace() {
        ...
        Listener listener = new Listener() {
            ...
            public void handleEvent(Event event) {
                if (event.stateMask == SWT.CTRL) {
                    assertEquals("Multiple key down events for 'Ctrl+Space'", 0, //$NON-NLS-1$
                    count++);
                }
            }
        };
        ...
        try {
            ...
            //$NON-NLS-1$
            AutomationUtil.performKeyCodeEvent(display, SWT.KeyDown, Action.findKeyCode("SPACE"));
            //$NON-NLS-1$
            AutomationUtil.performKeyCodeEvent(display, SWT.KeyUp, Action.findKeyCode("SPACE"));
