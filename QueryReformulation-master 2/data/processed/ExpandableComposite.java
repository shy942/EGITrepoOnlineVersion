    ...
    private class ExpandableLayout extends Layout implements ILayoutExtension {
        ...
        protected void layout(Composite parent, boolean changed) {
            ...
            if (textLabel != null) {
                ...
                if (textClient != null && (expansionStyle & LEFT_TEXT_CLIENT_ALIGNMENT) != 0) {
                    if (size.y < tcsize.y)
                        ty = tcsize.y / 2 - size.y / 2 + marginHeight + tvmargin;
                }
                //$NON-NLS-1$
            ...
            // max of label/text client
            int height = Math.max(tcsize.y, size.y);
            // or max of toggle
            height = Math.max(height, tsize.y);
            y += height;
            if (hasTitleBar())
                y += tvmargin;
            ...
            if (separatorControl != null) {
                y += VSPACE;
                separatorControl.setBounds(marginWidth, y, clientArea.width - marginWidth - marginWidth, SEPARATOR_HEIGHT);
                y += SEPARATOR_HEIGHT;
            }
        ...
        protected Point computeSize(Composite parent, int wHint, int hHint, boolean changed) {
            ...
            if ((expanded || (expansionStyle & COMPACT) == 0) && client != null) {
                ...
                Point dsize = null;
                Point csize = clientCache.computeSize(FormUtil.getWidthHint(cwHint, client), SWT.DEFAULT);
            ...
            Point result = new Point(width + marginWidth + marginWidth + thmargin + thmargin, height + marginHeight + marginHeight + tvmargin + tvmargin);
            return result;
        }
...
*/
    protected boolean isFixedStyle() {
        return (expansionStyle & TWISTIE) == 0 && (expansionStyle & TREE_NODE) == 0;
    }
...
*/
    protected boolean hasTitleBar() {
        return (getExpansionStyle() & TITLE_BAR) != 0 || (getExpansionStyle() & SHORT_TITLE_BAR) != 0;
    }
