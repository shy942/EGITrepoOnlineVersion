...
*/
        private final KeyStroke[] handleKeyUp(final Event event, final KeyStroke[] keyStrokes) {
...
*/
        private final KeyStroke[] insertKeyStroke(final Event event, KeyStroke[] keyStrokes) {
...
* silly
*/
            if ((SWT.NUM_LOCK == stroke.getNaturalKey()) || (SWT.CAPS_LOCK == stroke.getNaturalKey()) || (SWT.SCROLL_LOCK == stroke.getNaturalKey())) {
                return keyStrokes;
            }
            if (insertionIndex != -1) {
                ...
                final KeyStroke[][] deletedKeyStrokes = new KeyStroke[1][];
                insertionIndex = deleteSelection(keyStrokes, stroke.isComplete(), deletedKeyStrokes);
                keyStrokes = deletedKeyStrokes[0];
                if ((stroke.isComplete()) || (insertionIndex >= keyStrokes.length)) {
                    keyStrokes = insertStrokeAt(keyStrokes, stroke, insertionIndex);
                    clearInsertionIndex();
                }
            } else {
                ...
                if ((keyStrokes.length == 0) || (insertionIndex >= keyStrokes.length) || (isCursorInLastPosition())) {
        ...
        public void handleEvent(Event event) {
            switch(event.detail) {
                ...
                default:
                    // stroke
                    if (hasIncompleteStroke()) {
                        final KeyStroke[] oldKeyStrokes = getKeySequence().getKeyStrokes();
...
*/
    ...
    static {
        ...
        trappedKeys.add(SWTKeySupport.convertAcceleratorToKeyStroke(SWT.TAB | SWT.SHIFT));
...
*/
    public  KeySequenceText(Text wrappedText) {
        ...
        if (Util.isMac()) {
            ...
            final Font font = new Font(text.getDisplay(), "Lucida Grande", 13, //$NON-NLS-1$
            SWT.NORMAL);
            text.setFont(font);
...
*/
    private final int deleteSelection(final KeyStroke[] keyStrokes, final boolean allowIncomplete, final KeyStroke[][] deletedKeyStrokes) {
...
*/
        ...
        // keeps track of the start of the stroke
...
*/
        final int newLength = keyStrokesLength - (endStrokeIndex - startStrokeIndex + 1);
        ...
        System.arraycopy(keyStrokes, endStrokeIndex + 1, keyStrokeResult, startStrokeIndex, keyStrokesLength - endStrokeIndex - 1);
...
*/
        if (allowIncomplete) {
            ...
            KeyStroke incompleteStroke = KeyStroke.getInstance(modifierKeys, KeyStroke.NO_KEY);
            ...
            if ((startTextIndex + incompleteStrokeLength) <= start) {
                ...
                System.arraycopy(deletedKeyStrokes[0], startStrokeIndex, added, startStrokeIndex + 1, newLength - startStrokeIndex);
...
* @since 3.2
*/
    protected final void firePropertyChangeEvent(final KeySequence oldKeySequence) {
        if (listeners != null) {
...
*            list of key strokes.
*/
    private final KeyStroke[] insertStrokeAt(final KeyStroke[] keyStrokes, KeyStroke stroke, int index) {
        final int keyStrokesLength = keyStrokes.length;
        final KeyStroke currentStroke = (index >= keyStrokesLength) ? null : keyStrokes[index];
    ...
    public void setKeySequence(KeySequence newKeySequence) {
        ...
        if (maxStrokes != INFINITE) {
            ...
            if (maxStrokes < oldKeyStrokes.length) {
                ...
                System.arraycopy(oldKeyStrokes, 0, newKeyStrokes, 0, maxStrokes);
