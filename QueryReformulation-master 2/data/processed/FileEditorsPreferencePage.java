...
*/
    public void addResourceType(String newName, String newExtension) {
        ...
        String newFilename = (newName + (newExtension == null || newExtension.length() == 0 ? "" : //$NON-NLS-1$ //$NON-NLS-2$
        "." + newExtension)).toUpperCase();
...
*/
    ...
    protected Control createContents(Composite parent) {
        ...
        //layout the contents
        PreferenceLinkArea contentTypeArea = new PreferenceLinkArea(pageComponent, SWT.NONE, //$NON-NLS-1$
        ...
        if (resourceType != null) {
            ...
            for (int i = 0; i < array.length; i++) {
                ...
                if (defaultString != null) {
                    //$NON-NLS-1$
                    item.setText(editor.getLabel() + " " + defaultString);
                } else {
            ...
            for (int i = 0; i < contentTypes.length; i++) {
                ...
                for (int j = 0; j < array.length; j++) {
                    ...
                    if (foundItem == null) {
                        ...
                        // update the item to reflect its origin
...
*/
    private void setLockedItemText(TableItem item, String baseLabel) {
        item.setText(NLS.bind(WorkbenchMessages.FileEditorPreference_isLocked, baseLabel, ((IContentType) item.getData(DATA_FROM_CONTENT_TYPE)).getName()));
    }
    ...
    public void promptForEditor() {
        ...
        if (dialog.open() == Window.OK) {
            ...
            if (editor != null) {
                ...
                item.setData(DATA_EDITOR, editor);
                if (isEmpty) {
                    item.setText(editor.getLabel() + " " + //$NON-NLS-1$
...
*/
    public void promptForResourceType() {
        FileExtensionDialog dialog = new FileExtensionDialog(getControl().getShell(), WorkbenchMessages.FileExtension_shellTitle, IWorkbenchHelpContextIds.FILE_EXTENSION_DIALOG, WorkbenchMessages.FileExtension_dialogTitle, WorkbenchMessages.FileExtension_fileTypeMessage, WorkbenchMessages.FileExtension_fileTypeLabel);
        if (dialog.open() == Window.OK) {
...
*/
    public void removeSelectedEditor() {
        ...
        if (items.length > 0) {
            for (int i = 0; i < items.length; i++) {
                getSelectedResourceType().removeEditor((EditorDescriptor) items[i].getData(DATA_EDITOR));
                items[i].dispose();
            }
        }
