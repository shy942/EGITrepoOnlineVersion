...
public class AggregateNameObservableValue extends AbstractObservableValue {
    ...
    public  AggregateNameObservableValue(WritableValue value) {
        String[] properties = new String[] { "firstName", "middleName", "lastName" };
        ...
        listener = new IValueChangeListener() {
            ...
            public void handleValueChange(ValueChangeEvent event) {
                if (!isUpdating) {
                    fireValueChange(Diffs.createValueDiff(currentStringValue, doGetValue()));
                }
            }
        };
        int i = 0;
        for (String property : properties) {
            observableValues[i] = PojoProperties.value((Class<?>) value.getValueType(), property, String.class).observeDetail(value);
