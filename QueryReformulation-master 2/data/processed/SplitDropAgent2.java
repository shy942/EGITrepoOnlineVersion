...
*/
    ...
    private MUIElement getTargetElement(MUIElement dragElement, DnDInfo info) {
        ...
        if (dragElementLocation == EModelService.IN_SHARED_AREA || dragElementLocation == EModelService.IN_ACTIVE_PERSPECTIVE) {
            target = checkAreaEdge(dragElement, info);
        }
        if (target == null && dragElementLocation == EModelService.IN_ACTIVE_PERSPECTIVE || dragElementLocation == EModelService.OUTSIDE_PERSPECTIVE) {
    ...
    private MPartStack checkStacks(final MUIElement dragElement, final DnDInfo info) {
        ...
        List<MPartStack> stacks = ms.findElements(ms.getTopLevelWindowFor(dragElement), MPartStack.class, EModelService.PRESENTATION, new Selector() {
            ...
            public boolean select(MApplicationElement element) {
                ...
                if (dragElement instanceof MStackElement && stack == deParent && ms.countRenderableChildren(deParent) == 1)
                    return false;
                return true;
            }
        });
    ...
    private MUIElement checkAreaEdge(MUIElement dragElement, DnDInfo info) {
        ...
        List<MArea> areaList = dndManager.getModelService().findElements(persp, null, MArea.class, null, EModelService.IN_SHARED_AREA);
        if (areaList.size() > 0) {
    ...
    public boolean drop(MUIElement dragElement, DnDInfo info) {
        ...
        if (dndManager.getModelService().isLastEditorStack(relToElement)) {
            ...
            while (!(targetParent instanceof MArea)) targetParent = targetParent.getParent();
    ...
    private boolean isModified() {
        return dndManager.isModified && (relToElement instanceof MArea || relToElement instanceof MPerspective || dndManager.getModelService().isLastEditorStack(relToElement));
    }
    ...
    private void showFeedback() {
        ...
        Rectangle bb = ctrl.getBounds();
        bb = ctrl.getDisplay().map(ctrl.getParent(), null, bb);
        feedback = new SplitFeedbackOverlay(ctrl.getShell(), bb, side, ratio, isModified(), isModified());
