...
*/
        private void selectProposal(int index) {
            Assert.isTrue(index >= 0, //$NON-NLS-1$
...
*/
    private void addControlListener(Control control) {
        if (DEBUG) {
            ...
            "ContentProposalListener#installControlListener()");
        }
                ...
                switch(e.type) {
                    ...
                    case SWT.KeyDown:
                        ...
                        if (triggerKeyStroke != null) {
                            ...
                            if ((triggerKeyStroke.getModifierKeys() == KeyStroke.NO_KEY && triggerKeyStroke.getNaturalKey() == e.character) || // keycode and state must match
...
*/
        ...
        if (DEBUG) {
            System.out.println(//$NON-NLS-1$
            "ContentProposalAdapter#installControlListener() - installed");
        }
    }
    ...
    private void openProposalPopup(boolean autoActivated) {
        if (isValid()) {
            if (popup == null) {
                ...
                // must be done before getting proposals
                recordCursorPosition();
...
*/
    private boolean allowsAutoActivate() {
        return // there are specific autoactivation chars supplied
        (autoActivateString != null && autoActivateString.length() > 0) || // we autoactivate on everything
