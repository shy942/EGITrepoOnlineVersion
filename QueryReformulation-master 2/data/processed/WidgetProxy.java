...
*/
    public  WidgetProxy(final IConfigurationElement configurationElement, final String widgetAttributeName) {
        if (configurationElement == null) {
            ...
            "The configuration element backing a widget proxy cannot be null");
        }
        if (widgetAttributeName == null) {
            throw new NullPointerException(//$NON-NLS-1$
            "The attribute containing the widget class must be known");
        }
    ...
    private final boolean loadWidget() {
        if (firstLoad) {
            ...
            try {
                ...
                final String message = "The proxied widget was the wrong class";
                final IStatus status = new Status(IStatus.ERROR, WorkbenchPlugin.PI_WORKBENCH, 0, message, e);
                WorkbenchPlugin.log(message, status);
            } catch (final CoreException e) {
                final String message = "The proxied widget for '" + configurationElement.getAttribute(widgetAttributeName) + "' could not be loaded";
