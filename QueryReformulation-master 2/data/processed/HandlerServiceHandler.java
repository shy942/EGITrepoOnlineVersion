...
public class HandlerServiceHandler extends AbstractHandler {
    ...
    public boolean isEnabled() {
        ...
        // setEnabled(contexts);
        // getExecutionContext(contexts);
        IEclipseContext executionContext = contexts != null ? contexts.context : null;
        ...
        // getStaticContext(contexts);
        IEclipseContext staticContext = contexts.staticContext;
        Boolean result = (Boolean) ContextInjectionFactory.invoke(handler, CanExecute.class, executionContext, staticContext, Boolean.TRUE);
    ...
    public void setEnabled(Object evaluationContext) {
        ...
        ContextInjectionFactory.invoke(handler, SetEnabled.class, executionContext, staticContext, Boolean.TRUE);
        if (createContext) {
    ...
    public Object execute(ExecutionEvent event) throws ExecutionException {
        ...
        try {
            if (staticContext == null) {
                staticContext = localStaticContext = EclipseContextFactory.create(HandlerServiceImpl.TMP_STATIC_CONTEXT);
                staticContext.set(HandlerServiceImpl.PARM_MAP, event.getParameters());
            }
            Object result = ContextInjectionFactory.invoke(handler, Execute.class, executionContext, staticContext, missingExecute);
            if (result == missingExecute) {
                throw new ExecutionException(HANDLER_MISSING_EXECUTE_ANNOTATION, new NotHandledException(getClass().getName()));
            }
            return result;
        } finally {
