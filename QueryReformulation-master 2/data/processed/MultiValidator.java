...
*/
    ...
    private void checkObservable(IObservable target) {
        //$NON-NLS-1$
        Assert.isNotNull(target, "Target observable cannot be null");
        Assert.isTrue(realm.equals(target.getRealm()), //$NON-NLS-1$
        "Target observable must be in the same realm as MultiValidator");
    }
...
*/
    ...
    public IObservableValue getValidationStatus() {
        if (unmodifiableValidationStatus == null) {
            ObservableTracker.setIgnore(true);
            try {
                unmodifiableValidationStatus = Observables.unmodifiableObservableValue(validationStatus);
            } finally {
                ObservableTracker.setIgnore(false);
            }
            revalidate();
    ...
    protected final void revalidate() {
        class ValidationRunnable implements Runnable {
            ...
            public void run() {
                try {
                    ...
                    validationResult = ValidationStatus.error(e.getMessage(), e);
                }
            }
        }
        ...
        try {
            ...
            for (Iterator itNew = newTargets.iterator(); itNew.hasNext(); ) {
                ...
                if (newDependency == validationStatus || newDependency == unmodifiableValidationStatus || newDependency == targets || newDependency == unmodifiableTargets || newDependency == models) {
                    itNew.remove();
                }
            }
            // equality
