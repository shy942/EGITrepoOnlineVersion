...
*/
    public static MCommand createCommand(Command cmd, EModelService modelService, final MCategory categoryModel) throws NotDefinedException {
    ...
    public void init() {
        ...
        undefinedCategory = commandService.defineCategory(MApplication.class.getName(), "Application Category", //$NON-NLS-1$
        null);
        createCategories();
        createCommands();
            ...
            public void commandManagerChanged(CommandManagerEvent commandManagerEvent) {
                if (commandManagerEvent.isCommandChanged()) {
                    if (commandManagerEvent.isCommandDefined()) {
                        ...
                        final Command command = commandManagerEvent.getCommandManager().getCommand(commandId);
                        ...
                        try {
                            ...
                            final MCommand createdCommand = createCommand(command, modelService, categoryModel);
                            application.getCommands().add(createdCommand);
                        } catch (NotDefinedException e) {
                            Activator.getDefault().getLogService().log(0, "Failed to create command " + commandId, e);
                        }
                    }
                }
            }
    ...
    private void createCategory(MCategory catModel) {
        ...
        if (!category.isDefined()) {
            category.define(localize(catModel.getName(), catModel), localize(catModel.getDescription(), catModel));
        }
    }
