...
*/
    private int getNumRows(IContributionItem[] items) {
        ...
        for (int i = 0; i < items.length; i++) {
            ...
            if ((separatorFound) && (isChildVisible(items[i])) && (!items[i].isGroupMarker()) && (!items[i].isSeparator())) {
                numRows++;
...
*/
    public void refresh() {
        ...
        for (ListIterator<IContributionItem> locationIterator = displayedItems.listIterator(); locationIterator.hasNext(); ) {
...
* @param itemLocation
*/
    private void relocate(IContributionItem cbItem, int index, ArrayList<IContributionItem> contributionList, HashMap<IContributionItem, Integer> itemLocation) {
...
*/
    private void updateWrapIndices() {
        ...
        for (int i = 0; i < items.length; i++) {
            ...
            if ((!item.isSeparator()) && (!item.isGroupMarker()) && (isChildVisible(item)) && (coolItem != null) && (foundSeparator)) {
