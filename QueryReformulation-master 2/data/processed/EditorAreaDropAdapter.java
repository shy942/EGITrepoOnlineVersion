...
* the corresponding editorId */
        ...
        } else /* Open Editor for Marker (e.g. Tasks, Bookmarks, etc) */
        if (MarkerTransfer.getInstance().isSupportedType(event.currentDataType)) {
            Assert.isTrue(event.data instanceof IMarker[]);
            IMarker[] markers = (IMarker[]) event.data;
            for (int i = 0; i < markers.length; i++) {
                openNonExternalEditor(page, markers[i]);
            }
        } else /* Open Editor for resource */
        ...
        } else /* Open Editor for file from local file system */
