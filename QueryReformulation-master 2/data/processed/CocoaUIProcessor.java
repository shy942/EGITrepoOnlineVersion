...
*/
    ...
    //$NON-NLS-1$
    protected static final String CONTRIBUTOR_URI = "platform:/fragment/" + FRAGMENT_ID;
...
*/
    public void installWindowHandlers() {
        installHandler(defineCommand(//$NON-NLS-1$ //$NON-NLS-2$
        "org.eclipse.ui.category.window", //$NON-NLS-1$ //$NON-NLS-2$
        "org.eclipse.ui.cocoa.arrangeWindowsInFront", //$NON-NLS-1$
        "%command.arrangeWindows.name", "%command.arrangeWindows.desc", //$NON-NLS-1$
        CONTRIBUTOR_URI), ArrangeWindowHandler.class, CONTRIBUTOR_URI);
        installHandler(defineCommand(//$NON-NLS-1$ //$NON-NLS-2$
        "org.eclipse.ui.category.window", //$NON-NLS-1$ //$NON-NLS-2$
        "org.eclipse.ui.cocoa.minimizeWindow", "%command.minimize.name", "%command.minimize.desc", //$NON-NLS-1$ //$NON-NLS-2$
        CONTRIBUTOR_URI), MinimizeWindowHandler.class, CONTRIBUTOR_URI);
        MCommand toggleFullscreenCommand = defineCommand(//$NON-NLS-1$ //$NON-NLS-2$
        "org.eclipse.ui.category.window", //$NON-NLS-1$ //$NON-NLS-2$
        "org.eclipse.ui.cocoa.fullscreenWindow", "%command.fullscreen.name", "%command.fullscreen.desc", //$NON-NLS-1$//$NON-NLS-2$
        CONTRIBUTOR_URI);
        ...
        installKeybinding("org.eclipse.ui.contexts.window", "COMMAND+CTRL+F", //$NON-NLS-1$ //$NON-NLS-2$
        toggleFullscreenCommand);
        installHandler(defineCommand(//$NON-NLS-1$ //$NON-NLS-2$
        "org.eclipse.ui.category.window", //$NON-NLS-1$ //$NON-NLS-2$
        "org.eclipse.ui.cocoa.zoomWindow", "%command.zoom.name", "%command.zoom.desc", //$NON-NLS-1$//$NON-NLS-2$
        CONTRIBUTOR_URI), ZoomWindowHandler.class, CONTRIBUTOR_URI);
    }

    /** Add the special Cmd-W dialog helper */
    private void installCloseDialogHandlers() {
        MCommand closeDialogCommand = defineCommand(//$NON-NLS-1$
        "org.eclipse.ui.category.window", //$NON-NLS-1$
        COMMAND_ID_CLOSE_DIALOG, //$NON-NLS-1$
        "%command.closeDialog.name", "%command.closeDialog.desc", //$NON-NLS-1$
        CONTRIBUTOR_URI);
...
*/
    private void installKeybinding(String bindingContextId, String keysequence, MCommand cmd) {
        ...
        for (MBindingTable table : app.getBindingTables()) {
            ...
            if (table.getBindingContext() != null && bindingContextId.equals(table.getBindingContext().getElementId())) {
                bindingTable = table;
            }
        }
        if (bindingTable == null) {
            ...
            if (bindingContext == null) {
                statusReporter.get().report(new Status(IStatus.WARNING, CocoaUIProcessor.FRAGMENT_ID, //$NON-NLS-1$
                "No binding context exists for " + bindingContextId), StatusReporter.LOG);
                return;
            }
...
*/
    private String getClassURI(Class<?> clazz) {
        //$NON-NLS-1$
        return getBundleURI(clazz) + "/" + clazz.getName();
    }
