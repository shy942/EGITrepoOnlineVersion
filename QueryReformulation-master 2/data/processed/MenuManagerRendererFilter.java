        ...
        if (obj == null) {
            ...
            if (tmp instanceof MenuManager) {
                ...
                if (obj instanceof MPopupMenu) {
                    ...
                    if (popupMenu.getWidget() == null && menuManager.getMenu() != null) {
                        final MUIElement container = modelService.getContainer(popupMenu);
                        if (container instanceof MPart) {
                            MenuService.registerMenu(menuManager.getMenu().getParent(), popupMenu, ((MPart) container).getContext());
                        }
                    }
                }
            }
...
* @param recurse
*/
    public static void collectInfo(ExpressionInfo info, final MMenu menuModel, final MenuManagerRenderer renderer, final IEclipseContext evalContext, boolean recurse) {
...
*/
    ...
    public void cleanUp(final Menu menu, MMenu menuModel, MenuManager menuManager) {
        //$NON-NLS-1$
    ...
    public void dispose() {
        Menu[] keys = pendingCleanup.keySet().toArray(new Menu[pendingCleanup.size()]);
        for (Menu menu : keys) {
            cleanUp(menu, null, null);
        }
    }
