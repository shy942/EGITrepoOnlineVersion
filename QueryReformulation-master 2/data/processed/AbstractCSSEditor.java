...
*/
    protected void applyStyles(Object widget) {
        try {
            ...
            setCSSEngineStatuts("Apply style with " + (d2.getTime() - d1.getTime()) + "ms.");
        } catch (Exception ex) {
            handleExceptions(ex);
        }
    }
...
*/
    ...
    protected void fillTextareaWithDefaultStyleSheetContent(Object widget) {
        ...
        StringWriter writer = new StringWriter();
        try {
            CSSSerializerConfiguration configuration = (isHTMLSelector() ? getCSSHTMLSerializerConfiguration() : getCSSNativeWidgetSerializerConfiguration());
            serializer.serialize(writer, getCSSEngine(), widget, true, configuration);
