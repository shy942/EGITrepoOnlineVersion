...
public class ObservableStaleContractTest extends ObservableDelegateTest {
    ...
    public void testStaleEventObservable() throws Exception {
        ...
        assertEquals(formatFail("When notifying listeners of becoming stale the observable should be the source of the event."), observable, event.getObservable());
    }
    ...
    public void testRemoveStaleListener_RemovesListener() throws Exception {
        ...
        ensureStale(observable, false);
        delegate.setStale(observable, true);
        assertEquals(formatFail("Once removed stale listeners should not be notified of becoming stale."), 1, listener.count);
    }
    ...
    public void testStaleListenersAreNotNotifiedWhenObservableIsNoLongerStale() throws Exception {
        ...
        observable.addStaleListener(listener);
        delegate.setStale(observable, false);
        assertEquals(formatFail("Stale listeners should not be notified when the stale state changes from true to false."), 0, listener.count);
    }
    ...
    public void testObservableRealmIsCurrentOnStale() throws Exception {
        ...
        observable.addStaleListener(listener);
        delegate.setStale(observable, true);
        assertTrue(formatFail("When notifying listeners of becoming stale the observable's realm should be the current realm."), listener.isCurrentRealm);
    }
