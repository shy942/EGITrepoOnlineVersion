        ...
        if (input != null) {
            ...
            if (showInternal != isShowingInternal) {
                ...
                if (!editorTableViewer.getSelection().isEmpty()) {
                    hiddenSelectedEditor = (EditorDescriptor) editorTableViewer.getStructuredSelection().getFirstElement();
                }
            }
        }
        ...
        if (externalEditors == null) {
            ...
            try {
                ...
                if (cause instanceof CoreException) {
                    ...
                    status = new Status(IStatus.ERROR, PlatformUI.PLUGIN_ID, "Error while retrieving native editors", cause);
                }
    ...
    private IEditorDescriptor findBestExternalEditor() {
        ...
        if (program != null) {
            for (IEditorDescriptor descriptor : getExternalEditors()) {
                if (descriptor instanceof EditorDescriptor && program.equals(((EditorDescriptor) descriptor).getProgram())) {
                    return descriptor;
                }
            }
        }
...
*/
    protected void restoreWidgetValues() {
        ...
        if (fileName == null || selectedEditor == null || getFileExtension(fileName).equals(settings.get(STORE_ID_FILE_EXTENSION))) {
...
*/
    protected void saveWidgetValues() {
        ...
        if (rememberTypeButton == null || !rememberTypeButton.getSelection()) {
            ...
            //$NON-NLS-1$
            reg.setDefaultEditor("*." + getFileType(), selectedEditor);
        }
