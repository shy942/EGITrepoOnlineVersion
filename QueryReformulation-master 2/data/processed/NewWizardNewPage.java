...
class NewWizardNewPage implements ISelectionChangedListener {
    ...
    private static final String STORE_EXPANDED_CATEGORIES_ID = DIALOG_SETTING_SECTION_NAME + //$NON-NLS-1$
    "STORE_EXPANDED_CATEGORIES_ID";

    private static final String STORE_SELECTED_ID = DIALOG_SETTING_SECTION_NAME + //$NON-NLS-1$
    "STORE_SELECTED_ID";
    ...
    public  NewWizardNewPage(NewWizardSelectionPage mainPage, IWizardCategory wizardCategories, IWizardDescriptor[] primaryWizards, boolean projectsOnly) {
        ...
        if (this.primaryWizards.length > 0) {
            if (allPrimary(wizardCategories)) {
                // dont bother considering the categories as all wizards are primary
...
*/
    private void trimPrimaryWizards() {
        ...
        if (wizardCategories == null) {
            //No categories so nothing to trim
            return;
        }
...
*/
    protected void storeExpandedCategories() {
        ...
        settings.put(STORE_EXPANDED_CATEGORIES_ID, (String[]) expandedElementPaths.toArray(new String[expandedElementPaths.size()]));
    }
