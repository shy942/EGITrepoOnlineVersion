...
*/
    public void testSetIndexedSelection() {
        ...
        assertTrue("First elements do not match ", result.getFirstElement() == children[indices[0]]);
        int lastIndex = indices[indices.length - 1];
        assertTrue("Last elements do not match ", result.toArray()[result.size() - 1] == children[lastIndex]);
    }
    ...
    public void testSetInputDoesNotMaterializeEverything() {
        ...
        assertTrue("Expected less than " + fRootElement.getChildCount() + ", actual " + materializedSize, materializedSize < fRootElement.getChildCount());
