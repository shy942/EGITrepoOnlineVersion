...
*/
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        if (activityIds.size() == 1) {
            ...
            text.setText(MessageFormat.format(RESOURCE_BUNDLE.getString(//$NON-NLS-1$
            "requiresSingle"), new Object[] { activityText }));
            text = new Label(composite, SWT.NONE);
            text.setText(strings.getProperty(WorkbenchTriggerPointAdvisor.PROCEED_SINGLE, RESOURCE_BUNDLE.getString(WorkbenchTriggerPointAdvisor.PROCEED_SINGLE)));
            text.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
            text.setFont(dialogFont);
        } else {
            //$NON-NLS-1$
            ...
            CheckboxTableViewer viewer = CheckboxTableViewer.newCheckList(composite, SWT.CHECK | SWT.BORDER | SWT.SINGLE);
            ...
            text = new Label(composite, SWT.NONE);
            text.setText(strings.getProperty(WorkbenchTriggerPointAdvisor.PROCEED_MULTI, RESOURCE_BUNDLE.getString(WorkbenchTriggerPointAdvisor.PROCEED_MULTI)));
            text.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
            text.setFont(dialogFont);
        }
        ...
        dontAskButton.setText(strings.getProperty(WorkbenchTriggerPointAdvisor.DONT_ASK, RESOURCE_BUNDLE.getString(WorkbenchTriggerPointAdvisor.DONT_ASK)));
...
*/
    protected void setDetails() {
        if (selectedActivity == null) {
            detailsLabel.setText(strings.getProperty(WorkbenchTriggerPointAdvisor.NO_DETAILS, RESOURCE_BUNDLE.getString(WorkbenchTriggerPointAdvisor.NO_DETAILS)));
            //$NON-NLS-1$
            detailsText.setText("");
        } else {
            ...
            detailsLabel.setText(MessageFormat.format(RESOURCE_BUNDLE.getString("detailsLabel"), //$NON-NLS-1$
            new Object[] { name }));
...
*/
    private void setDetailButtonLabel() {
        if (!showDetails) {
            //$NON-NLS-1$
            detailsButton.setText(RESOURCE_BUNDLE.getString("showDetails"));
        } else {
            //$NON-NLS-1$
            detailsButton.setText(RESOURCE_BUNDLE.getString("hideDetails"));
        }
    }
...
*/
    ...
    protected void createButtonsForButtonBar(Composite parent) {
        ...
        detailsButton = createButton(parent, IDialogConstants.DETAILS_ID, "", //$NON-NLS-1$
        false);
