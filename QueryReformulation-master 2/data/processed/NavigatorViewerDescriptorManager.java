...
*/
    public NavigatorViewerDescriptor getNavigatorViewerDescriptor(String aViewerId) {
        NavigatorViewerDescriptor viewerDescriptor = (NavigatorViewerDescriptor) viewerDescriptors.get(aViewerId);
            ...
            if (TAG_VIEWER.equals(element.getName())) {
                ...
                if (tagPopupMenu.length == 0 && attPopupMenuId != null) {
                    ...
                    if (attPopupMenuId != null) {
                        NavigatorPlugin.logError(0, "A popupMenuId attribute and popupMenu element may NOT be concurrently specified. (see " + element.getNamespaceIdentifier() + ")", //$NON-NLS-1$ //$NON-NLS-2$
                        null);
                    } else if (tagPopupMenu.length > 1) {
                        NavigatorPlugin.logError(0, "Only one \"popupMenu\" child of \"viewer\" may be specified. (see " + element.getNamespaceIdentifier() + ")", //$NON-NLS-1$ //$NON-NLS-2$
                        null);
                    } else if (tagPopupMenu.length == 1) {
                        // valid case
                ...
                if (options.length == 1) {
                    ...
                    NavigatorPlugin.logError(0, "Only one \"options\" child of \"viewer\" may be specified. (see " + element.getNamespaceIdentifier() + ")", //$NON-NLS-1$ //$NON-NLS-2$
                    null);
                }
                return true;
            }
