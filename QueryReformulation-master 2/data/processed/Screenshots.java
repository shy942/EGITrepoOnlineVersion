...
*/
    public static String takeScreenshot(Class<?> testClass, String name, PrintStream out) {
        // ends up in testresults/linux.gtk.x86_6.0/<class>.<test>.png
        File resultsHtmlDir = getJunitReportOutput();
        if (resultsHtmlDir == null) {
            // Fallback. Warning: uses same file location on all test platforms:
            // eclipse-testing/test-eclipse/eclipse
            File eclipseDir = new File("").getAbsoluteFile();
            // ends up in testresults/html/<class>.<test>.png
    ...
    private static File getJunitReportOutput() {
        ...
        for (int i = 0; i < args.length - 1; i++) {
            if ("-junitReportOutput".equals(args[i])) {
                // see library.xml and org.eclipse.test.EclipseTestRunner
    ...
    private static void runEventQueue() {
        Display display = Display.getCurrent();
        for (int i = 0; i < 10; i++) {
            // workaround for https://bugs.eclipse.org/323272
