            ...
            if (statusHandler == null) {
                if (defaultHandlerDescriptor != null) {
                    try {
                        ...
                        logError("Errors during the default handler creating", ex);
                    }
                }
                if (statusHandler == null) {
    ...
    public void handle(StatusAdapter statusAdapter, int style) {
        try {
            ...
            if (statusAdapter == null) {
                ...
                new NullPointerException("StatusAdapter object is null"));
                return;
            }
            if (statusAdapter.getStatus() == null) {
                logError(//$NON-NLS-1$
                "Error occurred during status handling", //$NON-NLS-1$
                new NullPointerException("Status object is null"));
                return;
            }
            ...
            // notification
            if (!getStatusHandler().supportsNotification(INotificationTypes.HANDLED)) {
                generateFakeNotification(statusAdapter, style);
            }
        } catch (Throwable ex) {
            logError(statusAdapter.getStatus());
            logError("Error occurred during status handling", ex);
        }
    }
    ...
    private void doFireNotification(int type, StatusAdapter[] adapters) {
        ...
        for (int i = 0; i < oListeners.length; i++) {
            if (oListeners[i] instanceof INotificationListener) {
                ((INotificationListener) oListeners[i]).statusManagerNotified(type, adapters);
            }
        }
    }

    private void generateFakeNotification(StatusAdapter statusAdapter, int style) {
        if (((style & StatusManager.SHOW) == StatusManager.SHOW || (style & StatusManager.BLOCK) == StatusManager.BLOCK) && statusAdapter.getProperty(IProgressConstants.NO_IMMEDIATE_ERROR_PROMPT_PROPERTY) != Boolean.TRUE) {
