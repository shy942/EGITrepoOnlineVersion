...
*/
    ...
    public  MultiList(Realm realm, IObservableList<E>[] lists, Object elementType) {
        ...
        for (int i = 0; i < lists.length; i++) {
            Assert.isTrue(realm.equals(lists[i].getRealm()), //$NON-NLS-1$
            "All source lists in a MultiList must belong to the same realm");
        }
    }
...
*/
    public  MultiList(Realm realm, List<IObservableList<E>> lists, Object elementType) {
        ...
        this.elementType = elementType;
        for (IObservableList<E> list : lists) {
            Assert.isTrue(realm.equals(list.getRealm()), //$NON-NLS-1$
            "All source lists in a MultiList must belong to the same realm");
        }
    }
    ...
    private void listChanged(ListChangeEvent<? extends E> event) {
        ...
        Assert.isLegal(false, //$NON-NLS-1$
        "MultiList received a ListChangeEvent from an observable list that is not one of its sources.");
    }
    ...
    private final class MultiListItr implements Iterator<E> {
        ...
        @Override
        public E next() {
            while (iterIndex < iters.size() && !iters.get(iterIndex).hasNext()) iterIndex++;
    ...
    private class MultiListListItr implements ListIterator<E> {
        ...
        public int nextIndex() {
            int offset = 0;
            for (int i = 0; i < iterIndex; i++) offset += iters.get(i).nextIndex();
            return offset + iters.get(iterIndex).nextIndex();
        }

        @Override
        public E previous() {
            while (iterIndex >= 0 && !iters.get(iterIndex).hasPrevious()) iterIndex--;
