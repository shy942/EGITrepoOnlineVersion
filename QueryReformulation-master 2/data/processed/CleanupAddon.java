...
*/
    private static boolean shouldReactToChildVisibilityChanges(MUIElement theElement) {
        ...
        return (theElement instanceof MPartSashContainer || theElement instanceof MPartStack || theElement instanceof MCompositePart) && !theElement.getTags().contains(IPresentationEngine.HIDDEN_EXPLICITLY);
    }

    @Inject
    @Optional
    private void subscribeVisibilityChanged(@UIEventTopic(UIEvents.UIElement.TOPIC_VISIBLE) Event event) {
    ...
    @Inject
    @Optional
    private void subscribeRenderingChanged(@UIEventTopic(UIEvents.UIElement.TOPIC_TOBERENDERED) Event event) {
        MUIElement changedObj = (MUIElement) event.getProperty(UIEvents.EventTags.ELEMENT);
        ...
        if (isLastEditorStack(containerElement) || containerElement instanceof MPerspective || containerElement instanceof MPerspectiveStack)
            return;
        ...
        if (toBeRendered) {
            ...
            if (!container.isVisible() && !container.getTags().contains(IPresentationEngine.MINIMIZED))
