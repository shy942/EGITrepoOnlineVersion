...
public class MutableObservableCollectionContractTest extends ObservableDelegateTest {
    ...
    public void testRemoveAll_NoChange() throws Exception {
        ...
        assertEquals("List.removeAll on an empty list should not fire a list change event", 0, tracker.count);
    }
    ...
    public void testRetainAll_ChangeEventFiredAfterElementsAreRetained() throws Exception {
        ...
        ContainsListener listener1 = new ContainsListener(collection, element1).init();
        ContainsListener listener2 = new ContainsListener(collection, element2).init();
        // set contains the the opposite of the expected outcome to ensure they
        // get set
        listener1.contains = false;
        listener2.contains = true;
        collection.retainAll(Arrays.asList(new Object[] { element1 }));
        assertTrue(formatFail("When Collection.retainAll(...) fires the change event the element should have been retained in the Collection."), listener1.contains);
        assertFalse(formatFail("When Collection.retainAll(...) fires the change event the element should have been removed from the Collection."), listener2.contains);
    }
...
*/
    ...
    void assertDoesNotContainDuringChangeEvent(Runnable runnable, String methodName, IObservableCollection collection, Object elementNotContained) {
        ...
        assertFalse(formatFail(new StringBuffer("When ").append(methodName).append(" fires a change event the element should have been removed from the Collection.").toString()), listener.contains);
    }
...
*/
    ...
    void assertContainsDuringChangeEvent(Runnable runnable, String methodName, IObservableCollection collection, Object elementContained) {
        ...
        runnable.run();
        assertTrue(formatFail(new StringBuffer("When ").append(methodName).append(" fires a change event the element should have been added to the Collection.").toString()), listener.contains);
    }

    /* package */
    static class ContainsListener implements IChangeListener {
