...
public class DelayHandler extends AbstractHandler {
    ...
    public Object execute(ExecutionEvent event) {
        String syncStr = event.getParameter("org.eclipse.ui.monitoring.manualtesting.commands.delay.sync");
        final boolean sync = syncStr != null && !syncStr.isEmpty() && Boolean.parseBoolean(syncStr);
        String durationStr = event.getParameter("org.eclipse.ui.monitoring.manualtesting.commands.delay.duration");
        if (durationStr == null) {
            throw new IllegalArgumentException("org.eclipse.ui.monitoring.manualtesting.commands.delay.duration parameter not provided");
        }
        ...
        Runnable doDelay = new Runnable() {
            ...
            public void run() {
                ...
                System.out.printf("Delay for %.6fms complete. Actual duration: %.6fms%n", durationMs, (double) actualDuration / (double) NS_PER_MS);
            }
        };
        if (sync) {
