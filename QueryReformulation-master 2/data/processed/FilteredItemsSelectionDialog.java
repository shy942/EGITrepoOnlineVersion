...
*/
    private Label createHeader(Composite parent) {
        ...
        headerLabel.setText((getMessage() != null && getMessage().trim().length() > 0) ? getMessage() : WorkbenchMessages.FilteredItemsSelectionDialog_patternLabel);
...
*/
        protected void filterContent(GranualProgressMonitor monitor) throws CoreException {
            if (lastCompletedFilter != null && lastCompletedFilter.isSubFilter(this.itemsFilter)) {
                ...
                SubMonitor subMonitor = SubMonitor.convert(monitor, WorkbenchMessages.FilteredItemsSelectionDialog_searchJob_taskName, 100);
...
*/
    protected abstract static class SelectionHistory {
...
*/
        ...
        public void updateElement(int index) {
            FilteredItemsSelectionDialog.this.list.replace((lastFilteredItems.size() > index) ? lastFilteredItems.get(index) : null, index);
        }
...
*/
        public void reloadCache(boolean checkDuplicates, IProgressMonitor monitor) {
            ...
            SubMonitor subMonitor = SubMonitor.convert(monitor, WorkbenchMessages.FilteredItemsSelectionDialog_cacheRefreshJob, totalWork);
        ...
        private void checkDuplicates(IProgressMonitor monitor) {
            synchronized (lastFilteredItems) {
                SubMonitor subMonitor = SubMonitor.convert(monitor, WorkbenchMessages.FilteredItemsSelectionDialog_cacheRefreshJob_checkDuplicates, lastFilteredItems.size());
