...
public class TitleRegion extends Canvas {
    ...
    private class HoverListener implements MouseTrackListener, MouseMoveListener {
    ...
    private class TitleRegionLayout extends Layout implements ILayoutExtension {
        ...
        protected void layout(Composite composite, boolean flushCache) {
            ...
            layout(composite, true, carea.x, carea.y, carea.width, carea.height, flushCache);
        }

        private Point layout(Composite composite, boolean move, int x, int y, int width, int height, boolean flushCache) {
            int iwidth = width == SWT.DEFAULT ? SWT.DEFAULT : width - HMARGIN * 2;
            Point bsize = null;
            Point tsize = null;
            Point msize = null;
            if (busyLabel != null) {
            ...
            if (titleLabel.getVisible()) {
                ...
                if (twidth != SWT.DEFAULT) {
                    ...
                    int maxWidth = titleCache.computeSize(SWT.DEFAULT, SWT.DEFAULT).x;
            ...
            // size.x="+size.x); //$NON-NLS-1$ //$NON-NLS-2$
                ...
                int yloc = y + VMARGIN;
                if (bsize != null) {
                    busyLabel.setBounds(xloc, // yloc + height / 2 - bsize.y / 2,
                    yloc + (getFontHeight() >= bsize.y ? getFontHeight() : bsize.y) - 1 - bsize.y, bsize.x, bsize.y);
                ...
                if (titleLabel.getVisible()) {
                    ...
                    if (msize != null)
                        tw -= msize.x + SPACING;
                    titleLabel.setBounds(xloc, // yloc + height / 2 - tsize.y / 2,
                    yloc, tw, tsize.y);
                    // System.out.println("tw="+tw); //$NON-NLS-1$
                    xloc += tw + SPACING;
                }
                if (msize != null) {
                    menuHyperlink.setBounds(xloc, yloc + getFontHeight() / 2 - msize.y / 2, msize.x, msize.y);
                }
            }
            return size;
        }
    ...
    private void onPaint(PaintEvent e) {
        ...
        gc.fillRoundRectangle(carea.x + HMARGIN, carea.y + 2, carea.width - HMARGIN * 2, carea.height - 4, ARC_WIDTH, ARC_HEIGHT);
    ...
    private Color getHoverBackground() {
        if (hoverState == STATE_NORMAL)
            return null;
        Color color = getColor(hoverState == STATE_HOVER_FULL ? IFormColors.H_HOVER_FULL : IFormColors.H_HOVER_LIGHT);
        if (color == null)
            color = getDisplay().getSystemColor(hoverState == STATE_HOVER_FULL ? SWT.COLOR_WIDGET_BACKGROUND : SWT.COLOR_WIDGET_LIGHT_SHADOW);
        return color;
    }
    ...
    public void setHoverState(int state) {
        ...
        titleLabel.setBackground(color != null ? color : getColor(FormHeading.COLOR_BASE_BG));
        if (busyLabel != null)
            busyLabel.setBackground(color != null ? color : getColor(FormHeading.COLOR_BASE_BG));
        if (menuHyperlink != null)
            menuHyperlink.setBackground(color != null ? color : getColor(FormHeading.COLOR_BASE_BG));
        redraw();
    }
...
*/
    ...
    public final Point computeSize(int wHint, int hHint, boolean changed) {
        return ((TitleRegionLayout) getLayout()).computeSize(this, wHint, hHint, changed);
    }
