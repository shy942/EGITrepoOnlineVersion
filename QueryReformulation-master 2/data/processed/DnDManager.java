...
*/
    ...
    public void hostElement(MUIElement element, int xOffset, int yOffset) {
        ...
        if (feedbackStyle == HOSTED) {
            ...
            if (elementParent instanceof MPartStack && elementParent.getWidget() instanceof CTabFolder) {
                ...
                IPresentationEngine renderingEngine = dragWindow.getContext().get(IPresentationEngine.class);
                ...
                dragCtrl = (Control) renderingEngine.createGui(element, ctf, getModelService().getContainingContext(element));
    ...
    public void setDragHostVisibility(boolean visible) {
        if (dragHost == null || dragHost.isDisposed())
            return;
        if (visible) {
            if (dragHost.getChildren().length > 0 && dragHost.getChildren()[0] instanceof CTabFolder) {
