...
*/
    ...
    private void setupDelegateClass() throws SecurityException, NoSuchMethodException, IllegalArgumentException, IllegalAccessException, InvocationTargetException, NoSuchFieldException {
        ...
        object = invokeMethod(//$NON-NLS-1$
        OS.class, //$NON-NLS-1$
        "objc_allocateClassPair", //$NON-NLS-1$
        new Object[] { fieldObj, "SWTCocoaEnhancerDelegate", wrapPointer(0) });
        ...
        OS.class, //$NON-NLS-1$
        "class_addMethod", //$NON-NLS-1$
        new Object[] { wrapPointer(cls), wrapPointer(sel_toolbarButtonClicked_), wrapPointer(proc3), "@:@" });
        invokeMethod(//$NON-NLS-1$
        OS.class, //$NON-NLS-1$
        "objc_registerClassPair", new Object[] { wrapPointer(cls) });
    }
...
*/
    protected void allocateDelegate(Display display) {
        ...
        try {
            ...
            invokeMethod(//$NON-NLS-1$
            OS.class, //$NON-NLS-1$
            "object_setInstanceVariable", new Object[] { idValue, SWT_OBJECT, wrapPointer(delegateJniRef) });
...
*/
    ...
    void log(Exception e) {
        ...
        statusReporter.get().report(new Status(IStatus.WARNING, CocoaUIProcessor.FRAGMENT_ID, "Exception occurred during CocoaUI processing", e), //$NON-NLS-1$
        StatusReporter.LOG);
    }
...
*/
    ...
    private void monitorShellTopicChanges(@UIEventTopic(UIEvents.UIElement.TOPIC_WIDGET) Event event) {
        if (event.getProperty(UIEvents.EventTags.ELEMENT) instanceof MWindow && event.getProperty(UIEvents.EventTags.NEW_VALUE) != null) {
    ...
    protected void modifyWindowShell(MWindow window) {
        ...
        if (trimInitiallyVisible) {
            ...
            try {
                ...
                NSButton toolbarButton = (NSButton) invokeMethod(//$NON-NLS-1$
                NSWindow.class, //$NON-NLS-1$
                nsWindow, //$NON-NLS-1$
                "standardWindowButton", new Object[] { fieldValue });
                if (toolbarButton != null) {
                    toolbarButton.setTarget(delegate);
                    invokeMethod(//$NON-NLS-1$
                    NSControl.class, //$NON-NLS-1$
                    toolbarButton, //$NON-NLS-1$
                    "setAction", new Object[] { wrapPointer(sel_toolbarButtonClicked_) });
                }
            } catch (Exception e) {
                log(e);
            }
...
*/
    private boolean runCommand(String commandId) {
        ...
        Map<String, Object> params = COMMAND_ID_QUIT.equals(commandId) ? Collections.singletonMap(COMMAND_PARAMETER_ID_MAY_PROMPT, (Object) "true") : //$NON-NLS-1$
        null;
...
*/
    ...
    static long actionProc(long id, long sel, long arg0) throws Exception {
        ...
        if (jniRef[0] == 0)
            return 0;
        CocoaUIHandler delegate = (CocoaUIHandler) invokeMethod(//$NON-NLS-1$
        OS.class, //$NON-NLS-1$
