...
public class StringToNumberParser {
    ...
    private static final BigDecimal FLOAT_MAX_BIG_DECIMAL = new BigDecimal(Float.MAX_VALUE);

    private static final BigDecimal FLOAT_MIN_BIG_DECIMAL = new BigDecimal(-Float.MAX_VALUE);

    private static final BigDecimal DOUBLE_MAX_BIG_DECIMAL = new BigDecimal(Double.MAX_VALUE);

    private static final BigDecimal DOUBLE_MIN_BIG_DECIMAL = new BigDecimal(-Double.MAX_VALUE);
...
* @return result
*/
    public static ParseResult parse(Object value, NumberFormat numberFormat, boolean primitive) {
        if (!(value instanceof String)) {
            throw new IllegalArgumentException(//$NON-NLS-1$
            "Value to convert is not a String");
        }
...
*/
    public static String createParseErrorMessage(String value, ParsePosition position) {
        ...
        if (errorIndex < value.length()) {
            return BindingMessages.formatString(BindingMessages.VALIDATE_NUMBER_PARSE_ERROR, new Object[] { value, Integer.valueOf(errorIndex + 1), new Character(value.charAt(errorIndex)) });
        }
        return BindingMessages.formatString(BindingMessages.VALIDATE_NUMBER_PARSE_ERROR_NO_CHARACTER, new Object[] { value, Integer.valueOf(errorIndex + 1) });
    }
...
* @return message
*/
    public static String createOutOfRangeMessage(Number minValue, Number maxValue, NumberFormat numberFormat) {
        String min = null;
        String max = null;
        synchronized (numberFormat) {
        ...
        return BindingMessages.formatString("Validate_NumberOutOfRangeError", //$NON-NLS-1$
        new Object[] { min, max });
    }
...
*/
        ...
        "Number of type [" + number.getClass().getName() + "] is not supported.");
    }
