    ...
    public IStatus transferSettings(IPath newWorkspaceRoot) {
        try {
            ...
            if (workspaceFile == null)
                return new Status(IStatus.ERROR, WorkbenchPlugin.PI_WORKBENCH, WorkbenchMessages.WorkbenchSettings_CouldNotCreateDirectories);
            //$NON-NLS-1$
            File deltas = new File(currentLocation.toOSString(), "deltas.xml");
            if (deltas.exists()) {
                byte[] bytes = new byte[8192];
                FileInputStream inputStream = new FileInputStream(deltas);
                FileOutputStream outputStream = new FileOutputStream(new File(workspaceFile, //$NON-NLS-1$
                "deltas.xml"));
            ...
            //$NON-NLS-1$
            File workbenchModel = new File(currentLocation.toOSString(), "workbench.xmi");
            ...
            return new Status(IStatus.ERROR, WorkbenchPlugin.PI_WORKBENCH, WorkbenchMessages.Workbench_problemsSavingMsg, e);
        }
...
*/
    ...
    protected IPath getNewWorkbenchStateLocation(IPath newWorkspaceRoot) {
        //$NON-NLS-1$
        return newWorkspaceRoot.append(new Path(".metadata/.plugins/org.eclipse.e4.workbench"));
    }
}
