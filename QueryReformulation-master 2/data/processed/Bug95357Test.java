...
*/
    private void multipleEditors() throws PartInitException, CoreException {
        ...
        for (int i = 0; i < itsFilename.length; i++) {
            part[i] = IDE.openEditor(fActivePage, FileUtil.createFile(itsFilename[i], fProject), true);
        }
...
*/
    private void multipleEditorsOpen() {
        ...
        for (int i = 0; i < editors.length - 1; i++) {
            assertNull("Editor " + i + " " + editors[i].getName() + " should not be active", editors[i].getEditor(false));
        }
...
*/
    private void multipleStacks() throws PartInitException, CoreException {
        ...
        final int startAt = 2;
        fActivePage.closeAllEditors(false);
        IEditorPart last = IDE.openEditor(fActivePage, FileUtil.createFile(f1, fProject), true);
        IEditorPart current = IDE.openEditor(fActivePage, FileUtil.createFile(f2, fProject), true);
        // create the second editor stack using the second editor
        DragOperations.drag(current, new EditorDropTarget(new ExistingWindowProvider(fWin), 0, SWT.BOTTOM), false);
        ...
        for (int i = startAt; i < itsFilename.length; ++i) {
            ...
            last = current;
            current = IDE.openEditor(fActivePage, FileUtil.createFile(itsFilename[i], fProject), true);
        }
...
*/
    private void multipleStacksOnStartup() {
        ...
        for (IEditorReference editor : editors) {
            if (itsFilename[itsFilename.length - 1].equals(editor.getName())) {
                lastFile = editor;
            } else if (itsFilename[itsFilename.length - 2].equals(editor.getName())) {
        ...
        for (int i = 0; i < editors.length; ++i) {
            if (editors[i] != lastFile && editors[i] != secondLastFile) {
                assertNull("For file " + i + " " + editors[i].getName(), editors[i].getEditor(false));
            }
        }
    }
