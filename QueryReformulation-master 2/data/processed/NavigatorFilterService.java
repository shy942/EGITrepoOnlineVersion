        ...
        SafeRunner.run(new NavigatorSafeRunnable() {
            ...
            public void run() throws Exception {
                ...
                if (prefs.get(getFilterActivationPreferenceKey(), null) != null) {
                    String activatedFiltersPreferenceValue = prefs.get(getFilterActivationPreferenceKey(), null);
...
*/
    ...
    public ViewerFilter[] getVisibleFilters(boolean toReturnOnlyActiveFilters) {
        CommonFilterDescriptor[] descriptors = CommonFilterDescriptorManager.getInstance().findVisibleFilters(contentService);
...
*/
    ...
    public ViewerFilter getViewerFilter(ICommonFilterDescriptor descriptor) {
        ...
        synchronized (declaredViewerFilters) {
            ...
            if (filter == null) {
                declaredViewerFilters.put(descriptor, (filter = ((CommonFilterDescriptor) descriptor).createFilter()));
            }
        }
        return filter;
    }
    ...
    public ICommonFilterDescriptor[] getVisibleFilterDescriptors() {
        return CommonFilterDescriptorManager.getInstance().findVisibleFilters(contentService);
    }
...
*/
    public ICommonFilterDescriptor[] getVisibleFilterDescriptorsForUI() {
        return CommonFilterDescriptorManager.getInstance().findVisibleFilters(contentService, CommonFilterDescriptorManager.FOR_UI);
    }
...
*/
        ...
        if (updateFilterActivation) {
            if (nonUiVisible != null) {
                for (int i = 0; i < filterIdsToActivate.length; i++) nonUiVisible.add(filterIdsToActivate[i]);
