...
class TaskSorter extends ViewerComparator {
    ...
    static final int[] DEFAULT_PRIORITIES = { FOLDER, RESOURCE, LOCATION, DESCRIPTION, TYPE, PRIORITY, COMPLETION, CREATION_TIME };

    static final int[] DEFAULT_DIRECTIONS = { //type
    ...
    private int compareColumnValue(IMarker m1, IMarker m2, int depth) {
        ...
        switch(columnNumber) {
            ...
            case DESCRIPTION:
                {
                    /* description */
                    int result = getComparator().compare(MarkerUtil.getMessage(m1), MarkerUtil.getMessage(m2));
                    if (result == 0) {
                        return compareColumnValue(m1, m2, depth + 1);
                    }
                    return result * direction;
