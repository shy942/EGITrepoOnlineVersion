...
*/
    private ActivityDefinition getActivityDefinitionById(String id) {
        ...
        for (int i = 0; i < size; i++) {
            ActivityDefinition activityDef = (ActivityDefinition) activityDefinitions.get(i);
    ...
    private void load() throws IOException {
        ...
        for (Iterator i = activityDefinitions.iterator(); i.hasNext(); ) {
            ...
            if (//$NON-NLS-1$
            "".equals(store.getDefaultString(preferenceKey)))
                continue;
        ...
        for (int i = 0; i < defaultEnabledActivities.size(); ) {
            ...
            if (activityDef != null && activityDef.getEnabledWhen() != null) {
                ...
                StatusManager.getManager().handle(new Status(IStatus.WARNING, PlatformUI.PLUGIN_ID, //$NON-NLS-1$ //$NON-NLS-2$
        ...
        if (!activityRequirementBindingDefinitions.equals(super.activityRequirementBindingDefinitions)) {
            super.activityRequirementBindingDefinitions = Collections.unmodifiableList(new ArrayList(activityRequirementBindingDefinitions));
        ...
        if (!activityPatternBindingDefinitions.equals(super.activityPatternBindingDefinitions)) {
            super.activityPatternBindingDefinitions = Collections.unmodifiableList(new ArrayList(activityPatternBindingDefinitions));
            activityRegistryChanged = true;
        }
        if (!categoryActivityBindingDefinitions.equals(super.categoryActivityBindingDefinitions)) {
            super.categoryActivityBindingDefinitions = Collections.unmodifiableList(new ArrayList(categoryActivityBindingDefinitions));
            activityRegistryChanged = true;
        }
...
*/
    ...
    private void readActivityRequirementBindingDefinition(IConfigurationElement configurationElement) {
        ActivityRequirementBindingDefinition activityRequirementBindingDefinition = Persistence.readActivityRequirementBindingDefinition(new ConfigurationElementMemento(configurationElement), getNamespace(configurationElement));
    ...
    private void readActivityDefinition(IConfigurationElement configurationElement) {
        ActivityDefinition activityDefinition = Persistence.readActivityDefinition(new ConfigurationElementMemento(configurationElement), getNamespace(configurationElement));
        if (activityDefinition != null) {
            // this is not ideal, but core expressions takes an
            // IConfigurationElement or a w3c dom Document
            IConfigurationElement[] enabledWhen = configurationElement.getChildren(IWorkbenchRegistryConstants.TAG_ENABLED_WHEN);
    ...
    private void readActivityPatternBindingDefinition(IConfigurationElement configurationElement) {
        ActivityPatternBindingDefinition activityPatternBindingDefinition = Persistence.readActivityPatternBindingDefinition(new ConfigurationElementMemento(configurationElement), getNamespace(configurationElement));
    ...
    private void readCategoryActivityBindingDefinition(IConfigurationElement configurationElement) {
        CategoryActivityBindingDefinition categoryActivityBindingDefinition = Persistence.readCategoryActivityBindingDefinition(new ConfigurationElementMemento(configurationElement), getNamespace(configurationElement));
    ...
    private void readCategoryDefinition(IConfigurationElement configurationElement) {
        CategoryDefinition categoryDefinition = Persistence.readCategoryDefinition(new ConfigurationElementMemento(configurationElement), getNamespace(configurationElement));
