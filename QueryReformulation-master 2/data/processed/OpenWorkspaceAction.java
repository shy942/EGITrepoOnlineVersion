    ...
    class WorkspaceMRUAction extends Action {
        ...
        private String location;
         ...
         WorkspaceMRUAction(String location, ChooseWorkspaceData data) {
            // preserve the location directly -
    ...
    private IContributionItem[] getContributionItems() {
        ...
        for (int i = 0; i < workspaces.length; i++) {
            if (workspaces[i] != null && !workspaces[i].equals(current)) {
                list.add(new ActionContributionItem(new WorkspaceMRUAction(workspaces[i], data)));
            }
        }
        if (list.size() > 0) {
...
*/
    private String promptForWorkspace() {
        ...
        ChooseWorkspaceDialog dialog = new ChooseWorkspaceWithSettingsDialog(window.getShell(), data, true, false);
        dialog.prompt(true);
...
*/
    private String buildCommandLine(String workspace) {
        ...
        if (property == null) {
            MessageDialog.openError(window.getShell(), IDEWorkbenchMessages.OpenWorkspaceAction_errorTitle, NLS.bind(IDEWorkbenchMessages.OpenWorkspaceAction_errorMessage, PROP_VM));
            return null;
        }
