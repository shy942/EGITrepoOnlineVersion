...
*/
    public  ContributedAction(IServiceLocator locator, IConfigurationElement element) throws CommandNotMappedException {
        ...
        if (commandId == null) {
            ...
            if (obj instanceof IConfigurationElement) {
                ...
                if (contributionId == null) {
                    throw new CommandNotMappedException(//$NON-NLS-1$
                    "Action " + actionId + //$NON-NLS-1$
                    " configuration element invalid");
                }
            }
            // legacy bridge part
            ...
            if (mapping == null) {
                throw new CommandNotMappedException(//$NON-NLS-1$
                "No action mapping service available");
            }
        ...
        if (commandId == null) {
            throw new CommandNotMappedException(//$NON-NLS-1$
            "Action " + actionId + " in contribution " + //$NON-NLS-1$
            contributionId + //$NON-NLS-1$
            " not mapped to a command");
        }
