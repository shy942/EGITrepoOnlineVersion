...
*/
    ...
    private Object doCreate(String uriString, IEclipseContext context, IEclipseContext staticContext) {
        ...
        if (uriString.startsWith("platform:/plugin/")) {
            //$NON-NLS-1$
            Activator.log(LogService.LOG_ERROR, //$NON-NLS-1$
            "platform-style URIs deprecated for referencing types: " + uriString);
            //$NON-NLS-1$ //$NON-NLS-2$
            uriString = uriString.replace("platform:/plugin/", "bundleclass://");
            //$NON-NLS-1$
            Activator.log(LogService.LOG_ERROR, "URI rewritten as: " + uriString);
        }
        ...
        if (bundle != null) {
            ...
            contribution = null;
            Activator.log(LogService.LOG_ERROR, //$NON-NLS-1$
            "Unable to retrieve the bundle from the URI: " + uriString);
        }
        return contribution;
    }
        ...
        if (uri.segmentCount() > 1) {
            ...
            try {
                ...
                if (contribution == null) {
                    ...
                    "Unable to load class '" + clazz + "' from bundle '" + bundle.getBundleId() + //$NON-NLS-1$
                    "'";
    ...
    protected void processLanguages() {
        ...
        for (int i = 0; i < languageElements.length; i++) {
            ...
            try {
                //$NON-NLS-1$
                languages.put(//$NON-NLS-1$
                languageElement.getAttribute("name"), //$NON-NLS-1$
                languageElement.createExecutableExtension("contributionFactory"));
    ...
    protected Bundle getBundle(URI platformURI) {
        if (platformURI.authority() == null) {
            //$NON-NLS-1$
            Activator.log(LogService.LOG_ERROR, "Failed to get bundle for: " + platformURI);
            return null;
        }
