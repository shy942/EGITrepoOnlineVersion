...
public class EventLoopMonitorThread extends Thread {
    ...
    private static final Tracer tracer = Tracer.create(TRACE_PREFIX, PreferenceConstants.PLUGIN_ID + TRACE_EVENT_MONITOR);
...
*/
        public void checkParameters() throws IllegalArgumentException {
            ...
            if (longEventWarningThreshold <= 0) {
                problems.append(NEW_LINE_AND_BULLET + NLS.bind(Messages.EventLoopMonitorThread_warning_threshold_error_1, longEventWarningThreshold));
            }
            if (longEventErrorThreshold < longEventWarningThreshold) {
                problems.append(NEW_LINE_AND_BULLET + NLS.bind(Messages.EventLoopMonitorThread_error_threshold_too_low_error_2, longEventErrorThreshold, longEventWarningThreshold));
            }
            if (deadlockThreshold <= 0) {
                problems.append(NEW_LINE_AND_BULLET + NLS.bind(Messages.EventLoopMonitorThread_deadlock_error_1, deadlockThreshold));
            } else if (deadlockThreshold <= longEventErrorThreshold) {
                problems.append(NEW_LINE_AND_BULLET + NLS.bind(Messages.EventLoopMonitorThread_deadlock_threshold_too_low_error_2, deadlockThreshold, longEventErrorThreshold));
            }
            if (problems.length() != 0) {
                throw new IllegalArgumentException(NLS.bind(Messages.EventLoopMonitorThread_invalid_argument_error_1, problems.toString()));
            }
        }
    }
            ...
            switch(event.type) {
                case SWT.PreEvent:
                    if (!doesEventIndicateResponsiveUI(event.detail)) {
                        // Ignore events that may be produced during a UI freeze.
                        break;
                    }
                    nestingLevel++;
...
*/
        ...
        private void saveAndResetNestingLevel() {
            if (nestingLevelStackSize < nestingLevelStack.length) {
                ...
                MonitoringPlugin.logError(NLS.bind(Messages.EventLoopMonitorThread_max_event_loop_depth_exceeded_1, nestingLevelStack.length), null);
                shutdown();
            }
        }
...
*/
    private static class EventHistory {
        ...
        // Index of the first recorded event.
        private int start;
...
*/
    ...
    private final AtomicReference<LongEventInfo> eventToPublish = new AtomicReference<LongEventInfo>(null);
...
*/
        ...
        // Immediately updated by resetStalledEventState.
        long pollingDelay = 0;
        // Immediately updated by resetStalledEventState.
        long grabStackSampleAt = 0;
        // Immediately updated by resetStalledEventState.
        long lastEventStartOrResumeTime = 0;
...
*/
            if (lastEventStartOrResumeTime != currEventStartOrResumeTime || starved) {
                ...
                if (tracer != null && starved) {
                    if (starvedAwakeCurrentCycle) {
                        tracer.trace(String.format(//$NON-NLS-1$
                        "Starvation detected! Polling loop took a significant amount of threshold: %dms", awakeDuration));
                    }
                    if (starvedAsleepCurrentCycle) {
                        tracer.trace(String.format(//$NON-NLS-1$
                        "Starvation detected! Expected to sleep for %dms but actually slept for %dms", sleepFor, sleepDuration));
                    }
                }
            } else if (lastEventStartOrResumeTime != 0) {
                ...
                if (deadlockTimerStart != 0) {
                    ...
                    if (totalDuration >= deadlockThreshold) {
                        ...
                        if (uiThreadFilter.shouldLogEvent(stackSamples, numSamples, uiThreadId)) {
                            logEvent(new UiFreezeEvent(deadlockTimerStart, totalDuration, Arrays.copyOf(stackSamples, numSamples), true, starvedAwake, starvedAsleep));
                            // Don't log potential deadlock more than once.
                            deadlockTimerStart = 0;
                        }
                    }
                }
            ...
            if (eventSnapshot != null) {
                ...
                if (uiThreadFilter.shouldLogEvent(stackSamples, numSamples, uiThreadId)) {
                    logEvent(new UiFreezeEvent(eventSnapshot.start, eventSnapshot.duration, Arrays.copyOf(stackSamples, numSamples), false, starvedAwake, starvedAsleep));
                }
                resetStalledEventState = true;
            }
    ...
    private ThreadInfo[] captureThreadStacks(boolean dumpAllThreads) {
        if (dumpAllThreads) {
            ThreadInfo[] threadStacks = threadMXBean.dumpAllThreads(dumpLockedMonitors, dumpLockedSynchronizers);
            ...
            for (int i = 0; i < threadStacks.length; i++) {
                ...
                if (threadId != monitoringThreadId) {
                    if (threadId == uiThreadId) {
                        ...
                        // Skip the non-interesting thread.
                        continue;
                    }
        ...
        for (IConfigurationElement element : configElements) {
            try {
                ...
                if (object instanceof IUiFreezeEventLogger) {
                    ...
                    MonitoringPlugin.logWarning(NLS.bind(Messages.EventLoopMonitorThread_invalid_logger_type_error_4, new Object[] { object.getClass().getName(), IUiFreezeEventLogger.class.getClass().getSimpleName(), EXTENSION_ID, element.getContributor().getName() }));
                }
    ...
    private void logEvent(UiFreezeEvent event) {
        ...
        for (int i = 0; i < externalLoggers.size(); i++) {
            ...
            try {
                ...
                i--;
                MonitoringPlugin.logError(NLS.bind(Messages.EventLoopMonitorThread_external_exception_error_1, currentLogger.getClass().getName()), t);
            }
        }
    }
}
