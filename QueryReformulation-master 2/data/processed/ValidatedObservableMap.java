...
*/
    public  ValidatedObservableMap(final IObservableMap target, final IObservableValue validationStatus) {
    ...
    private void applyDiff(MapDiff diff, Map map) {
        for (Iterator iterator = diff.getRemovedKeys().iterator(); iterator.hasNext(); ) map.remove(iterator.next());
        for (Iterator iterator = diff.getChangedKeys().iterator(); iterator.hasNext(); ) {
            Object key = iterator.next();
            map.put(key, diff.getNewValue(key));
        }
        for (Iterator iterator = diff.getAddedKeys().iterator(); iterator.hasNext(); ) {
            Object key = iterator.next();
            map.put(key, diff.getNewValue(key));
        }
    }
