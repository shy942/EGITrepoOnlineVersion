...
*/
    ...
    public final <T extends MApplicationElement> T createModelElement(Class<T> elementType) {
        ...
        "Unsupported model object type: " + elementType.getCanonicalName());
    }

    private <T> void findElementsRecursive(MApplicationElement searchRoot, Class<T> clazz, Selector matcher, List<T> elements, int searchFlags) {
        ...
        if (classMatch && matcher.select(searchRoot)) {
            if (!elements.contains(searchRoot)) {
                @SuppressWarnings("unchecked") T element = (T) searchRoot;
...
*/
            ...
            if (searchRoot instanceof MWindow && (searchFlags & OUTSIDE_PERSPECTIVE) == 0 && (primaryStack = getPrimaryPerspectiveStack((MWindow) searchRoot)) != null) {
            ...
            if (searchContainer instanceof MPerspectiveStack) {
                ...
                @SuppressWarnings("unchecked") MElementContainer<MUIElement> container = (MElementContainer<MUIElement>) searchRoot;
...
*/
    ...
    private void showElementInWindow(MWindow window, MUIElement element) {
        ...
        if (parent == null && element instanceof MWindow) {
            ...
            @SuppressWarnings("unchecked") MElementContainer<MUIElement> container = (MElementContainer<MUIElement>) parent;
    ...
    @Override
    public <T extends MUIElement> void move(T element, MElementContainer<? super T> newParent, boolean leavePlaceholder) {
    ...
    @Override
    public <T extends MUIElement> void move(T element, MElementContainer<? super T> newParent, int index, boolean leavePlaceholder) {
        // Cache where we were
...
*/
    ...
    public void hideLocalPlaceholders(MWindow window, MPerspective perspective) {
        List<MPlaceholder> globals = findElements(window, null, MPlaceholder.class, null, OUTSIDE_PERSPECTIVE | IN_SHARED_AREA);
