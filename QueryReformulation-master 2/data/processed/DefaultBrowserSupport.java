...
public class DefaultBrowserSupport extends AbstractWorkbenchBrowserSupport {
    ...
    protected IWebBrowser getExistingWebBrowser(String browserId) {
        try {
            ...
            if (obj instanceof IWebBrowser)
                ...
                @SuppressWarnings("rawtypes") HashMap wmap = (HashMap) obj;
    ...
    public IWebBrowser createBrowser(int style, String browserId, String name, String tooltip) throws PartInitException {
        ...
        if ((style & AS_EXTERNAL) != 0 || (WebBrowserPreference.getBrowserChoice() != WebBrowserPreference.INTERNAL && !isHelpEditor) || !WebBrowserUtil.canUseInternalWebBrowser()) {
        ...
        if (webBrowser instanceof InternalBrowserInstance) {
            ...
            @SuppressWarnings("unchecked") HashMap<Integer, IWebBrowser> wmap = (HashMap<Integer, IWebBrowser>) browserIdMap.get(browserId);
            if (wmap == null) {
                wmap = new HashMap();
    ...
    protected void removeBrowser(IWebBrowser browser) {
        ...
        if (browser instanceof InternalBrowserInstance) {
            ...
            if (entry != null && entry instanceof HashMap) {
                @SuppressWarnings("rawtypes") HashMap wmap = (HashMap) entry;
                wmap.remove(key);
                if (wmap.isEmpty())
