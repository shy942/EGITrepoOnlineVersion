...
public class BindingTable {
    ...
    private static int compareSchemes(String[] activeSchemeIds, final String schemeId1, final String schemeId2) {
...
*/
    ...
    public void addBinding(Binding binding) {
        if (!getId().equals(binding.getContextId())) {
            ...
            "Binding context " + binding.getContextId() + " does not match " + //$NON-NLS-1$
            getId());
        }
    ...
    private void evaluateOrderedBindings(TriggerSequence sequence, Binding binding) {
        ...
        if (bindingList != null) {
            if (bindingList.isEmpty()) {
                ...
                int rc = compareSchemes(BEST_SEQUENCE.getActiveSchemes(), msb.getSchemeId(), lsb.getSchemeId());
                if (rc == 0) {
                    ...
                    while (i.hasNext() && rc == 0) {
                        Binding next = i.next();
                        rc = compareSchemes(BEST_SEQUENCE.getActiveSchemes(), prev.getSchemeId(), next.getSchemeId());
                        if (rc == 0) {
