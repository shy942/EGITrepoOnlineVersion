...
public class PartRenderingEngineTests {
    ...
    private boolean checkMacBug466636() {
        if (Platform.OS_MACOSX.equals(Platform.getOS())) {
            System.out.println("skipping " + PartRenderingEngineTests.class.getName() + "#" + this.getClass().getSimpleName() + " on Mac for now, see bug 466636");
            return true;
        }
        return false;
    }
    ...
    public void testPartStack_SetActiveChildBug299379() throws Exception {
        ...
        assertEquals("Activating another part should've altered the tab folder's selection", 1, tabFolder.getSelectionIndex());
    }
    ...
    public void testPartStack_SetActiveChild2Bug299379() throws Exception {
        ...
        assertEquals("Showing a part should alter the tab folder's selection", 1, tabFolder.getSelectionIndex());
    }
    ...
    public void testPartStack_SetActiveChild3Bug299379() throws Exception {
        ...
        assertEquals(0, tabFolder.getSelectionIndex());
        assertEquals("The shown part should be the active part", shownPart, stack.getSelectedElement());
    }
    ...
    public void testPartStack_SetActiveChild4Bug299379() throws Exception {
        ...
        assertEquals("Switching the active child should've changed the folder's selection", 1, tabFolder.getSelectionIndex());
    }
    ...
    public void testPartStack_SetActiveChild5Bug295250() throws Exception {
        ...
        partB.setContributionURI("bundleclass://org.eclipse.e4.ui.tests/org.eclipse.e4.ui.tests.workbench.SampleView");
        stack.getChildren().add(partB);
        assertEquals("Adding a part to a stack should not cause the stack's active child to change", partA, stack.getSelectedElement());
        assertNull("The object should not have been instantiated", partB.getObject());
    }
    ...
    public void testCreateGuiBug301021() throws Exception {
        ...
        // the first window
        EPartService service = window2.getContext().get(EPartService.class);
    ...
    public void testPart_ToBeRendered2() throws Exception {
        ...
        assertEquals("Rendering another part in the stack should not change the selection", 0, tabFolder.getSelectionIndex());
        assertEquals(partA, stack.getSelectedElement());
    ...
    public void testCTabItem_SetControl_Bug304211() {
        ...
        wb = new E4Workbench(application, appContext);
        wb.createAndRunUI(window);
        IPresentationEngine engine = (IPresentationEngine) appContext.get(IPresentationEngine.class.getName());
        CTabFolder folder = (CTabFolder) stack.getWidget();
        CTabItem itemA = folder.getItem(0);
        assertEquals("The presentation engine should have created the part and set it", partA.getWidget(), itemA.getControl());
        MPart partB = ems.createModelElement(MPart.class);
        partB.setElementId("partB");
        partB.setContributionURI("bundleclass://org.eclipse.e4.ui.tests/org.eclipse.e4.ui.tests.workbench.SampleView");
        ...
        assertNull("For a stack, the object will not be rendered unless explicitly required", item2.getControl());
        // ask the engine to render the part
        engine.createGui(partB);
        assertEquals("The presentation engine should have created the part and set it", partB.getWidget(), item2.getControl());
        // select the new part to display it to the user
        stack.setSelectedElement(partB);
        assertEquals("Selecting the element should not have changed anything", partB.getWidget(), item2.getControl());
    }
    ...
    public void testSelectedElementNullingTBR() {
        ...
        partB.setToBeRendered(false);
        assertTrue("Changing the TBR of a non-selected element should not change the value of the container's seletedElement", container.getSelectedElement() == partA);
        // Ensure that changing the TBR state of the selected element results in
        // it going null
        container.setSelectedElement(partA);
        partA.setToBeRendered(false);
        assertTrue("Changing the TBR of the selected element should have set the field to null", container.getSelectedElement() == null);
    }
    ...
    public void testSelectedElementNullingParentChange() {
        ...
        assertTrue("Changing the parent of a non-selected element should not change the value of the container's seletedElement", container.getSelectedElement() == partA);
        // Ensure that changing the parent of the selected element
        // results in it going null
        container.setSelectedElement(partA);
        container.getChildren().remove(partA);
        assertTrue("Changing the parent of the selected element should have set the field to null", container.getSelectedElement() == null);
    }
    ...
    public void testRemoveGuiBug324228_1() {
        ...
        appContext.get(EModelService.class).removePerspectiveModel(perspectiveB, window);
        assertFalse(view.isDestroyed());
    ...
    public void testBug326087() {
        ...
        application.getContext().get(IPresentationEngine.class).removeGui(sashContainer);
    ...
    public void testBug334644_01() {
        ...
        assertNotNull("Rendered window should have a widget", window.getWidget());
        assertNotNull("Rendered window should have a context", window.getContext());
    }
...
*/
    ...
    public void testBut336225() {
        ...
        appContext.get(IPresentationEngine.class).removeGui(window);
        assertFalse("The shell should not have been disposed first", impl.shellEagerlyDestroyed);
    }
    ...
    public void testBug330662() {
        ...
        partService.switchPerspective(perspectiveB);
        window.getContext().get(IPresentationEngine.class).removeGui(perspectiveA);
...
*/
    ...
    private void testBug326175(boolean visible) {
        ...
        if (visible) {
            assertEquals(detachedWindow.getContext(), window.getContext().getActiveChild());
        } else {
            assertEquals(part.getContext(), window.getContext().getActiveChild());
        }
    }
    ...
    public void testBug342439_01() {
        ...
        assertEquals(2, folder.getItemCount());
        assertNull(partA.getWidget());
        window.getContext().get(EPartService.class).showPart(partA, PartState.ACTIVATE);
        assertEquals(2, folder.getItemCount());
        assertNotNull(partA.getWidget());
    }
    ...
    public void testBug343442() {
        ...
        assertEquals(partStack.getWidget(), ((Control) toolBar.getWidget()).getParent().getParent());
    }
    ...
    public void testBug372226() {
        ...
        MPart part = ems.createModelElement(MPart.class);
        window.getSharedElements().add(part);
        appContext.get(EModelService.class).hostElement(part, window, subShell, window.getContext());
    ...
    public void test_persistState_371087() {
        ...
        view.errorOnWidgetDisposal = true;
        part.setToBeRendered(false);
        assertTrue("The view should have been destroyed", view.isStatePersisted());
        assertNull(part.getObject());
        assertNull(part.getContext());
    }
