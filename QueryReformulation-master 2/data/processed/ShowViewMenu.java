...
*/
    ...
    private CommandContributionItemParameter getItem(String viewId) {
        ...
        CommandContributionItemParameter parms = new PluginCCIP(desc, window, viewId, IWorkbenchCommandConstants.VIEWS_SHOW_VIEW, CommandContributionItem.STYLE_PUSH);
        parms.label = label;
        parms.icon = desc.getImageDescriptor();
        parms.parameters = new HashMap();
        parms.parameters.put(VIEW_ID_PARM, viewId);
        if (makeFast) {
            parms.parameters.put(IWorkbenchCommandConstants.VIEWS_SHOW_VIEW_PARM_FASTVIEW, //$NON-NLS-1$
            "true");
        }
        return parms;
    }
    ...
    private ParameterizedCommand getCommand(ICommandService commandService, final boolean makeFast) {
        ...
        if (makeFast) {
            try {
                ...
                parms = new Parameterization[] { new Parameterization(parmDef, //$NON-NLS-1$
                "true") };
            } catch (NotDefinedException e) {
            }
        }
