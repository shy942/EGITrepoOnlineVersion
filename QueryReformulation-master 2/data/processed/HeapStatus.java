...
*/
    public  HeapStatus(Composite parent, IPreferenceStore prefStore) {
        ...
        //$NON-NLS-1$
        ImageDescriptor imageDesc = WorkbenchImages.getWorkbenchImageDescriptor("elcl16/trash.png");
        ...
        // medium red
        lowMemCol = new Color(display, 255, 70, 70);
        // light orange
        freeMemCol = new Color(display, 255, 190, 125);
...
*/
    private void setMark() {
        // get up-to-date stats before taking the mark
        updateStats();
        mark = usedMem;
        hasChanged = true;
        redraw();
...
*/
    ...
    private void paintButton(GC gc) {
        ...
        if (gcImage != null) {
            // button y
            int by = (rect.height - imgBounds.height) / 2 + rect.y;
    ...
    private void paintCompositeMaxUnknown(GC gc) {
        ...
        int h = rect.height;
        // button width
        int bw = imgBounds.width;
        // divider x
        int dx = x + w - bw - 2;
        // status width
        int sw = w - bw - 3;
        // used mem width
        int uw = (int) (sw * usedMem / totalMem);
        // used mem right edge
        int ux = x + 1 + uw;
        if (bgCol != null) {
    ...
    private void paintCompositeMaxKnown(GC gc) {
        ...
        int sw = w - bw - 3;
        // used mem width
        int uw = (int) (sw * usedMem / maxMem);
        // used mem right edge
        int ux = x + 1 + uw;
        // current total mem width
        int tw = (int) (sw * totalMem / maxMem);
        // current total mem right edge
        int tx = x + 1 + tw;
        gc.setBackground(bgCol);
        gc.fillRectangle(rect);
        gc.setForeground(sepCol);
