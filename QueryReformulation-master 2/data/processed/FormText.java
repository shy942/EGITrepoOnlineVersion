...
*/
    ...
    // private fields
    //"true".equalsIgnoreCase(Platform.getDebugOption(FormUtil.DEBUG_TEXT));
    private static final boolean DEBUG_TEXT = false;

    //"true".equalsIgnoreCase(Platform.getDebugOption(FormUtil.DEBUG_TEXTSIZE));
    private static final boolean DEBUG_TEXTSIZE = false;

    //"true".equalsIgnoreCase(Platform.getDebugOption(FormUtil.DEBUG_FOCUS));
    ...
    private class FormTextLayout extends Layout implements ILayoutExtension {
        ...
        public Point computeSize(Composite composite, int wHint, int hHint, boolean changed) {
            ...
            if (DEBUG_TEXT) {
                ...
                "FormText computeSize: " + (stop - start) + //$NON-NLS-1$
                "ms");
            }
            if (DEBUG_TEXTSIZE) {
                //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                System.out.println("FormText (" + model.getAccessibleText() + "), computeSize: wHint=" + wHint + ", result=" + result);
            }
            return result;
        }
        ...
        protected void layout(Composite composite, boolean flushCache) {
            ...
            if (DEBUG_TEXTSIZE) {
                //$NON-NLS-1$ //$NON-NLS-2$
                System.out.println("FormText layout (" + model.getAccessibleText() + "), carea=" + carea);
            }
...
*/
    ...
    private void handleFocusChange() {
        if (DEBUG_FOCUS) {
            System.out.println(//$NON-NLS-1$
            "Handle focus change: hasFocus=" + hasFocus + ", mouseFocus=" + //$NON-NLS-1$
            mouseFocus);
        }
        if (hasFocus) {
...
*/
    ...
    public Point computeSize(int wHint, int hHint, boolean changed) {
        ...
        if (DEBUG_TEXTSIZE)
            //$NON-NLS-1$
            System.out.println("FormText Computed size: " + trim);
