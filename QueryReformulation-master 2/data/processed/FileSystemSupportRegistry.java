...
*/
    public  FileSystemSupportRegistry() {
        ...
        IExtensionPoint point = Platform.getExtensionRegistry().getExtensionPoint(IDEWorkbenchPlugin.IDE_WORKBENCH, FILESYSTEM_SUPPORT);
        if (point == null) {
            return;
        }
...
*/
    private FileSystemConfiguration newConfiguration(final IConfigurationElement element) {
        ...
        SafeRunner.run(new ISafeRunnable() {
            ...
            public void run() {
                try {
                    contributors[0] = (FileSystemContributor) IDEWorkbenchPlugin.createExtension(element, ATT_CLASS);
        ...
        FileSystemConfiguration config = new FileSystemConfiguration(name, contributors[0], fileSystem);
        return config;
    }
...
*/
    public FileSystemConfiguration[] getConfigurations() {
        if (allConfigurations == null) {
            allConfigurations = new FileSystemConfiguration[registeredContributions.size() + 1];
            allConfigurations[0] = defaultConfiguration;
            Iterator iterator = registeredContributions.iterator();
            int index = 0;
            while (iterator.hasNext()) {
                allConfigurations[++index] = (FileSystemConfiguration) iterator.next();
            }
        }
