...
public class ListDiffTest extends TestCase {
    ...
    public void testAccept_Replace_RemoveBeforeAdd() {
        createListDiff(remove(0, "element0"), add(0, "element1")).accept(visitor);
        assertEquals("replace(0,element0,element1)", visitor.log);
    }

    public void testAccept_Replace_AddBeforeRemove() {
        createListDiff(add(0, "element1"), remove(1, "element0")).accept(visitor);
        assertEquals("replace(0,element0,element1)", visitor.log);
    }

    public void testAccept_AllPatterns() {
        createListDiff(new ListDiffEntry[] { // Replace (remove before add)
        remove(0, "element0"), add(0, "element1"), // Replace (add before remove)
        add(0, "element3"), remove(1, "element2"), // Remove
        remove(1, "element4"), // Add
        add(2, "element5"), // Move forward (remove before add)
        remove(5, "element6"), add(6, "element6"), // Move forward (add before remove)
        add(7, "element6"), remove(5, "element6"), // Move backward (remove before add)
        remove(12, "element7"), add(11, "element7"), // Move backward (add before remove)
        add(11, "element7"), remove(13, "element7"), // Remove then add in place -- treat as replace
        remove(11, "element8"), add(11, "element8"), // add and remove
        add(12, "element9"), remove(12, "element9") }).accept(visitor);
        assertEquals("replace(0,element0,element1), replace(0,element2,element3), " + "remove(1,element4), " + "add(2,element5), " + "move(5,6,element6), move(5,6,element6), " + "move(12,11,element7), move(12,11,element7), " + "replace(11,element8,element8), " + "add(12,element9), remove(12,element9)", visitor.log);
    }
