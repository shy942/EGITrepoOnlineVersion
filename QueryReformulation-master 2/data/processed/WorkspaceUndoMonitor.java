...
*/
    private  WorkspaceUndoMonitor() {
        if (Policy.DEBUG_UNDOMONITOR) {
            //$NON-NLS-1$
            System.out.println(DEBUG_PREFIX + "Installing listeners");
        }
...
*/
    public void shutdown() {
        if (Policy.DEBUG_UNDOMONITOR) {
            //$NON-NLS-1$
            System.out.println(DEBUG_PREFIX + "Shutting Down");
        }
...
*/
    private void checkOperationHistory() {
        if (Policy.DEBUG_UNDOMONITOR) {
            //$NON-NLS-1$
            System.out.println(DEBUG_PREFIX + "Checking Operation History...");
        }
        IUndoableOperation currentOp = getOperationHistory().getUndoOperation(WorkspaceUndoUtil.getWorkspaceUndoContext());
...
*/
    private void flushWorkspaceHistory(IUndoableOperation op) {
        if (Policy.DEBUG_UNDOMONITOR) {
            System.out.println(DEBUG_PREFIX + "Flushing undo history due to " + //$NON-NLS-1$
            op);
        }
...
*/
    private void resetChangeCount() {
        ...
        if (Policy.DEBUG_UNDOMONITOR) {
            //$NON-NLS-1$
            System.out.println(DEBUG_PREFIX + "Resetting change count to 0");
        }
    }
...
*/
    private void incrementChangeCount() {
        numChanges++;
        if (Policy.DEBUG_UNDOMONITOR) {
            System.out.println(DEBUG_PREFIX + "Incrementing workspace change count.  Count = " + //$NON-NLS-1$
            numChanges);
        }
    }
}
