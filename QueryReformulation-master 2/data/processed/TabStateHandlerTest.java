...
public class TabStateHandlerTest {
    ...
    public void testValidateElement() throws Exception {
        ...
        assertTrue(handler.validateElement(MBasicFactory.INSTANCE.createPartStack()));
    }

    @Test
    public void testValidateElementWhenInvalidElement() throws Exception {
        assertFalse(handler.validateElement(MBasicFactory.INSTANCE.createTrimBar()));
        assertFalse(handler.validateElement(null));
    }

    @Test
    public void testValidateValues() throws Exception {
        assertTrue(handler.validateValues(null, placeHolder(MBasicFactory.INSTANCE.createPart())));
        ...
        assertTrue(handler.validateValues(null, CSSConstants.CSS_CONTENT_CHANGE_CLASS));
    }

    @Test
    public void testValidateValuesWhenInvalidValue() throws Exception {
        assertFalse(handler.validateValues(null, MBasicFactory.INSTANCE.createPart()));
    ...
    public void testHandleEventWhenTabBusyEvent() throws Exception {
        ...
        // when
        handler.handleEvent(event(new EventParam(UIEvents.EventTags.ELEMENT, part), new EventParam(UIEvents.EventTags.NEW_VALUE, CSSConstants.CSS_BUSY_CLASS), new EventParam(UIEvents.EventTags.OLD_VALUE, null)));
        // then
    ...
    public void testHandleEventWhenTabIdleEvent() throws Exception {
        ...
        renderer.tabItemForPart = tabItem;
        // when
        handler.handleEvent(event(new EventParam(UIEvents.EventTags.ELEMENT, part), new EventParam(UIEvents.EventTags.NEW_VALUE, null), new EventParam(UIEvents.EventTags.OLD_VALUE, CSSConstants.CSS_BUSY_CLASS)));
        // then
        assertEquals(0, part.getTags().size());
        assertTrue(renderer.setCSSInfoExecuted);
        assertTrue(renderer.reapplyStylesExecuted);
    ...
    public void testHandleEventWhenTabContentChangedEventAndTabInactive() throws Exception {
        ...
        // when
        handler.handleEvent(event(new EventParam(UIEvents.EventTags.ELEMENT, part), new EventParam(UIEvents.EventTags.NEW_VALUE, CSSConstants.CSS_CONTENT_CHANGE_CLASS), new EventParam(UIEvents.EventTags.OLD_VALUE, null)));
        // then
        assertTrue(part.getTags().contains(CSSConstants.CSS_HIGHLIGHTED_CLASS));
        assertFalse(part.getTags().contains(CSSConstants.CSS_CONTENT_CHANGE_CLASS));
        assertTrue(renderer.setCSSInfoExecuted);
        assertTrue(renderer.reapplyStylesExecuted);
        tabItem1.dispose();
        tabItem2.dispose();
    ...
    public void testHandleEventWhenTabActivateEventAndItsContentChanged() throws Exception {
        ...
        renderer.tabItemForPart = tabItem;
        // when
        handler.handleEvent(event(new EventParam(UIEvents.EventTags.ELEMENT, part), new EventParam(UIEvents.EventTags.NEW_VALUE, placeHolder(part)), new EventParam(UIEvents.EventTags.OLD_VALUE, null)));
        // then
        assertFalse(part.getTags().contains(CSSConstants.CSS_HIGHLIGHTED_CLASS));
        assertTrue(renderer.setCSSInfoExecuted);
        assertTrue(renderer.reapplyStylesExecuted);
    ...
    public void testHandleEventWhenTabActivateEventAndTabItemForPartNotFound() throws Exception {
        ...
        CTabItem tabItem = new CTabItem(tabFolder, SWT.NONE);
        part.getTags().add(CSSConstants.CSS_HIGHLIGHTED_CLASS);
        // just to expose the scenario condition
        renderer.tabItemForPart = null;
        // when
        handler.handleEvent(event(new EventParam(UIEvents.EventTags.ELEMENT, part), new EventParam(UIEvents.EventTags.NEW_VALUE, placeHolder(part)), new EventParam(UIEvents.EventTags.OLD_VALUE, null)));
        // then
        assertTrue(part.getTags().contains(CSSConstants.CSS_HIGHLIGHTED_CLASS));
    ...
    private MPlaceholder placeHolder(final MPart part) {
        return (MPlaceholder) Proxy.newProxyInstance(getClass().getClassLoader(), new Class<?>[] { MPlaceholder.class }, new InvocationHandler() {
