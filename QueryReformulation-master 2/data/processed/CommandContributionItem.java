...
*/
    public  CommandContributionItem(CommandContributionItemParameter contributionParameters) {
        ...
        if (command != null) {
            ...
            IWorkbenchLocationService wls = contributionParameters.serviceLocator.getService(IWorkbenchLocationService.class);
...
*/
    ...
    void createCommand(String commandId, Map parameters) {
        if (commandId == null) {
            StatusManager.getManager().handle(StatusUtil.newStatus(IStatus.ERROR, //$NON-NLS-1$
            "Unable to create menu item \"" + getId() + "\", no command id", //$NON-NLS-1$
            null));
            return;
        }
        ...
        if (!cmd.isDefined()) {
            StatusManager.getManager().handle(StatusUtil.newStatus(IStatus.ERROR, //$NON-NLS-1$
            "Unable to create menu item \"" + getId() + "\", command \"" + commandId + "\" not defined", //$NON-NLS-1$ //$NON-NLS-2$
            null));
            return;
        }
        ...
        if (text == null) {
            if (command != null) {
                try {
                    ...
                    StatusManager.getManager().handle(StatusUtil.newStatus(IStatus.ERROR, "Update item failed " + getId(), e));
                }
            }
        }
    ...
    private void establishReferences() {
        if (command != null) {
            ...
            try {
                ...
                StatusManager.getManager().handle(StatusUtil.newStatus(IStatus.ERROR, "Unable to register menu item \"" + getId() + "\", command \"" + command.getId() + "\" not defined", null));
            }
    ...
    private void handleWidgetSelection(Event event) {
        ...
        try {
            ...
            StatusManager.getManager().handle(StatusUtil.newStatus(IStatus.ERROR, "Failed to execute item " + getId(), e));
        } catch (NotDefinedException e) {
            StatusManager.getManager().handle(StatusUtil.newStatus(IStatus.ERROR, "Failed to execute item " + getId(), e));
                ...
                if (event.detail == 4) {
                    ...
                    menuManager.addMenuListener(new IMenuListener2() {
                        ...
                        public void menuAboutToShow(IMenuManager manager) {
                            ...
                            menuService.populateContributionManager(menuManager, //$NON-NLS-1$
                            "menu:" + id);
                        }
    ...
    private void updateIcons() {
        if (widget instanceof MenuItem) {
            ...
            try {
                ...
                StatusManager.getManager().handle(new Status(IStatus.ERROR, WorkbenchPlugin.PI_WORKBENCH, "Failed to load image", e));
            }
            disposeOldImages();
...
*/
    public CommandContributionItemParameter getData() {
        CommandContributionItemParameter data = new CommandContributionItemParameter(serviceLocator, getId(), null, style);
