...
*/
        protected void contributeMenu(IConfigurationElement menuElement, IMenuManager mng, boolean appendIfMissing) {
            ...
            if (label == null) {
                ...
                "Plugin \'" + menuElement.getContributor().getName() + "\' invalid Menu Extension (label == null): " + //$NON-NLS-1$
                id);
                return;
            }
            ...
            if (path != null) {
                ...
                if (parent == null) {
                    ideLog(//$NON-NLS-1$
                    "Plugin \'" + menuElement.getContributor().getName() + //$NON-NLS-1$
                    "\' invalid Menu Extension (Path \'" + path + "\' is invalid): " + //$NON-NLS-1$
                    id);
                    return;
                }
            }
            ...
            if (sep == null) {
                if (appendIfMissing) {
                    ...
                    WorkbenchPlugin.log(//$NON-NLS-1$
                    "Plugin \'" + menuElement.getContributor().getName() + //$NON-NLS-1$
                    "\' invalid Menu Extension (Group \'" + group + "\' is invalid): " + //$NON-NLS-1$
                    id);
                    return;
                }
            }
        ...
        protected void contributeMenuAction(ActionDescriptor ad, IMenuManager menu, boolean appendIfMissing) {
            ...
            if (mpath != null) {
                ...
                if (parent == null) {
                    ...
                    ideLog("Plug-in '" + ad.getPluginId() + "' contributed an invalid Menu Extension (Path: '" + mpath + "' is invalid): " + ad.getId());
                    return;
                }
            }
            // Find reference group.
            ...
            if (sep == null) {
                if (appendIfMissing) {
                    ...
                    WorkbenchPlugin.log(//$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                    "Plug-in '" + ad.getPluginId() + "' contributed an invalid Menu Extension (Group: '" + mgroup + "' is invalid): " + ad.getId());
                    return;
                }
            }
            // Add action.
        ...
        protected void contributeToolbarAction(ActionDescriptor ad, IToolBarManager toolbar, boolean appendIfMissing) {
            ...
            if (sep == null) {
                if (appendIfMissing) {
                    ...
                    WorkbenchPlugin.log(//$NON-NLS-1$
                    "Plug-in '" + ad.getPluginId() + //$NON-NLS-1$
                    "' invalid Toolbar Extension (Group \'" + tgroup + "\' is invalid): " + //$NON-NLS-1$
                    ad.getId());
                    return;
                }
            }
