...
*/
    ...
    private void bind(Control parent) {
        ...
        ViewerSupport.bind(peopleViewer, new WritableList(realm, model.getPersonList(), SimpleModel.class), BeanProperties.values(SimplePerson.class, new String[] { "name", "state" }));
        ...
        Binding b = dbc.bindValue(WidgetProperties.text(SWT.Modify).observe(name), BeanProperties.value((Class) selectedPerson.getValueType(), "name", String.class).observeDetail(selectedPerson), new CustomUpdateValueStrategy().setConverter(upperCaseConverter).setAfterGetValidator(vowelValidator), null);
        // AggregateValidationStatus status = new AggregateValidationStatus(dbc
        // .getBindings(), AggregateValidationStatus.MAX_SEVERITY);
        dbc.bindValue(WidgetProperties.text().observe(validationStatus), b.getValidationStatus(), null, new UpdateValueStrategy().setConverter(new ObjectToStringConverter()));
        dbc.bindValue(WidgetProperties.text(SWT.Modify).observe(address), BeanProperties.value((Class) selectedPerson.getValueType(), "address", String.class).observeDetail(selectedPerson));
        dbc.bindValue(WidgetProperties.text(SWT.Modify).observe(city), BeanProperties.value((Class) selectedPerson.getValueType(), "city", String.class).observeDetail(selectedPerson));
        dbc.bindValue(WidgetProperties.text(SWT.Modify).observe(state), BeanProperties.value((Class) selectedPerson.getValueType(), "state", String.class).observeDetail(selectedPerson));
        TableViewer ordersViewer = new TableViewer(ordersTable);
        ViewerSupport.bind(ordersViewer, BeanProperties.list((Class) selectedPerson.getValueType(), "orders", SimpleOrder.class).observeDetail(selectedPerson), BeanProperties.values(SimpleOrder.class, new String[] { "orderNumber", "date" }));
    }
}
