...
public class WorkbenchSourceProvider extends AbstractSourceProvider implements INullSelectionListener {
    ...
    private static final String STATUS_LINE_VIS = ISources.ACTIVE_WORKBENCH_WINDOW_NAME + //$NON-NLS-1$
    ".isStatusLineVisible";
...
*/
    ...
    public final void selectionChanged(final IWorkbenchPart part, final ISelection newSelection) {
        if (Util.equals(selection, newSelection))
            // we have already handled the change
            return;
        ...
        if (sources != 0) {
            if (DEBUG) {
                if ((sources & ISources.ACTIVE_PART) != 0) {
                    ...
                    "Active part changed to " + lastActivePart);
                }
                if ((sources & ISources.ACTIVE_PART_ID) != 0) {
                    logDebuggingInfo(//$NON-NLS-1$
                    "Active part id changed to " + lastActivePartId);
                }
                if ((sources & ISources.ACTIVE_SITE) != 0) {
                    logDebuggingInfo(//$NON-NLS-1$
                    "Active site changed to " + lastActivePartSite);
                }
                if ((sources & ISources.ACTIVE_EDITOR) != 0) {
                    logDebuggingInfo(//$NON-NLS-1$
                    "Active editor changed to " + lastActiveEditor);
                }
                if ((sources & ISources.ACTIVE_EDITOR_ID) != 0) {
                    logDebuggingInfo(//$NON-NLS-1$
                    "Active editor id changed to " + lastActiveEditorId);
                }
            }
        ...
        public void propertyChange(PropertyChangeEvent event) {
            if (WorkbenchWindow.PROP_COOLBAR_VISIBLE.equals(event.getProperty())) {
                ...
                if (!lastCoolbarVisibility.equals(newValue)) {
                    fireSourceChanged(ISources.ACTIVE_WORKBENCH_WINDOW_SUBORDINATE, ISources.ACTIVE_WORKBENCH_WINDOW_IS_COOLBAR_VISIBLE_NAME, newValue);
                ...
                if (!lastPerspectiveBarVisibility.equals(newValue)) {
                    fireSourceChanged(ISources.ACTIVE_WORKBENCH_WINDOW_SUBORDINATE, ISources.ACTIVE_WORKBENCH_WINDOW_IS_PERSPECTIVEBAR_VISIBLE_NAME, newValue);
                ...
                if (!lastStatusLineVisibility.equals(newValue)) {
                    fireSourceChanged(ISources.ACTIVE_WORKBENCH_WINDOW_SUBORDINATE, ISources.ACTIVE_WORKBENCH_WINDOW_NAME + ".isStatusLineVisible", //$NON-NLS-1$
                    newValue);
...
*/
        ...
        public final void handleEvent(final Event event) {
            if (!(event.widget instanceof Shell)) {
                if (DEBUG) {
                    //$NON-NLS-1$
                    logDebuggingInfo("WSP: passOnEvent: " + event.widget);
                }
                return;
            }
            if (DEBUG) {
                //$NON-NLS-1$
                logDebuggingInfo("\tWSP:lastActiveShell: " + lastActiveShell);
                //$NON-NLS-1$
                logDebuggingInfo("\tWSP:lastActiveWorkbenchWindowShell" + lastActiveWorkbenchWindowShell);
            }
            final Map currentState = getCurrentState();
            final Shell newActiveShell = (Shell) currentState.get(ISources.ACTIVE_SHELL_NAME);
            final WorkbenchWindow newActiveWorkbenchWindow = (WorkbenchWindow) currentState.get(ISources.ACTIVE_WORKBENCH_WINDOW_NAME);
            final Shell newActiveWorkbenchWindowShell = (Shell) currentState.get(ISources.ACTIVE_WORKBENCH_WINDOW_SHELL_NAME);
            // dont update the coolbar/perspective bar visibility unless we're
            // processing a workbench window change
            final Boolean newCoolbarVisibility = newActiveWorkbenchWindow == null ? lastCoolbarVisibility : (newActiveWorkbenchWindow.getCoolBarVisible() ? Boolean.TRUE : Boolean.FALSE);
            final Boolean newPerspectiveBarVisibility = newActiveWorkbenchWindow == null ? lastPerspectiveBarVisibility : (newActiveWorkbenchWindow.getPerspectiveBarVisible() ? Boolean.TRUE : Boolean.FALSE);
            final Boolean newStatusLineVis = newActiveWorkbenchWindow == null ? lastStatusLineVisibility : (newActiveWorkbenchWindow.getStatusLineVisible() ? Boolean.TRUE : Boolean.FALSE);
            ...
            if (shellChanged && windowChanged) {
                ...
                sourceValuesByName.put(ISources.ACTIVE_SHELL_NAME, newActiveShell);
                sourceValuesByName.put(ISources.ACTIVE_WORKBENCH_WINDOW_NAME, newActiveWorkbenchWindow);
                sourceValuesByName.put(ISources.ACTIVE_WORKBENCH_WINDOW_SHELL_NAME, newActiveWorkbenchWindowShell);
                ...
                if (perspectiveBarChanged) {
                    sourceValuesByName.put(ISources.ACTIVE_WORKBENCH_WINDOW_IS_PERSPECTIVEBAR_VISIBLE_NAME, newPerspectiveBarVisibility);
                    sourceFlags |= ISources.ACTIVE_WORKBENCH_WINDOW_SUBORDINATE;
                }
                if (perspectiveIdChanged) {
                    sourceValuesByName.put(ISources.ACTIVE_WORKBENCH_WINDOW_ACTIVE_PERSPECTIVE_NAME, perspectiveId);
                    sourceFlags |= ISources.ACTIVE_WORKBENCH_WINDOW_SUBORDINATE;
                }
                if (DEBUG) {
                    logDebuggingInfo(//$NON-NLS-1$
                    "Active shell changed to " + newActiveShell);
                    logDebuggingInfo(//$NON-NLS-1$
                    "Active workbench window changed to " + newActiveWorkbenchWindow);
                    logDebuggingInfo(//$NON-NLS-1$
                    "Active workbench window shell changed to " + newActiveWorkbenchWindowShell);
                    logDebuggingInfo(//$NON-NLS-1$
                    "Active workbench window coolbar visibility " + newCoolbarVisibility);
                    logDebuggingInfo(//$NON-NLS-1$
                    "Active workbench window perspective bar visibility " + newPerspectiveBarVisibility);
                    logDebuggingInfo(//$NON-NLS-1$
                    "Active workbench window status line visibility " + newStatusLineVis);
                }
    ...
    protected void handleInputChanged(IEditorPart editor) {
        ...
        if (!Util.equals(newInput, lastEditorInput)) {
            fireSourceChanged(ISources.ACTIVE_EDITOR, ISources.ACTIVE_EDITOR_INPUT_NAME, newInput == null ? IEvaluationContext.UNDEFINED_VARIABLE : newInput);
