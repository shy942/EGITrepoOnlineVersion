...
*/
        int[] getPolygon(boolean border) {
            ...
            int b = border ? 1 : 0;
            if (arrowOnLeft) {
                return new int[] { 0, 0, e.x - b, 0, e.x - b, e.y - b, hao + haw, e.y - b, hao + haw / 2, e.y + hah - b, hao, e.y - b, 0, e.y - b, 0, 0 };
            }
            return new int[] { 0, 0, e.x - b, 0, e.x - b, e.y - b, e.x - hao - b, e.y - b, e.x - hao - haw / 2, e.y + hah - b, e.x - hao - haw, e.y - b, 0, e.y - b, 0, 0 };
        }
        ...
        void setText(String t, Control hoverNear, Control targetControl) {
            ...
            if (!t.equals(text)) {
                ...
                if (!oldSize.equals(newSize)) {
                    ...
                    arrowOnLeft = hoverNear.getLocation().x <= targetControl.getLocation().x;
                    setNewShape();
                }
            }
            if (hoverNear != null) {
...
*/
    public void addFieldDecoration(FieldDecoration decoration, int position, boolean showOnFocus) {
        ...
        if (decDatas[i] == null) {
            ...
            decDatas[i] = new FieldDecorationData(decoration, label, formData, showOnFocus);
        } else {
...
*/
    private FormData createFormDataForIndex(int index, Image image) {
        Assert.isTrue(index >= 0 && index < DECORATION_SLOTS, //$NON-NLS-1$
        "Index out of range");
        FormData data = new FormData();
        switch(index) {
            case LEFT_TOP:
...
*/
    private FieldDecorationData getDecorationData(FieldDecoration dec) {
        for (int i = 0; i < DECORATION_SLOTS; i++) {
            if (decDatas[i] != null && dec == decDatas[i].decoration && decDatas[i].label != null && !decDatas[i].label.isDisposed()) {
                return decDatas[i];
            }
        }
        return null;
