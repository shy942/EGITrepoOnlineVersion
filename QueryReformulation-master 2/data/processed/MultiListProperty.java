...
*/
    ...
    protected List<E> doGetList(S source) {
        ...
        for (int i = 0; i < properties.length; i++) list.addAll(properties[i].getList(source));
        return list;
    }
            ...
            public void handleRemove(int index, E element) {
                ...
                for (int i = 0; i < properties.length; i++) {
                    ...
                    if (subListIndex < subList.size()) {
                        ListDiff<E> diff = Diffs.createListDiff(Diffs.createListDiffEntry(subListIndex, false, element));
                        properties[i].updateList(source, diff);
                        return;
                    }
                    offset += subList.size();
