...
*            {@link IViewActionDelegate}.
*/
    public  ActionDelegateHandlerProxy(final IConfigurationElement element, final String delegateAttributeName, final String actionId, final ParameterizedCommand command, final IWorkbenchWindow window, final String style, final Expression enabledWhenExpression, final String viewId) {
        if (element == null) {
        ...
        if (delegateAttributeName == null) {
            throw new NullPointerException(//$NON-NLS-1$
            "The attribute containing the action delegate must be known");
        }
        if (window == null) {
            throw new NullPointerException(//$NON-NLS-1$
            "The workbench window for a delegate must not be null");
        }
...
*/
    private void updateDelegate(final IAction action, final IEvaluationContext context) {
        ...
        if (editorDelegate != null) {
            ...
            updateActivePart(activeEditor == IEvaluationContext.UNDEFINED_VARIABLE ? null : (IWorkbenchPart) activeEditor);
    ...
    public void setEnabled(Object evaluationContext) {
        ...
        if (enabledWhenExpression != null) {
            try {
                ...
                final StringBuffer message = new StringBuffer("An exception occurred while evaluating the enabledWhen expression for ");
                if (delegate != null) {
...
*/
            ...
            try {
                ...
                if (//$NON-NLS-1$
                "org.eclipse.ui.actionSets".equals(name) && delegate instanceof IWorkbenchWindowActionDelegate) {
                    windowDelegate = (IWorkbenchWindowActionDelegate) delegate;
                } else if (//$NON-NLS-1$
                "org.eclipse.ui.editorActions".equals(name) && delegate instanceof IEditorActionDelegate) {
                    editorDelegate = (IEditorActionDelegate) delegate;
                } else if (//$NON-NLS-1$
                "org.eclipse.ui.viewActions".equals(name) && delegate instanceof IViewActionDelegate) {
                    ...
                    if (//$NON-NLS-1$
                    "objectContribution".equals(parent.getName()) && delegate instanceof IObjectActionDelegate) {
                ...
                final String message = "The proxied delegate was the wrong class";
                final IStatus status = new Status(IStatus.ERROR, WorkbenchPlugin.PI_WORKBENCH, 0, message, e);
                WorkbenchPlugin.log(message, status);
                return false;
            } catch (final CoreException e) {
                final String message = "The proxied delegate for '" + element.getAttribute(delegateAttributeName) + "' could not be loaded";
