            ...
            if (i % 2 == 0) {
                if (stringTokenizer.hasMoreTokens()) {
                    ...
                    if (modifierKey == null || !modifierKeys.add(modifierKey)) {
                        throw new ParseException(//$NON-NLS-1$
                        "Cannot create key stroke with duplicate or non-existent modifier key: " + token);
                    }
                    ...
                    if (naturalKey == null) {
                        throw new ParseException(//$NON-NLS-1$
                        "Cannot create key stroke with invalid natural key: " + token);
                    }
                }
            }
            i++;
