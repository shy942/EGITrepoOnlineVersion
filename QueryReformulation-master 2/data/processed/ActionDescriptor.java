...
*/
    public  ActionDescriptor(IConfigurationElement actionElement, int targetType, Object target) {
        ...
        if ("FORCE_TEXT".equals(actionElement.getAttribute(IWorkbenchRegistryConstants.ATT_MODE))) {
            //$NON-NLS-1$
        ...
        if (label == null) {
            ...
            "Invalid action declaration (label == null): " + id);
        ...
        if (helpContextId != null) {
            ...
            if (helpContextId.indexOf(".") == -1) {
                ...
                // qualified)
                fullID = actionElement.getNamespace() + "." + //$NON-NLS-1$
                helpContextId;
            }
        ...
        String extendingPluginId = actionElement.getDeclaringExtension().getNamespace();
        if (icon != null) {
        ...
        if (disabledIcon != null) {
            action.setDisabledImageDescriptor(AbstractUIPlugin.imageDescriptorFromPlugin(extendingPluginId, disabledIcon));
        }
        if (accelerator != null) {
    ...
    private void processAccelerator(IAction action, String acceleratorText) {
        ...
        if (Character.isDigit(acceleratorText.charAt(0))) {
            try {
                ...
                WorkbenchPlugin.log("Invalid accelerator declaration for action: " + id, e);
            }
        } else {
