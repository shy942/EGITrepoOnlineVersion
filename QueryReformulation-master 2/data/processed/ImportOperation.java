...
* class.
*/
    void collectExistingReadonlyFiles(IPath sourceStart, List sources, ArrayList noOverwrite, ArrayList overwriteReadonly, int policy, IProgressMonitor monitor) {
        ...
        while (sourceIter.hasNext()) {
            ...
            if (folder != null) {
                ...
                if (provider.isFolder(nextSource)) {
                    collectExistingReadonlyFiles(newDestinationPath, provider.getChildren(nextSource), noOverwrite, overwriteReadonly, POLICY_FORCE_OVERWRITE, subMonitor.split(100));
                }
            } else {
        ...
        for (int i = 0; i < segmentCount; i++) {
            ...
            if (!currentFolder.exists()) {
                if (createVirtualFolder)
                    ((IFolder) currentFolder).create(IResource.VIRTUAL, true, null);
                else if (createLinks)
...
*/
    public IStatus getStatus() {
        ...
        return new MultiStatus(PlatformUI.PLUGIN_ID, IStatus.OK, errors, DataTransferMessages.ImportOperation_importProblems, null);
    }
...
*/
    void importFile(Object fileObject, int policy, IProgressMonitor mon) {
        ...
        if (targetPath != null && (targetPath.toFile().equals(new File(fileObjectPath)))) {
            errorTable.add(new Status(IStatus.ERROR, PlatformUI.PLUGIN_ID, 0, NLS.bind(DataTransferMessages.ImportOperation_targetSameAsSourceError, fileObjectPath), null));
            return;
        }
        InputStream contentStream = provider.getContents(fileObject);
        if (contentStream == null) {
            errorTable.add(new Status(IStatus.ERROR, PlatformUI.PLUGIN_ID, 0, NLS.bind(DataTransferMessages.ImportOperation_openStreamError, fileObjectPath), null));
            return;
        }
        try {
            if (createVirtualFolder || createLinks || createLinkFilesOnly) {
                ...
                targetResource.createLink(createRelativePath(new Path(provider.getFullPath(fileObject)), targetResource), 0, subMonitor.split(50));
            } else {
                if (targetResource.exists()) {
...
*/
    private void setResourceAttributes(IFile targetResource, Object fileObject) {
        ...
        if (fileObject instanceof File) {
            ...
            try {
                targetResource.setResourceAttributes(((TarLeveledStructureProvider) provider).getResourceAttributes(fileObject));
                // TarEntry time is in secs. Convert to msecs
                timeStamp = ((TarEntry) fileObject).getTime() * 1000;
            } catch (CoreException e) {
                errorTable.add(e.getStatus());
            }
...
*/
    void validateFiles(List sourceFiles, IProgressMonitor monitor) {
        ...
        collectExistingReadonlyFiles(destinationPath, sourceFiles, noOverwrite, overwriteReadonly, POLICY_DEFAULT, monitor);
