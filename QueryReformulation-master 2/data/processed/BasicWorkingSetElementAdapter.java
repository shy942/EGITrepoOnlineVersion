...
*/
    private void parseOptions(String classNameAndOptions, Type record) {
        for (StringTokenizer toker = new StringTokenizer(classNameAndOptions, ";"); toker.hasMoreTokens(); ) {
            //$NON-NLS-1$
                ...
                for (StringTokenizer pair = new StringTokenizer(token, "="); pair.hasMoreTokens(); ) {
                    ...
                    if (pair.countTokens() == 2) {
                        ...
                        if ("adapt".equals(param)) {
                            //$NON-NLS-1$
                            //$NON-NLS-1$
                            record.flags ^= "true".equals(value) ? Type.ADAPT : Type.NONE;
                        }
                    }
                }
            }
...
*/
    private PackageAdmin getPackageAdmin() {
        if (packageTracker == null) {
            packageTracker = new ServiceTracker(WorkbenchPlugin.getDefault().getBundleContext(), PackageAdmin.class.getName(), null);
            packageTracker.open();
        }
