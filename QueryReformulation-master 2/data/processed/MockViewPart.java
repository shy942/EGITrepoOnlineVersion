...
public class MockViewPart extends MockWorkbenchPart implements IViewPart {
    ...
    private class DummyAction extends Action {
        ...
        public  DummyAction() {
            setText("Monkey");
            setImageDescriptor(getViewSite().getWorkbenchWindow().getWorkbench().getSharedImages().getImageDescriptor(ISharedImages.IMG_TOOL_DELETE));
        }
    }
...
*/
    ...
    public void dispose() {
        ...
        // clients).
        Assert.assertTrue("Contribution items should not be removed from the site until after the part is disposed", getViewSite().getActionBars().getToolBarManager().find(toolbarItem.getId()) == toolbarItem);
        super.dispose();
    }
