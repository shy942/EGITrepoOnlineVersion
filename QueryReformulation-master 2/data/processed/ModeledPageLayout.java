...
public class ModeledPageLayout implements IPageLayout {
    ...
    public static MStackElement createViewModel(MApplication application, String id, boolean visible, WorkbenchPage page, EPartService partService, boolean createReferences) {
        ...
        if (partDesc != null) {
            ...
            // drawn
            if (visible && createReferences && CompatibilityPart.COMPATIBILITY_VIEW_URI.equals(partDesc.getContributionURI())) {
    ...
    private MUIElement insertView(String viewId, int relationship, float ratio, String refId, boolean visible, boolean withStack) {
        ...
        MStackElement viewModel = createViewModel(application, viewId, visible && !isFiltered, page, partService, createReferences);
        MUIElement retVal = viewModel;
        if (viewModel != null) {
            if (withStack) {
                // Default id...basically unusable //$NON-NLS-1$
                String stackId = viewId + "MStack";
