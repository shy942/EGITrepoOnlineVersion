...
public class JavaBeanObservableMapTest extends AbstractDefaultRealmTestCase {
    ...
    public void testGetValue() throws Exception {
        assertEquals("The 'value' from the map should be the value of the property of the model.", model1.getValue(), map.get(model1));
    }
    ...
    public void testModifyObservableMap_FiresMapChange() {
        ...
        assertEquals(1, tracker.count);
        assertDiff(tracker.event.diff, Collections.singletonMap("key", "oldValue"), Collections.singletonMap("key", "newValue"));
    }
    ...
    private static void assertDiff(MapDiff diff, Map oldMap, Map newMap) {
        ...
        // unmodifiable
        diff.applyTo(oldMap);
        assertEquals("applying diff to list did not produce expected result", newMap, oldMap);
    }
