...
*            accept parameters.
*/
    public final void define(final String name, final String description, final Category category, final IParameter[] parameters) {
...
* @since 3.2
*/
    public final void define(final String name, final String description, final Category category, final IParameter[] parameters, ParameterType returnType) {
...
* @since 3.2
*/
    public final void define(final String name, final String description, final Category category, final IParameter[] parameters, ParameterType returnType, final String helpContextId) {
        if (name == null) {
            //$NON-NLS-1$
        ...
        final boolean parametersChanged = !Util.equals(this.parameters, parameters);
        this.parameters = parameters;
        final boolean returnTypeChanged = !Util.equals(this.returnType, returnType);
        this.returnType = returnType;
        final boolean helpContextIdChanged = !Util.equals(this.helpContextId, helpContextId);
        this.helpContextId = helpContextId;
        fireCommandChanged(new CommandEvent(this, categoryChanged, definedChanged, descriptionChanged, false, nameChanged, parametersChanged, returnTypeChanged, helpContextIdChanged));
    }
...
*/
    ...
    public final Object execute(final ExecutionEvent event) throws ExecutionException, NotHandledException {
        ...
        //$NON-NLS-1$
        final NotHandledException e = new NotHandledException("There is no handler to execute. " + getId());
        if (shouldFireEvents) {
            fireNotHandled(e);
        }
        throw e;
...
* @since 3.2
*/
    public final Object executeWithChecks(final ExecutionEvent event) throws ExecutionException, NotDefinedException, NotEnabledException, NotHandledException {
        if (shouldFireEvents) {
            firePreExecute(event);
        }
        final IHandler handler = this.handler;
        // bug 369159 working
        if ((handler != null) && "org.eclipse.ui.internal.MakeHandlersGo".equals(//$NON-NLS-1$
        handler.getClass().getName())) {
            return handler.execute(event);
        }
        if (!isDefined()) {
            final NotDefinedException exception = new NotDefinedException(//$NON-NLS-1$
            "Trying to execute a command that is not defined. " + getId());
            if (shouldFireEvents) {
        ...
        if ((handler != null) && (handler.isHandled())) {
            ...
            if (!isEnabled()) {
                final NotEnabledException exception = new NotEnabledException(//$NON-NLS-1$
        ...
        //$NON-NLS-1$
        final NotHandledException e = new NotHandledException("There is no handler to execute for command " + getId());
        if (shouldFireEvents) {
            fireNotHandled(e);
        }
        throw e;
...
*/
    private final void fireNotDefined(final NotDefinedException e) {
        ...
        if (DEBUG_COMMAND_EXECUTION) {
            Tracing.printTrace("COMMANDS", //$NON-NLS-1$ //$NON-NLS-2$
            "execute" + Tracing.SEPARATOR + "not defined: id=" + getId() + "; exception=" + //$NON-NLS-1$ //$NON-NLS-2$
            e);
        }
        if (executionListeners != null) {
...
*/
    private final void fireNotEnabled(final NotEnabledException e) {
        ...
        if (DEBUG_COMMAND_EXECUTION) {
            Tracing.printTrace("COMMANDS", //$NON-NLS-1$ //$NON-NLS-2$
            "execute" + Tracing.SEPARATOR + "not enabled: id=" + getId() + "; exception=" + //$NON-NLS-1$ //$NON-NLS-2$
            e);
        }
        if (executionListeners != null) {
            for (final IExecutionListener object : executionListeners) {
...
*/
    private final void fireNotHandled(final NotHandledException e) {
        ...
        if (DEBUG_COMMAND_EXECUTION) {
            Tracing.printTrace("COMMANDS", //$NON-NLS-1$ //$NON-NLS-2$
            "execute" + Tracing.SEPARATOR + "not handled: id=" + getId() + "; exception=" + //$NON-NLS-1$ //$NON-NLS-2$
            e);
        }
        if (executionListeners != null) {
...
*/
    private final void firePostExecuteFailure(final ExecutionException e) {
        ...
        if (DEBUG_COMMAND_EXECUTION) {
            Tracing.printTrace("COMMANDS", //$NON-NLS-1$ //$NON-NLS-2$
            "execute" + Tracing.SEPARATOR + "failure: id=" + getId() + "; exception=" + //$NON-NLS-1$ //$NON-NLS-2$
            e);
        }
        if (executionListeners != null) {
            for (final IExecutionListener listener : executionListeners) {
...
*/
    private final void firePostExecuteSuccess(final Object returnValue) {
        ...
        if (DEBUG_COMMAND_EXECUTION) {
            Tracing.printTrace("COMMANDS", //$NON-NLS-1$ //$NON-NLS-2$
            "execute" + Tracing.SEPARATOR + "success: id=" + getId() + //$NON-NLS-1$ //$NON-NLS-2$
...
*/
    private final void firePreExecute(final ExecutionEvent event) {
        ...
        if (DEBUG_COMMAND_EXECUTION) {
            Tracing.printTrace("COMMANDS", //$NON-NLS-1$ //$NON-NLS-2$
            "execute" + Tracing.SEPARATOR + "starting: id=" + getId() + "; event=" + //$NON-NLS-1$ //$NON-NLS-2$
            event);
        }
        if (executionListeners != null) {
            for (final IExecutionListener listener : executionListeners) {
...
*/
    public final Category getCategory() throws NotDefinedException {
        if (!isDefined()) {
            ...
            "Cannot get the category from an undefined command. " + id);
        }
        return category;
    }
...
*/
    public final boolean isEnabled() {
        ...
        try {
            ...
            if (DEBUG_HANDLERS) {
                Tracing.printTrace("HANDLERS", "Handler " + handler + " for " + id + " threw unexpected exception");
...
*/
    public final boolean setHandler(final IHandler handler) {
        ...
        if ((DEBUG_HANDLERS) && ((DEBUG_HANDLERS_COMMAND_ID == null) || (DEBUG_HANDLERS_COMMAND_ID.equals(id)))) {
            //$NON-NLS-1$
            final StringBuffer buffer = new StringBuffer("Command('");
            buffer.append(id);
            //$NON-NLS-1$
    ...
    public final String toString() {
        if (string == null) {
            ...
            try {
                ...
                buffer.write("\t\t");
                //$NON-NLS-1$
                buffer.write(parameters == null ? "" : Arrays.toString(parameters));
                buffer.write(',');
                //$NON-NLS-1$
