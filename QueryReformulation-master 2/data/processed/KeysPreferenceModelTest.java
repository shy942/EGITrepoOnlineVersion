...
*/
    ...
    public void testDefaults() throws Exception {
        ...
        assertNotNull(cm.getContextIdToElement().get(IContextService.CONTEXT_ID_DIALOG_AND_WINDOW));
        SchemeModel sm = controller.getSchemeModel();
        boolean foundDefault = false;
        i = sm.getSchemes().iterator();
        while (i.hasNext()) {
            SchemeElement e = (SchemeElement) i.next();
            if (e.getId().equals(IBindingService.DEFAULT_DEFAULT_ACTIVE_SCHEME_ID)) {
                foundDefault = true;
            }
        }
        assertTrue("No default scheme", foundDefault);
        assertEquals(IBindingService.DEFAULT_DEFAULT_ACTIVE_SCHEME_ID, sm.getSelectedElement().getId());
    ...
    public void testContexts() throws Exception {
        ...
        controller.init(getWorkbench());
        ContextModel cm = controller.getContextModel();
        ContextElement dialog = (ContextElement) cm.getContextIdToElement().get(IContextService.CONTEXT_ID_DIALOG);
        assertNull(cm.getSelectedElement());
    ...
    public void testBindings() throws Exception {
        ...
        ContextModel cm = controller.getContextModel();
        BindingModel bm = controller.getBindingModel();
        BindingElement activateEditor = getBindingElement(bm, ID_ACTIVATE_EDITOR);
        ...
        assertTrue(cm.getSelectedElement().getId().equals(activateEditor.getContext().getId()));
        ...
        assertNull(bm.getSelectedElement());
        assertTrue(cm.getSelectedElement().getId().equals(activateEditor.getContext().getId()));
        expected = new PropertyChangeEvent[] { new PropertyChangeEvent(controller.getConflictModel(), CommonModel.PROP_SELECTED_ELEMENT, activateEditor, null), new PropertyChangeEvent(controller.getBindingModel(), CommonModel.PROP_SELECTED_ELEMENT, activateEditor, null) };
        assertChanges(expected, events);
    }
    ...
    public void testBasicConflicts() throws Exception {
        ...
        final BindingElement activateEditor = getBindingElement(bm, ID_ACTIVATE_EDITOR);
        assertNotNull(activateEditor);
    ...
    public void testConflictSelection() throws Exception {
        ...
        PropertyChangeEvent[] expected = new PropertyChangeEvent[] { new PropertyChangeEvent(controller.getBindingModel(), CommonModel.PROP_SELECTED_ELEMENT, conflict1, conflict3), new PropertyChangeEvent(controller.getConflictModel(), CommonModel.PROP_SELECTED_ELEMENT, conflict1, conflict3) };
        assertChanges(expected, events);
    }
    ...
    public void testUpdateContext() throws Exception {
        ...
        final ContextElement dialog = (ContextElement) cm.getContextIdToElement().get(IContextService.CONTEXT_ID_DIALOG);
        final ContextElement window = (ContextElement) cm.getContextIdToElement().get(IContextService.CONTEXT_ID_WINDOW);
        final BindingModel bm = controller.getBindingModel();
    ...
    public void failsOnWinAndLinuxWith16VMtestUpdateKeySequence() throws Exception {
        ...
        final ParameterizedCommand c3parameterized = c3model.getParameterizedCommand();
        assertEquals(dialog, conflict2.getContext());
    ...
    public void testChangeSchemesTwice() throws Exception {
        ...
        while (i.hasNext()) {
            ...
            if (e.getId().equals(SCHEME_EMACS_ID)) {
                emacsScheme = e;
            } else if (e.getId().equals(IBindingService.DEFAULT_DEFAULT_ACTIVE_SCHEME_ID)) {
                defaultScheme = e;
            }
        }
        assertNotNull(emacsScheme);
    ...
    private void assertChangeEvent(int eventNum, PropertyChangeEvent expected, PropertyChangeEvent event) {
        assertEquals("source: " + eventNum, expected.getSource(), event.getSource());
        assertEquals("property: " + eventNum, expected.getProperty(), event.getProperty());
        assertEquals("old: " + eventNum, expected.getOldValue(), event.getOldValue());
        assertEquals("new: " + eventNum, expected.getNewValue(), event.getNewValue());
    }
    ...
    private void assertChanges(PropertyChangeEvent[] expected, List events) {
        ...
        for (int i = 0; i < expected.length; i++) {
            assertChangeEvent(i, expected[i], (PropertyChangeEvent) events.get(i));
        }
    }
