...
*/
    @Inject
    public  ResourceHandler(@Named(IWorkbench.PERSIST_STATE) boolean saveAndRestore, @Named(IWorkbench.CLEAR_PERSISTED_STATE) boolean clearPersistedState) {
...
*/
    ...
    public Resource loadMostRecentModel() {
        ...
        if (restore && saveAndRestore) {
            ...
            if (!hasTopLevelWindows(resource)) {
                if (logger != null) {
                    ...
                    "The persisted workbench has no top-level windows, so reinitializing with defaults.");
                }
                resource = null;
            }
        }
        ...
        if (!hasTopLevelWindows(resource) && logger != null) {
            ...
            "Continuing execution, but the missing windows may cause other initialization failures.");
        }
...
*/
    ...
    private Resource createResource() {
        ...
        //$NON-NLS-1$
        return resourceSetImpl.createResource(URI.createURI("workbench.xmi"));
    }

    private File getWorkbenchSaveLocation() {
        //$NON-NLS-1$
        File workbenchData = new File(getBaseLocation(), "workbench.xmi");
    ...
    private Resource loadResource(URI uri) {
        ...
        try {
            ...
            logger.error(e, "Unable to load resource " + uri.toString());
            return null;
        }
    ...
    protected long getLastApplicationModification() {
        ...
        Map<String, ?> attributes = resourceSetImpl.getURIConverter().getAttributes(applicationDefinitionInstance, Collections.singletonMap(URIConverter.OPTION_REQUESTED_ATTRIBUTES, Collections.singleton(URIConverter.ATTRIBUTE_TIME_STAMP)));
