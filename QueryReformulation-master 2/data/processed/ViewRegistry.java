...
*/
    //$NON-NLS-1$
    public static final String ORIGINAL_COMPATIBILITY_VIEW_CLASS = "originalCompatibilityViewClass";
...
*/
    //$NON-NLS-1$
    public static final String ORIGINAL_COMPATIBILITY_VIEW_BUNDLE = "originalCompatibilityViewBundle";
    ...
    void postConstruct() {
        //$NON-NLS-1$
        IExtensionPoint point = extensionRegistry.getExtensionPoint("org.eclipse.ui.views");
        for (IExtension extension : point.getExtensions()) {
            // find the category first
            for (IConfigurationElement element : extension.getConfigurationElements()) {
                if (element.getName().equals(IWorkbenchRegistryConstants.TAG_CATEGORY)) {
                    ViewCategory category = new ViewCategory(element.getAttribute(IWorkbenchRegistryConstants.ATT_ID), element.getAttribute(IWorkbenchRegistryConstants.ATT_NAME));
                    ...
                    try {
                        ...
                        logger.error("Unable to create sticky view descriptor.", e.getStatus());
                    }
                }
            }
        }
    ...
    private void createDescriptor(IConfigurationElement element, boolean e4View) {
        ...
        descriptor.setAllowMultiple(Boolean.parseBoolean(element.getAttribute(IWorkbenchRegistryConstants.ATT_ALLOW_MULTIPLE)));
        ...
        if (iconURI == null) {
            descriptor.setIconURI(MenuHelper.getImageUrl(workbench.getSharedImages().getImageDescriptor(ISharedImages.IMG_DEF_VIEW)));
        } else {
    ...
    public IViewDescriptor[] getViews() {
        Collection<?> allowedViews = WorkbenchActivityHelper.restrictCollection(descriptors.values(), new ArrayList());
