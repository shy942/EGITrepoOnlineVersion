...
*/
    public  WorkingSetDescriptor(IConfigurationElement configElement) throws CoreException {
        ...
        if (name == null) {
            throw new CoreException(new Status(IStatus.ERROR, WorkbenchPlugin.PI_WORKBENCH, 0, //$NON-NLS-1$
            "Invalid extension (missing class name): " + id, null));
        }
        ...
        if (containsChildren.length > 0) {
            ...
            for (int i = 0; i < containsChildren.length; i++) {
                ...
                if (//$NON-NLS-1$
                "true".equals(child.getAttribute(IWorkbenchRegistryConstants.ATT_ADAPTABLE)))
    ...
    public IWorkingSetPage createWorkingSetPage() {
        ...
        if (pageClassName != null) {
            try {
                ...
                WorkbenchPlugin.log("Unable to create working set page: " + pageClassName, exception.getStatus());
            }
        }
...
*/
    public IWorkingSetElementAdapter createWorkingSetElementAdapter() {
        ...
        try {
            result = (IWorkingSetElementAdapter) WorkbenchPlugin.createExtension(configElement, ATT_ELEMENT_ADAPTER_CLASS);
        } catch (CoreException exception) {
...
*/
    public IWorkingSetUpdater createWorkingSetUpdater() {
        ...
        try {
            ...
            WorkbenchPlugin.log("Unable to create working set updater: " + updaterClassName, exception.getStatus());
        }
        return result;
    }
