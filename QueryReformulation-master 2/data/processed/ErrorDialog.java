...
*/
    public  ErrorDialog(Shell parentShell, String dialogTitle, String message, IStatus status, int displayMask) {
        super(parentShell);
        this.title = dialogTitle == null ? JFaceResources.getString(//$NON-NLS-1$
        "Problem_Occurred") : dialogTitle;
        this.message = message == null ? status.getMessage() : //$NON-NLS-1$
        JFaceResources.format("Reason", message, status.getMessage());
        this.status = status;
