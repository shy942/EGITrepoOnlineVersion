...
*/
    ...
    public void handleEvent(Event event) {
        ...
        if (event.type == SWT.MenuDetect || (isWindowsOS && event.type == SWT.MouseDown && event.button != 1)) {
            ...
            if (this.fHorizontalScrollHandler.mousePosOverScroll(fScrollable, controlPos) || this.fVerticalScrollHandler.mousePosOverScroll(fScrollable, controlPos)) {
            ...
            if (event.widget == fScrollable) {
                Rectangle currClientArea = fScrollable.getClientArea();
                if (this.fHorizontalScrollHandler.startDragOnMouseDown(fScrollable, controlPos, fLastHorizontalAndTopPixel, currClientArea) || this.fVerticalScrollHandler.startDragOnMouseDown(fScrollable, controlPos, fLastHorizontalAndTopPixel, currClientArea)) {
                    this.stopEventPropagation(event);
            ...
            if (!fHorizontalScrollHandler.isDragging() && !fVerticalScrollHandler.isDragging()) {
                ...
                boolean showHorizontal = ((proximityRectHorizontal != null && proximityRectHorizontal.contains(controlPos.x, controlPos.y)) && currClientArea.width < (fHorizontalScrollHandler.fScrollBar.getMaximum() - fHorizontalScrollHandler.fScrollBar.getMinimum()));
                boolean showVertical = ((proximityRectVertical != null && proximityRectVertical.contains(controlPos.x, controlPos.y)) && currClientArea.height < (fVerticalScrollHandler.fScrollBar.getMaximum() - fVerticalScrollHandler.fScrollBar.getMinimum()));
            ...
            if (fHorizontalScrollHandler.dragOnMouseMove(fScrollable, controlPos) || fVerticalScrollHandler.dragOnMouseMove(fScrollable, controlPos)) {
                this.stopEventPropagation(event);
                return;
            }
            checkChangedHorizontalAndTopPixel();
...
*/
    ...
    protected boolean setMouseNearScrollScrollBar(boolean mouseNearVerticalScroll, boolean mouseNearHorizontalScroll) {
        if (mouseNearVerticalScroll != fVerticalScrollHandler.getMouseNearScrollScrollBar() || mouseNearHorizontalScroll != fHorizontalScrollHandler.getMouseNearScrollScrollBar()) {
