...
*/
    ...
    private static URI parseIntoURI(String uri) {
        ...
        if (archiveScheme) {
            ...
            authority = uri.substring(i, j);
            i = j;
        } else if (scheme != null && (i == uri.length() || uri.charAt(i) != SEGMENT_SEPARATOR)) {
            hierarchical = false;
        ...
        if (// && uri.charAt(i) == FRAGMENT_SEPARATOR (implied)
        i < uri.length()) {
    ...
    private static boolean segmentsRemain(String uri, int i) {
        return i < uri.length() && uri.charAt(i) != QUERY_SEPARATOR && uri.charAt(i) != FRAGMENT_SEPARATOR;
    }
...
*/
    public boolean hasEmptyPath() {
        // (authority == null && device == null && !absolutePath) -> scheme == null
        return authority == null && device == null && !hasAbsolutePath() && segments.length == 0;
    }
...
*/
    public boolean isCurrentDocumentReference() {
        // (authority == null && device == null && !absolutePath) -> scheme == null
        return authority == null && device == null && !hasAbsolutePath() && segments.length == 0 && query == null;
    }
...
*/
    public boolean isEmpty() {
        // (authority == null && device == null && !absolutePath) -> scheme == null
        return authority == null && device == null && !hasAbsolutePath() && segments.length == 0 && query == null && fragment == null;
    }
...
*/
    public boolean isFile() {
        return isHierarchical() && ((isRelative() && !hasQuery()) || SCHEME_FILE.equalsIgnoreCase(scheme));
    }
...
*/
    public boolean isPlatform() {
        return isHierarchical() && !hasAuthority() && segmentCount() >= 2 && SCHEME_PLATFORM.equalsIgnoreCase(scheme);
    }
...
*/
    ...
    public boolean equals(Object object) {
        ...
        if (!(object instanceof URI))
            return false;
        URI uri = (URI) object;
        return hashCode == uri.hashCode() && equals(scheme, uri.scheme(), true) && equals(authority, isHierarchical() ? uri.authority() : uri.opaquePart()) && equals(device, uri.device()) && equals(query, uri.query()) && equals(fragment, uri.fragment()) && segmentsEqual(uri);
    }
...
*/
    public URI resolve(URI base, boolean preserveRootParents) {
        ...
        return new URI(true, base.scheme(), newAuthority, newDevice, newAbsolutePath, newSegments, newQuery, fragment);
    }
...
*/
    public URI deresolve(URI base, boolean preserveRootParents, boolean anyRelPath, boolean shorterRelPath) {
        ...
        String[] newSegments = segments;
        String newQuery = query;
        if (equals(authority, base.authority()) && (hasDevice() || hasPath() || (!base.hasDevice() && !base.hasPath()))) {
            ...
            if (equals(device, base.device()) && (hasPath() || !base.hasPath())) {
                ...
                // user rejects a relative path: keep absolute or no path
                } else if (hasPath() == base.hasPath() && segmentsEqual(base) && equals(query, base.query())) {
                    ...
                    newAbsolutePath = false;
                    newSegments = NO_SEGMENTS;
                } else // exception if (!hasAbsolutePath())
        ...
        // no validation needed since all components are from existing URIs
        return new URI(true, null, newAuthority, newDevice, newAbsolutePath, newSegments, newQuery, fragment);
    }
    ...
    private String[] findRelativePath(URI base, boolean preserveRootParents) {
        ...
        for (int count = startCount < endCount ? startCount : endCount - 1; diff < count && startPath[diff].equals(endPath[diff]); diff++) {
        // Empty statement.
        }
...
*/
    public URI appendSegment(String segment) {
        ...
        return new URI(true, scheme, authority, device, newAbsolutePath, newSegments, query, fragment);
    }
...
*/
    public URI trimSegments(int i) {
        ...
        return new URI(true, scheme, authority, device, hasAbsolutePath(), newSegments, query, fragment);
    }
...
*/
    public boolean hasTrailingPathSeparator() {
        return segments.length > 0 && SEGMENT_EMPTY.equals(segments[segments.length - 1]);
    }
...
*/
    public boolean isPrefix() {
        return isHierarchical() && query == null && fragment == null && (hasTrailingPathSeparator() || (hasAbsolutePath() && segments.length == 0));
    }
...
*/
    public URI replacePrefix(URI oldPrefix, URI newPrefix) {
        ...
        // no validation needed since all components are from existing URIs
        return new URI(true, newPrefix.scheme(), newPrefix.authority(), newPrefix.device(), newPrefix.hasAbsolutePath(), mergedSegments, query, fragment);
    }
    ...
    private String[] getTailSegments(URI prefix) {
        ...
        if (!isHierarchical() || !equals(scheme, prefix.scheme(), true) || !equals(authority, prefix.authority()) || !equals(device, prefix.device()) || hasAbsolutePath() != prefix.hasAbsolutePath()) {
            return null;
        }
...
*/
    public static String encodeOpaquePart(String value, boolean ignoreEscaped) {
        ...
        return result != null && result.length() > 0 && result.charAt(0) == SEGMENT_SEPARATOR ? "%2F" + result.substring(1) : result;
    }
...
*/
    ...
    private static String encodeURI(String uri, boolean ignoreEscaped, int fragmentLocationStyle) {
        ...
        int j = fragmentLocationStyle == FRAGMENT_FIRST_SEPARATOR ? uri.indexOf(FRAGMENT_SEPARATOR) : fragmentLocationStyle == FRAGMENT_LAST_SEPARATOR ? uri.lastIndexOf(FRAGMENT_SEPARATOR) : -1;
        if (j != -1) {
