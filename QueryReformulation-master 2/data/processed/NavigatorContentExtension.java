            ...
            SafeRunner.run(new NavigatorSafeRunnable() {
                ...
                public void run() throws Exception {
                    if (contentProvider == null) {
                        ITreeContentProvider treeContentProvider = descriptor.createContentProvider();
                        if (treeContentProvider != null) {
                            contentProvider = new SafeDelegateTreeContentProvider(treeContentProvider);
                            contentProvider.init(new CommonContentExtensionSite(getId(), contentService, appliedMemento));
                ...
                public void run() throws Exception {
                    if (labelProvider == null) {
                        ...
                        if (tempLabelProvider instanceof ICommonLabelProvider) {
                            ...
                            labelProvider.init(new CommonContentExtensionSite(getId(), contentService, appliedMemento));
                        } else {
...
*/
    ...
    protected final void complainDisposedIfNecessary() {
        if (isDisposed) {
            ...
            "INavigatorContentExtension " + descriptor.getId() + //$NON-NLS-1$
            " is disposed!");
        }
    }
...
*/
    private NavigatorContentExtension[] getOverridingExtensions(Object anElement, boolean triggerPoint) {
        ...
        for (Iterator contentDescriptorsItr = descriptor.getOverriddingExtensions().iterator(); contentDescriptorsItr.hasNext(); ) {
            overridingDescriptor = (NavigatorContentDescriptor) contentDescriptorsItr.next();
            if (contentService.isActive(overridingDescriptor.getId()) && contentService.isVisible(overridingDescriptor.getId()) && (anElement == null || (triggerPoint ? overridingDescriptor.isTriggerPoint(anElement) : overridingDescriptor.isPossibleChild(anElement)))) {
        ...
        if (Policy.DEBUG_EXTENSION_SETUP) {
            System.out.println(this + //$NON-NLS-1$
            " overriding: " + (triggerPoint ? "(trigger pt: " : "(poss child: ") + anElement + "): " + //$NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$
            overridingExtensions);
        }
        return (NavigatorContentExtension[]) overridingExtensions.toArray(new NavigatorContentExtension[overridingExtensions.size()]);
    }

    @Override
    public String toString() {
        //$NON-NLS-1$
        return descriptor.toString() + " Instance";
    }
}
