        ...
        if (canAdapt()) {
            ...
            if (newSelection.size() != ssel.size()) {
                if (Policy.DEBUG_CONTRIBUTIONS) {
                    ...
                    "Error adapting selection to " + getObjectClass() + ". Contribution " + getID(config) + //$NON-NLS-1$ //$NON-NLS-2$
                    " is being ignored");
                }
                return false;
            }
                ...
                if (ad.getAction() instanceof ObjectPluginAction) {
                    ...
                    ISafeRunnable runnable = new ISafeRunnable() {
                        ...
                        public void handleException(Throwable exception) {
                            WorkbenchPlugin.log(//$NON-NLS-1$
                            "Failed to update action " + action.getId(), exception);
                        }
    ...
    public boolean isApplicableTo(Object object) {
        ...
        if (canAdapt()) {
            ...
            if (adapted == null) {
                if (Policy.DEBUG_CONTRIBUTIONS) {
                    WorkbenchPlugin.log(//$NON-NLS-1$
                    "Error adapting " + object.getClass().getName() + //$NON-NLS-1$
                    " to " + getObjectClass() + ". Contribution " + getID(config) + //$NON-NLS-1$ //$NON-NLS-2$
                    " is being ignored");
                }
            } else {
                object = adapted;
