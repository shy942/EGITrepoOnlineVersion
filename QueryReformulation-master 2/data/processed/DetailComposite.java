...
public class DetailComposite extends Composite {
    ...
    public  DetailComposite(MDirtyable dirtyable, final Composite parent) {
        ...
        URL url = FileLocator.find(Platform.getBundle("org.eclipse.e4.demo.contacts"), new Path("images/dummy.png"), null);
        ...
        // Leads to Aggregate
        createText(composite, "Name:", "name");
        ...
        createVerticalSpace(composite);
        // Bind the image
        final IObservableValue imageObservableValue = PojoProperties.value((Class<?>) contactValue.getValue(), "image", ImageData.class).observeDetail(contactValue);
        ...
        dbc.bindValue(WidgetProperties.image().observe(imageLabel), scaledImage, new UpdateValueStrategy(UpdateValueStrategy.POLICY_NEVER), null);
    ...
    private Text createText(final Composite parent, final String labelText, final String property) {
        final Label label = new Label(parent, SWT.NONE);
        // the extra space is due to a bug in
        ...
        if (property != null) {
            if (property.equals("name")) {
                dbc.bindValue(WidgetProperties.text(SWT.Modify).observe(text), new AggregateNameObservableValue(contactValue));
            } else {
                dbc.bindValue(WidgetProperties.text(SWT.Modify).observe(text), PojoProperties.value((Class<?>) contactValue.getValueType(), property, String.class).observeDetail(contactValue));
            }
        }
