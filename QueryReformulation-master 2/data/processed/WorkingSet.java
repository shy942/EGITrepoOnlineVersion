    ...
    void restoreWorkingSet() {
        ...
        for (int i = 0; i < itemMementos.length; i++) {
            ...
            if (factoryID == null) {
                ...
                "Unable to restore working set item - no factory ID.");
                continue;
            }
            final IElementFactory factory = PlatformUI.getWorkbench().getElementFactory(factoryID);
            if (factory == null) {
                WorkbenchPlugin.log(//$NON-NLS-1$
                "Unable to restore working set item - cannot instantiate factory: " + factoryID);
                continue;
            }
            SafeRunner.run(new SafeRunnable(//$NON-NLS-1$
            "Unable to restore working set item - exception while invoking factory: " + factoryID) {
                ...
                public void run() throws Exception {
                    ...
                    if (item == null) {
                        if (Policy.DEBUG_WORKING_SETS)
                            WorkbenchPlugin.log(//$NON-NLS-1$
                            "Unable to restore working set item - cannot instantiate item: " + factoryID);
                    } else
                        items.add(item);
                }
            });
        ...
        if (workingSetMemento != null) {
            ...
            while (iterator.hasNext()) {
                ...
                if (persistable != null) {
                    ...
                    SafeRunner.run(new SafeRunnable(//$NON-NLS-1$
                    "Problems occurred while saving persistable item state") {
