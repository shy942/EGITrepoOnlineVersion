...
public class TrimBarRenderer extends SWTPartRenderer {
    ...
    public void processContents(MElementContainer<MUIElement> me) {
        ...
        ContributionsAnalyzer.gatherTrimContributions(trimModel, application.getTrimContributions(), trimModel.getElementId(), toContribute, eContext);
    ...
    private void addTrimContributions(final MTrimBar trimModel, ArrayList<MTrimContribution> toContribute, IEclipseContext ctx, final ExpressionContext eContext) {
        ...
        MTrimmedWindow topWin = (MTrimmedWindow) modelService.getTopLevelWindowFor(trimModel);
        ...
        while (!done) {
            ArrayList<MTrimContribution> curList = new ArrayList(toContribute);
            int retryCount = toContribute.size();
            toContribute.clear();
            for (final MTrimContribution contribution : curList) {
                final ArrayList<MTrimElement> toRemove = new ArrayList();
                if (!ContributionsAnalyzer.processAddition(trimModel, contribution, toRemove, existingToolbarIds)) {
