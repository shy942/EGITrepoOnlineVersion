    ...
    class EventTester {
        ...
        EventHandler attListener = new EventHandler() {
            ...
            public void handleEvent(Event event) {
                assertTrue(event.getTopic().equals(topic), //$NON-NLS-1$
                "Incorrect Topic: " + event.getTopic());
                String attId = (String) event.getProperty(EventTags.ATTNAME);
                int attIndex = getAttIndex(attId);
                //$NON-NLS-1$
                assertTrue(attIndex >= 0, "Unknown Attribite: " + attId);
...
*/
        ...
        public void reset() {
            for (int i = 0; i < hasFired.length; i++) hasFired[i] = false;
        }
    ...
    public class AppElementTester extends EventTester {
         ...
         AppElementTester(IEventBroker eventBroker) {
            super("AppElement", ApplicationElement.TOPIC_ALL, new String[] { ApplicationElement.ELEMENTID, ApplicationElement.TAGS, ApplicationElement.PERSISTEDSTATE }, eventBroker);
        }
    }
    ...
    public class CommandTester extends EventTester {
         ...
         CommandTester(IEventBroker eventBroker) {
            super("Command", Command.TOPIC_ALL, new String[] { Command.COMMANDNAME }, eventBroker);
        }
    }
    ...
    public class ContextTester extends EventTester {
         ...
         ContextTester(IEventBroker eventBroker) {
            super("Context", Context.TOPIC_ALL, new String[] { Context.CONTEXT, Context.VARIABLES }, eventBroker);
        }
    }
    ...
    public class ContributionTester extends EventTester {
         ...
         ContributionTester(IEventBroker eventBroker) {
            super("Contribution", Contribution.TOPIC_ALL, new String[] { Contribution.CONTRIBUTIONURI, Contribution.OBJECT }, eventBroker);
        }
    }
    ...
    public class ElementContainerTester extends EventTester {
         ...
         ElementContainerTester(IEventBroker eventBroker) {
            super("ElementContainer", ElementContainer.TOPIC_ALL, new String[] { ElementContainer.CHILDREN, ElementContainer.SELECTEDELEMENT }, eventBroker);
        }
    }
    ...
    public class DirtyableTester extends EventTester {
         ...
         DirtyableTester(IEventBroker eventBroker) {
            super("Dirtyable", Dirtyable.TOPIC_ALL, new String[] { Dirtyable.DIRTY }, eventBroker);
        }
    }
    ...
    public class InputTester extends EventTester {
         ...
         InputTester(IEventBroker eventBroker) {
            super("Input", Input.TOPIC_ALL, new String[] { Input.INPUTURI }, eventBroker);
        }
    }
    ...
    public class UIElementTester extends EventTester {
         ...
         UIElementTester(IEventBroker eventBroker) {
            super("UIElement", UIElement.TOPIC_ALL, new String[] { UIElement.RENDERER, UIElement.TOBERENDERED, UIElement.PARENT, UIElement.ONTOP, UIElement.VISIBLE, UIElement.CONTAINERDATA, UIElement.WIDGET }, eventBroker);
        }
    }
    ...
    public class UIItemTester extends EventTester {
         ...
         UIItemTester(IEventBroker eventBroker) {
            super("UIItem", UILabel.TOPIC_ALL, new String[] { UILabel.LABEL, UILabel.ICONURI, UILabel.TOOLTIP }, eventBroker);
        }
    }
    ...
    public class WindowTester extends EventTester {
         ...
         WindowTester(IEventBroker eventBroker) {
            super("Window", Window.TOPIC_ALL, new String[] { Window.MAINMENU, Window.X, Window.Y, Window.WIDTH, Window.HEIGHT }, eventBroker);
        }
    }
    ...
    public void testAllTopics() {
        IEventBroker eventBroker = (IEventBroker) applicationContext.get(IEventBroker.class.getName());
        ...
        // on incorrect topics
        EventTester[] allTesters = { appTester, commandTester, contextTester, contributionTester, elementContainerTester, inputTester, parameterTester, uiElementTester, uiItemTester, windowTester };
        ...
        appTester.reset();
        allData.setElementId(newId);
        assertTrue("event thrown on No-Op", appTester.getAttIds(true).length == 0);
        // Command
        reset(allTesters);
        ...
        // Window tests
        reset(allTesters);
        MWindow window = ((MApplication) applicationElement).getChildren().get(0);
        window.setX(1234);
        window.setY(1234);
