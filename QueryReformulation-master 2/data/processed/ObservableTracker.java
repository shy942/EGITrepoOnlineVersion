...
*/
    ...
    private static void checkUnmatchedIgnore(Runnable runnable) {
        if (isIgnore()) {
            Policy.getLog().log(new Status(IStatus.ERROR, Policy.JFACE_DATABINDING, //$NON-NLS-1$
            "There were " + currentIgnoreCount.get() + //$NON-NLS-1$
            " unmatched setIgnore(true) invocations in runnable " + runnable));
        }
    }
...
*/
    public static void setIgnore(boolean ignore) {
        ...
        int newCount = (lastCount == null ? 0 : lastCount.intValue()) + (ignore ? 1 : -1);
        if (newCount < 0)
            //$NON-NLS-1$
...
*/
    private static String toString(IObservable observable) {
        return //$NON-NLS-1$
        observable.getClass().getName() + "@" + Integer.toHexString(System.identityHashCode(observable));
    }
...
*/
    public static void getterCalled(IObservable observable) {
        if (observable.isDisposed())
            ...
            "Getter called on disposed observable " + toString(observable));
        Realm realm = observable.getRealm();
        if (!realm.isCurrent())
            Assert.isTrue(false, //$NON-NLS-1$
            "Getter called outside realm of observable " + toString(observable));
        if (isIgnore())
            return;
