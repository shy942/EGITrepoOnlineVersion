    ...
    protected Control createContents(Composite parent) {
        ...
        if (folder.getItemCount() > 0) {
            if (lastSelectedTabId != null) {
                ...
                for (int i = 0; i < items.length; i++) if (items[i].getData(ID).equals(lastSelectedTabId)) {
...
*/
    private void tabSelected(TabItem item) {
        if (item.getData() instanceof IConfigurationElement) {
            ...
            try {
                final InstallationPage page = (InstallationPage) element.createExecutableExtension(IWorkbenchRegistryConstants.ATT_CLASS);
        ...
        Button button = createButton(buttonManager.getParent(), IDialogConstants.CLOSE_ID, IDialogConstants.CLOSE_LABEL, true);
    ...
    private IConfigurationElement[] loadElements() {
        IExtensionPoint point = Platform.getExtensionRegistry().getExtensionPoint("org.eclipse.ui", //$NON-NLS-1$ //$NON-NLS-2$
        "installationPages");
