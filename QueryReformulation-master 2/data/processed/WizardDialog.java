...
*/
    private MessageDialog createWizardClosingDialog() {
        MessageDialog result = new MessageDialog(getShell(), //$NON-NLS-1$
        JFaceResources.getString("WizardClosingDialog.title"), null, //$NON-NLS-1$
        JFaceResources.getString("WizardClosingDialog.message"), MessageDialog.QUESTION, 0, IDialogConstants.OK_LABEL) {
...
*/
    private void restoreUIState(Map<String, Object> saveState) {
        //$NON-NLS-1$
        restoreEnableState(backButton, saveState, "back");
        //$NON-NLS-1$
        restoreEnableState(nextButton, saveState, "next");
        //$NON-NLS-1$
        restoreEnableState(finishButton, saveState, "finish");
        //$NON-NLS-1$
        restoreEnableState(cancelButton, saveState, "cancel");
        //$NON-NLS-1$
        restoreEnableState(helpButton, saveState, "help");
        //$NON-NLS-1$
        Object pageValue = saveState.get("page");
        if (pageValue != null) {
...
*/
    private Map<String, Object> saveUIState(boolean keepCancelEnabled) {
        ...
        //$NON-NLS-1$
        saveEnableStateAndSet(backButton, savedState, "back", false);
        //$NON-NLS-1$
        saveEnableStateAndSet(nextButton, savedState, "next", false);
        //$NON-NLS-1$
        saveEnableStateAndSet(finishButton, savedState, "finish", false);
        //$NON-NLS-1$
        ...
        if (currentPage != null) {
            savedState.put("page", //$NON-NLS-1$
            ControlEnableState.disable(currentPage.getControl()));
        }
        return savedState;
    }
...
*/
    private void updateForPage(IWizardPage page) {
        ...
        if (page.getControl() == null) {
            ...
            Assert.isNotNull(page.getControl(), //$NON-NLS-1$
            JFaceResources.format(//$NON-NLS-1$
            JFaceResources.getString("WizardDialog.missingSetControl"), page.getName()));
...
*/
    private void stopped(Object savedState) {
        if (getShell() != null && !getShell().isDisposed()) {
            ...
            @SuppressWarnings("unchecked") Map<String, Object> state = (Map<String, Object>) savedState;
