...
public class Snippet018CheckboxTableViewerCheckedSelection {
    ...
    static class View {
        ...
        private void createUI(Shell shell) {
            ...
            friendsViewer = CheckboxTableViewer.newCheckList(friendsComposite, SWT.SINGLE | SWT.BORDER | SWT.FULL_SELECTION);
        ...
        private void bindUI() {
            ...
            addPersonButton.addListener(SWT.Selection, new Listener() {
                ...
                public void handleEvent(Event event) {
                    InputDialog dlg = new InputDialog(shell, "Add Person", "Enter name:", "<Name>", new IInputValidator() {
            ...
            ViewerSupport.bind(peopleViewer, people, BeanProperties.values(Person.class, new String[] { "name", "friends" }));
            final IObservableValue selectedPerson = ViewersObservables.observeSingleSelection(peopleViewer);
            ...
            dbc.bindValue(WidgetProperties.text(SWT.Modify).observe(personName), BeanProperties.value((Class) selectedPerson.getValueType(), "name", String.class).observeDetail(selectedPerson));
            ViewerSupport.bind(friendsViewer, people, BeanProperties.value(Person.class, "name"));
            dbc.bindSet(ViewersObservables.observeCheckedElements(friendsViewer, Person.class), BeanProperties.set((Class) selectedPerson.getValueType(), "friends", Person.class).observeDetail(selectedPerson));
        }
    }
}
