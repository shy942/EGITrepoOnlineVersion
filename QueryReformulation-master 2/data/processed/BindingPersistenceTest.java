...
*/
    public final void testAutoLoad() throws ParseException {
        // Get the services.
        ICommandService commandService = fWorkbench.getAdapter(ICommandService.class);
        IBindingService bindingService = fWorkbench.getAdapter(IBindingService.class);
        bindingService.readRegistryAndPreferences(commandService);
        // Check the pre-conditions.
        final String emacsSchemeId = EMACS_SCHEME_ID;
        assertFalse("The active scheme should be Emacs yet", emacsSchemeId.equals(bindingService.getActiveScheme().getId()));
        ...
        for (i = 0; i < bindings.length; i++) {
            ...
            if ((binding.getType() == Binding.USER) && (formalKeySequence.equals(binding.getTriggerSequence()))) {
        ...
        assertEquals("The active scheme should now be Emacs", emacsSchemeId, bindingService.getActiveScheme().getId());
    ...
    public final void testSinglePlatform() throws Exception {
        ...
        ICommandService commandService = fWorkbench.getAdapter(ICommandService.class);
        IBindingService bindingService = fWorkbench.getAdapter(IBindingService.class);
        ParameterizedCommand about = new ParameterizedCommand(commandService.getCommand("org.eclipse.ui.help.aboutAction"), null);
            ...
            if (binding.getType() == Binding.SYSTEM) {
                ...
                if (about.equals(binding.getParameterizedCommand())) {
                    if (m18A.equals(binding.getTriggerSequence())) {
                        ...
                        assertTrue("failure for platform: " + binding.getPlatform(), Util.WS_CARBON.equals(binding.getPlatform()) || Util.WS_COCOA.equals(binding.getPlatform()));
                    }
                }
            }
        }
    ...
    public final void TODOtestBindingTransform() throws Exception {
        ICommandService commandService = fWorkbench.getAdapter(ICommandService.class);
        IBindingService bindingService = fWorkbench.getAdapter(IBindingService.class);
        ParameterizedCommand addWS = new ParameterizedCommand(commandService.getCommand("org.eclipse.ui.navigate.addToWorkingSet"), null);
            ...
            if (binding.getType() == Binding.SYSTEM) {
                ...
                if (addWS.equals(binding.getParameterizedCommand())) {
                    if (m18w.equals(binding.getTriggerSequence())) {
                        ...
                        numOfMarkers++;
                        assertTrue(platform, Util.WS_CARBON.equals(platform) || Util.WS_COCOA.equals(platform) || Util.WS_GTK.equals(platform) || Util.WS_WIN32.equals(platform));
                    }
        ...
        IContextService contextService = fWorkbench.getService(IContextService.class);
    ...
    public void testModifierWithPlatform() throws Exception {
        ICommandService commandService = fWorkbench.getAdapter(ICommandService.class);
        IBindingService bindingService = fWorkbench.getAdapter(IBindingService.class);
        ParameterizedCommand importCmd = new ParameterizedCommand(commandService.getCommand("org.eclipse.ui.file.import"), null);
        Binding[] bindings = bindingService.getBindings();
        int numOfMarkers = 0;
        for (final Binding binding : bindings) {
            if (binding.getType() != Binding.SYSTEM) {
    ...
    public void testModifierNotApplied() throws Exception {
        ICommandService commandService = fWorkbench.getAdapter(ICommandService.class);
        IBindingService bindingService = fWorkbench.getAdapter(IBindingService.class);
        ParameterizedCommand exportCmd = new ParameterizedCommand(commandService.getCommand("org.eclipse.ui.file.export"), null);
        Binding[] bindings = bindingService.getBindings();
        for (final Binding binding : bindings) {
            if (binding.getType() != Binding.SYSTEM) {
                continue;
    ...
    public void testDifferentPlatform() throws Exception {
        ICommandService commandService = fWorkbench.getAdapter(ICommandService.class);
        IBindingService bindingService = fWorkbench.getAdapter(IBindingService.class);
        ParameterizedCommand backCmd = new ParameterizedCommand(commandService.getCommand("org.eclipse.ui.navigate.back"), null);
        Binding[] bindings = bindingService.getBindings();
        for (final Binding binding : bindings) {
            if (binding.getType() != Binding.SYSTEM) {
                continue;
    ...
    public void testPasteAndRedoBindingEmacs() throws Exception {
        ...
        final Scheme emacsScheme = bindingService.getScheme(EMACS_SCHEME_ID);
        assertNotNull(emacsScheme);
        final Scheme defaultScheme = bindingService.getScheme(IBindingService.DEFAULT_DEFAULT_ACTIVE_SCHEME_ID);
        assertNotNull(defaultScheme);
        final Binding[] originalBindings = bindingService.getBindings();
        bindingService.savePreferences(emacsScheme, originalBindings);
        ParameterizedCommand pasteCmd = new ParameterizedCommand(commandService.getCommand(IWorkbenchCommandConstants.EDIT_PASTE), null);
        ParameterizedCommand redoCmd = new ParameterizedCommand(commandService.getCommand(IWorkbenchCommandConstants.EDIT_REDO), null);
    ...
    protected void doTearDown() throws Exception {
        final IPreferenceStore store = WorkbenchPlugin.getDefault().getPreferenceStore();
        store.setValue("org.eclipse.ui.commands", "<?xml version=\"1.0\" encoding=\"UTF-8\"?><org.eclipse.ui.commands><activeKeyConfiguration keyConfigurationId=\"" + IBindingService.DEFAULT_DEFAULT_ACTIVE_SCHEME_ID + "\"/></org.eclipse.ui.commands>");
        IBindingService bindingService = fWorkbench.getAdapter(IBindingService.class);
        // reset keybindings
