    ...
    public  StructuredContentProviderTest() {
        ...
        shell = new Shell(Display.getCurrent(), SWT.SHELL_TRIM);
        {
            ...
            Group operation = new Group(shell, SWT.NONE);
            {
                ...
                createRadioButton(//$NON-NLS-1$
                operation, //$NON-NLS-1$
                currentFunction, //$NON-NLS-1$
                "f(x) = x", Integer.valueOf(SomeMathFunction.OP_IDENTITY));
                createRadioButton(//$NON-NLS-1$
                operation, //$NON-NLS-1$
                currentFunction, //$NON-NLS-1$
                "f(x) = 2 * x", Integer.valueOf(SomeMathFunction.OP_MULTIPLY));
                createRadioButton(operation, currentFunction, "f(x) = floor(x)", //$NON-NLS-1$
                Integer.valueOf(SomeMathFunction.OP_ROUND));
            ...
            new ControlUpdater(sumLabel) {
                ...
                protected void updateControl() {
                    ...
                    "The sum of the above " + size + " doubles is " + //$NON-NLS-1$
                    sum);
                }
            };
...
*/
    private void createDataModel() {
        ...
        // of radio buttons
        currentFunction = new WritableValue(realm, Integer.valueOf(SomeMathFunction.OP_MULTIPLY), null);
