...
public class CSSPropertyTextSWTHandler extends AbstractCSSPropertyTextHandler {
    ...
    @Override
    public void applyCSSPropertyTextTransform(Object element, final CSSValue value, String pseudo, CSSEngine engine) throws Exception {
        Widget widget = (Widget) element;
        String defaultText = (String) widget.getData(CSSSWTConstants.TEXT_KEY);
        if (element instanceof Text) {
            final Text text = (Text) widget;
            VerifyListener listener = (VerifyListener) text.getData("CSSModifyTextListener");
            ...
            String newText = getTextTransform(text.getText(), value, defaultText);
        ...
        if (element instanceof Label) {
            ...
            label.setText(getTextTransform(label.getText(), value, defaultText));
            return;
        }
        if (element instanceof Button) {
            Button button = (Button) element;
            button.setText(getTextTransform(button.getText(), value, defaultText));
            return;
        }
    }
