...
*/
        ...
        if (haveParent) {
            ...
            EContextService cs = ((KeyBindingService) activeService).workbenchPartSite.getService(EContextService.class);
...
*/
            IHandlerService hs = ((KeyBindingService) activeService).workbenchPartSite.getService(IHandlerService.class);
            Iterator<Entry<IAction, IHandlerActivation>> i = ((KeyBindingService) activeService).actionToProxy.entrySet().iterator();
            while (i.hasNext()) {
...
*/
    ...
    public void registerAction(IAction action) {
        ...
        if (action instanceof CommandLegacyActionWrapper) {
            ...
            "Cannot register a CommandLegacyActionWrapper back into the system");
            return;
        }
        ...
        if (commandId != null) {
            ...
            IHandlerService hs = workbenchPartSite.getService(IHandlerService.class);
            actionToProxy.put(action, hs.activateHandler(commandId, new ActionHandler(action), new LegacyHandlerSubmissionExpression(null, partSite.getShell(), partSite)));
        }
    }
