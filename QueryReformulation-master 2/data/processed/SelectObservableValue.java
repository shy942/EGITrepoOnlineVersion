...
*/
    ...
    private int indexOfValue(Object value) {
        for (int i = 0; i < options.size(); i++) if (Util.equals(options.get(i).value, value))
            return i;
        return -1;
    }

    private int indexOfObservable(IObservableValue<? extends Boolean> observable) {
        for (int i = 0; i < options.size(); i++) if (options.get(i).observable == observable)
            return i;
        return -1;
    }
}
