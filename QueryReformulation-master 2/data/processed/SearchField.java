...
public class SearchField {
    ...
    void createControls(final Composite parent, MApplication application, MWindow window) {
        ...
        QuickAccessProvider[] providers = new QuickAccessProvider[] { new PreviousPicksProvider(previousPicksList), new EditorProvider(), new ViewProvider(application, window), new PerspectiveProvider(), commandProvider, new ActionProvider(), new WizardProvider(), new PreferenceProvider(), new PropertiesProvider() };
...
*/
        ...
        // just for debugging, not shown anywhere
        shell.setText(QuickAccessMessages.QuickAccess_EnterSearch);
    ...
    @Inject
    @Optional
    protected void keybindingPreferencesChanged(@SuppressWarnings("restriction") @Preference(nodePath = "org.eclipse.ui.workbench", value = "org.eclipse.ui.commands") String preferenceValue) {
...
*/
    ...
    private void updateQuickAccessText() {
        ...
        if (triggerSequence != null) {
            txtQuickAccess.setToolTipText(NLS.bind(QuickAccessMessages.QuickAccess_TooltipDescription, triggerSequence.format()));
        } else {
    ...
    private void hookUpSelectAll() {
        ...
        Expression focusExpr = new Expression() {
            ...
            public EvaluationResult evaluate(IEvaluationContext context) {
                return EvaluationResult.valueOf(SearchField.class.getName().equals(context.getVariable(ISources.ACTIVE_FOCUS_CONTROL_ID_NAME)));
            }
        };
...
*/
    ...
    void layoutShell() {
        ...
        shell.setBounds(getConstrainedShellBounds(display, new Rectangle(compBounds.x, compBounds.y + compBounds.height, width, height)));
        shell.layout();
    }
...
*/
    ...
    private void storeDialog() {
        ...
        for (int i = 0; i < orderedElements.length; i++) {
            ...
            //$NON-NLS-1$
            textEntries[i] = elementText.size() + "";
        }
