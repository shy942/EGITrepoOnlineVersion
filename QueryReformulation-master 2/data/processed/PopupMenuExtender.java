...
*/
    public  PopupMenuExtender(final String id, final MenuManager menu, final ISelectionProvider prov, final IWorkbenchPart part, IEclipseContext context, final boolean includeEditorInput) {
        super();
        this.menu = menu;
    ...
    private void createModelFor(String id) {
        ...
        for (MMenu item : modelPart.getMenus()) {
            if (id.equals(item.getElementId()) && item instanceof MPopupMenu && item.getTags().contains("popup")) {
                //$NON-NLS-1$
...
*/
    public final boolean matches(final MenuManager menuManager, final ISelectionProvider selectionProvider, final IWorkbenchPart part) {
    ...
    private void addEditorActions(IMenuManager mgr, Set<IObjectActionContributor> alreadyContributed) {
        ISelectionProvider activeEditor = new ISelectionProvider() {
            ...
            public void addSelectionChangedListener(ISelectionChangedListener listener) {
                ...
                "This ISelectionProvider is static, and cannot be modified.");
            }
        ...
        if (ObjectActionContributorManager.getManager().contributeObjectActions(part, mgr, activeEditor, alreadyContributed)) {
...
*/
    private void addObjectActions(IMenuManager mgr, Set<IObjectActionContributor> alreadyContributed) {
        if (selProvider != null) {
            if (ObjectActionContributorManager.getManager().contributeObjectActions(part, mgr, selProvider, alreadyContributed)) {
                mgr.add(new Separator());
            }
        }
    }
...
*/
    ...
    private void cleanUpContributionCache() {
        if (!actionContributionCache.isEmpty()) {
            PluginActionContributionItem[] items = actionContributionCache.toArray(new PluginActionContributionItem[actionContributionCache.size()]);
