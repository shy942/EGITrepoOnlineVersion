...
*/
    ...
    private boolean addModel(Object source, Saveable model) {
        if (model == null) {
            logWarning("Ignored attempt to add invalid saveable", source, //$NON-NLS-1$
            model);
            return false;
        }
        ...
        if (modelsForSource.add(model)) {
            ...
            logWarning("Ignored attempt to add saveable that was already registered", source, //$NON-NLS-1$
            model);
        }
        return result;
    }
...
*/
    private boolean decrementRefCount(Map<Saveable, Integer> referenceMap, Saveable key) {
        ...
        if (refCount == null)
            //$NON-NLS-1$
            Assert.isTrue(false, key + ": " + key.getName());
    ...
    private boolean removeModel(Object source, Saveable model) {
        ...
        Set<Saveable> modelsForSource = modelMap.get(source);
        if (modelsForSource == null) {
            logWarning("Ignored attempt to remove a saveable when no saveables were known", source, //$NON-NLS-1$
            model);
        } else {
            if (modelsForSource.remove(model)) {
                ...
                logWarning("Ignored attempt to remove a saveable that was not registered", source, //$NON-NLS-1$
                model);
            }
        }
        return result;
    ...
    private void logWarning(String message, Object source, Saveable model) {
        ...
        AssertionFailedException assertionFailedException = new AssertionFailedException(//$NON-NLS-1$
        "unknown saveable: " + model + " from part: " + //$NON-NLS-1$
        source);
    ...
    public void handleLifecycleEvent(SaveablesLifecycleEvent event) {
        ...
        switch(event.getEventType()) {
            ...
            case SaveablesLifecycleEvent.PRE_CLOSE:
                ...
                boolean canceled = promptForSavingIfNecessary(PlatformUI.getWorkbench().getActiveWorkbenchWindow(), modelsClosing, modelsDecrementing, !event.isForce());
                if (canceled) {
                    event.setVeto(true);
                }
                break;
            ...
            case SaveablesLifecycleEvent.DIRTY_CHANGED:
                fireModelLifecycleEvent(new SaveablesLifecycleEvent(this, event.getEventType(), event.getSaveables(), false));
                break;
        }
    }
...
*/
    private void removeModels(Object source, Saveable[] modelArray) {
        ...
        if (removed.size() > 0) {
            fireModelLifecycleEvent(new SaveablesLifecycleEvent(this, SaveablesLifecycleEvent.POST_OPEN, removed.toArray(new Saveable[removed.size()]), false));
        }
    }
...
*/
    private void addModels(Object source, Saveable[] modelArray) {
        ...
        if (added.size() > 0) {
            fireModelLifecycleEvent(new SaveablesLifecycleEvent(this, SaveablesLifecycleEvent.POST_OPEN, added.toArray(new Saveable[added.size()]), false));
        }
    }
...
*/
    ...
    public Object preCloseParts(List<IWorkbenchPart> partsToClose, boolean save, IShellProvider shellProvider, final IWorkbenchWindow window) {
        return preCloseParts(partsToClose, false, save, shellProvider, window);
    }

    public Object preCloseParts(List<IWorkbenchPart> partsToClose, boolean addNonPartSources, boolean save, IShellProvider shellProvider, final IWorkbenchWindow window) {
        ...
        if (save) {
            boolean canceled = promptForSavingIfNecessary(shellProvider, window, postCloseInfo.modelsClosing, postCloseInfo.modelsDecrementing, true);
            if (canceled) {
                return null;
            }
        }
...
*/
    private boolean promptForSavingIfNecessary(final IWorkbenchWindow window, Set<Saveable> modelsClosing, Map<Saveable, Integer> modelsDecrementing, boolean canCancel) {
        return promptForSavingIfNecessary(window, window, modelsClosing, modelsDecrementing, canCancel);
    }
    ...
    private boolean promptForSavingIfNecessary(IShellProvider shellProvider, IWorkbenchWindow window, Set<Saveable> modelsClosing, Map<Saveable, Integer> modelsDecrementing, boolean canCancel) {
        ...
        boolean shouldCancel = modelsToOptionallySave.isEmpty() ? false : promptForSaving(modelsToOptionallySave, shellProvider, window, canCancel, true);
        if (shouldCancel) {
            return true;
        }
        ...
        return modelsToSave.isEmpty() ? false : promptForSaving(modelsToSave, shellProvider, window, canCancel, false);
    }
...
* @return true if the user canceled
*/
    public boolean promptForSaving(List<Saveable> modelsToSave, final IShellProvider shellProvider, IRunnableContext runnableContext, final boolean canCancel, boolean stillOpenElsewhere) {
        ...
        if (modelsToSave.size() > 0) {
            ...
            if (dontPrompt) {
                ...
                String[] buttons;
                if (canCancel) {
                    buttons = new String[] { IDialogConstants.YES_LABEL, IDialogConstants.NO_LABEL, IDialogConstants.CANCEL_LABEL };
                } else {
                ...
                if (stillOpenElsewhere) {
                    String message = NLS.bind(WorkbenchMessages.EditorManager_saveChangesOptionallyQuestion, model.getName());
                    MessageDialogWithToggle dialogWithToggle = new MessageDialogWithToggle(shellProvider.getShell(), WorkbenchMessages.Save_Resource, null, message, MessageDialog.QUESTION, buttons, 0, WorkbenchMessages.EditorManager_closeWithoutPromptingOption, false) {

                        @Override
                        protected int getShellStyle() {
                            return (canCancel ? SWT.CLOSE : SWT.NONE) | SWT.TITLE | SWT.BORDER | SWT.APPLICATION_MODAL | SWT.SHEET | getDefaultOrientation();
                        }
                    };
                    dialog = dialogWithToggle;
                } else {
                    String message = NLS.bind(WorkbenchMessages.EditorManager_saveChangesQuestion, model.getName());
                    dialog = new MessageDialog(shellProvider.getShell(), WorkbenchMessages.Save_Resource, null, message, MessageDialog.QUESTION, 0, buttons) {
...
*/
    public boolean saveModels(final List<Saveable> finalModels, final IShellProvider shellProvider, IRunnableContext runnableContext) {
...
* @return <code>true</code> if the operation was canceled
*/
    public boolean saveModels(final List<Saveable> finalModels, final IShellProvider shellProvider, IRunnableContext runnableContext, final boolean blockUntilSaved) {
        IRunnableWithProgress progressOp = new IRunnableWithProgress() {

            @Override
            public void run(IProgressMonitor monitor) {
                IProgressMonitor monitorWrap = new EventLoopProgressMonitor(monitor);
                SubMonitor subMonitor = SubMonitor.convert(monitorWrap, WorkbenchMessages.Saving_Modifications, finalModels.size());
...
*/
    public void postClose(Object postCloseInfoObject) {
        ...
        if (removed.size() > 0) {
            fireModelLifecycleEvent(new SaveablesLifecycleEvent(this, SaveablesLifecycleEvent.POST_CLOSE, removed.toArray(new Saveable[removed.size()]), false));
        }
    }
...
*/
    public void dirtyChanged(IWorkbenchPart part) {
        ...
        if (saveables.length > 0) {
            fireModelLifecycleEvent(new SaveablesLifecycleEvent(this, SaveablesLifecycleEvent.DIRTY_CHANGED, saveables, false));
        }
    }
