...
*/
    ...
    void createWidget(Composite parent, MToolControl toolControl) {
        ...
        boolean showOpenOnPerspectiveBar = PrefUtil.getAPIPreferenceStore().getBoolean(IWorkbenchPreferenceConstants.SHOW_OPEN_ON_PERSPECTIVE_BAR);
        ...
        bar.addMouseListener(new MouseListener() {
            ...
            public void mouseUp(MouseEvent e) {
                ...
                if (curItem != null && curItem.getData() instanceof MPerspective) {
                    ...
                    boolean atStart = (perspSwitcherToolbar.getStyle() & SWT.HORIZONTAL) != 0 ? e.x < center.x : e.y < center.y;
                    ...
                    if (!atStart)
                        // We're 'after' the item we're over
                        dropPerspIndex++;
                    if (dropPerspIndex > dragPerspIndex)
                        // Need to account for the removal of
                        dropPerspIndex--;
    ...
    private ToolItem addPerspectiveItem(MPerspective persp) {
        ...
        // HACK !! accounts for the 'open' and the
        int index = perspIndex + 2;
        // separator
        final ToolItem psItem = index < perspSwitcherToolbar.getItemCount() ? new ToolItem(perspSwitcherToolbar, SWT.RADIO, index) : new ToolItem(perspSwitcherToolbar, SWT.RADIO);
        ...
        if (!foundImage || PrefUtil.getAPIPreferenceStore().getBoolean(IWorkbenchPreferenceConstants.SHOW_TEXT_ON_PERSPECTIVE_BAR)) {
    ...
    private void selectPerspective() {
        ...
        ParameterizedCommand command = commandService.createCommand(IWorkbenchCommandConstants.PERSPECTIVES_SHOW_PERSPECTIVE, Collections.EMPTY_MAP);
    ...
    private void addShowTextItem(final Menu menu) {
        ...
        showtextMenuItem.addSelectionListener(new SelectionAdapter() {
            ...
            public void widgetSelected(SelectionEvent e) {
                ...
                if (preference != PrefUtil.getAPIPreferenceStore().getDefaultBoolean(IWorkbenchPreferenceConstants.SHOW_TEXT_ON_PERSPECTIVE_BAR)) {
        ...
        showtextMenuItem.setSelection(PrefUtil.getAPIPreferenceStore().getBoolean(IWorkbenchPreferenceConstants.SHOW_TEXT_ON_PERSPECTIVE_BAR));
    }
    ...
    void resize() {
        ...
        Color gradientTop = new Color(comp.getDisplay(), backgroundRGB.red + 12, backgroundRGB.green + 10, backgroundRGB.blue + 10);
        int h = size.y;
        int curveStart = 0;
        int curve_width = 5;
