...
public final class ShowPerspectiveHandler extends AbstractHandler {
    ...
    public final Object execute(final ExecutionEvent event) throws ExecutionException {
        ...
        final String newWindow = (String) parameters.get(IWorkbenchCommandConstants.PERSPECTIVES_SHOW_PERSPECTIVE_PARM_NEWWINDOW);
        if (value == null) {
            openOther(window);
        } else {
            if (newWindow == null || newWindow.equalsIgnoreCase("false")) {
                //$NON-NLS-1$
...
*/
    private void openNewWindowPerspective(String perspectiveId, IWorkbenchWindow activeWorkbenchWindow) throws ExecutionException {
        ...
        try {
            ...
            ErrorDialog.openError(activeWorkbenchWindow.getShell(), WorkbenchMessages.ChangeToPerspectiveMenu_errorTitle, e.getMessage(), e.getStatus());
        }
    }
...
*             If the perspective could not be opened.
*/
    private final void openOther(final IWorkbenchWindow activeWorkbenchWindow) throws ExecutionException {
        final SelectPerspectiveDialog dialog = new SelectPerspectiveDialog(activeWorkbenchWindow.getShell(), WorkbenchPlugin.getDefault().getPerspectiveRegistry());
        dialog.open();
        ...
        if (descriptor != null) {
            int openPerspMode = WorkbenchPlugin.getDefault().getPreferenceStore().getInt(IPreferenceConstants.OPEN_PERSP_MODE);
...
*             If the perspective could not be opened.
*/
    private final void openPerspective(final String perspectiveId, final IWorkbenchWindow activeWorkbenchWindow) throws ExecutionException {
        final IWorkbench workbench = PlatformUI.getWorkbench();
        ...
        if (desc == null) {
            ...
            "Perspective " + perspectiveId + //$NON-NLS-1$
            " cannot be found.");
        }
        try {
            if (activePage != null) {
