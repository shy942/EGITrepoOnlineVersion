...
*/
    ...
    static final QualifiedName INFRASTRUCTURE_PROPERTY = new QualifiedName(WorkbenchPlugin.PI_WORKBENCH, //$NON-NLS-1$
...
*/
    static IStatus exceptionStatus(Throwable exception) {
        return StatusUtil.newStatus(IStatus.ERROR, //$NON-NLS-1$
...
*/
    static void openProgressView(IWorkbenchWindow window) {
        ...
        try {
            IViewDescriptor reference = WorkbenchPlugin.getDefault().getViewRegistry().find(IProgressConstants.PROGRESS_VIEW_ID);
            if (reference == null) {
                return;
            }
...
*/
    public static URL getProgressSpinnerLocation() {
        try {
            //$NON-NLS-1$
            return new URL(getIconsRoot(), "progress_spinner.png");
