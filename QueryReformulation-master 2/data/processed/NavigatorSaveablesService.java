        ...
        synchronized (instances) {
            if (event.getType() == BundleEvent.STARTED) {
                ...
                for (Iterator<NavigatorSaveablesService> it = instances.iterator(); it.hasNext(); ) {
                    NavigatorSaveablesService instance = it.next();
    ...
    private class LifecycleListener implements ISaveablesLifecycleListener {
        ...
        public void handleLifecycleEvent(SaveablesLifecycleEvent event) {
            ...
            if (shownSaveables != null && shownSaveables.length > 0) {
                outsideListener.handleLifecycleEvent(new SaveablesLifecycleEvent(saveablesSource, SaveablesLifecycleEvent.DIRTY_CHANGED, shownSaveables, false));
            }
        }
    }
...
*/
    @Override
    public void init(final ISaveablesSource saveablesSource, final StructuredViewer viewer, ISaveablesLifecycleListener outsideListener) {
...
*/
    private Saveable[] computeSaveables() {
        ...
        Set<Object> roots = new HashSet<Object>(Arrays.asList(contentProvider.getElements(viewerInput)));
        ...
        if (saveablesProviders == null) {
            ...
            for (int i = 0; i < descriptors.length; i++) {
                ...
                if (Platform.getBundle(pluginId).getState() != Bundle.ACTIVE) {
                    List<NavigatorContentDescriptor> inactiveDescriptors = inactivePluginsWithSaveablesProviders.get(pluginId);
            ...
            saveablesProviders = result.toArray(new SaveablesProvider[result.size()]);
        }
        ...
        if (addedSaveables.size() > 0) {
            Display.getDefault().asyncExec(new Runnable() {
                ...
                public void run() {
                    ...
                    outsideListener.handleLifecycleEvent(new SaveablesLifecycleEvent(saveablesSource, SaveablesLifecycleEvent.POST_OPEN, addedSaveables.toArray(new Saveable[addedSaveables.size()]), false));
                }
            });
        }
        ...
        if (removedSaveables.size() > 0) {
            Display.getDefault().asyncExec(new Runnable() {
                ...
                public void run() {
                    ...
                    outsideListener.handleLifecycleEvent(new SaveablesLifecycleEvent(saveablesSource, SaveablesLifecycleEvent.PRE_CLOSE, removedSaveables.toArray(new Saveable[removedSaveables.size()]), true));
                    outsideListener.handleLifecycleEvent(new SaveablesLifecycleEvent(saveablesSource, SaveablesLifecycleEvent.POST_CLOSE, removedSaveables.toArray(new Saveable[removedSaveables.size()]), false));
                }
            });
        }
    }
...
*/
    private void updateSaveablesProviders(String startedBundleId) {
        ...
        List descriptors = inactivePluginsWithSaveablesProviders.get(startedBundleId);
