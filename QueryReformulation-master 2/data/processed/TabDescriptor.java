...
*/
    public  TabDescriptor(IConfigurationElement configurationElement) {
        ...
        if (configurationElement != null) {
            ...
            if (imageString != null) {
                image = AbstractUIPlugin.imageDescriptorFromPlugin(configurationElement.getDeclaringExtension().getNamespaceIdentifier(), imageString).createImage();
            }
...
*/
    private boolean insertSectionDescriptor(ISectionDescriptor target) {
        ...
        for (int i = 0; i < getSectionDescriptors().size(); i++) {
            ISectionDescriptor descriptor = (ISectionDescriptor) getSectionDescriptors().get(i);
...
*/
    private void handleTabError(IConfigurationElement configurationElement, CoreException exception) {
        ...
        IStatus status = new Status(IStatus.ERROR, pluginId, TabbedPropertyViewStatusCodes.TAB_ERROR, message, exception);
