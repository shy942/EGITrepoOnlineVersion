...
public class FormHeading extends Canvas {
    ...
    private class DefaultMessageToolTipManager implements IMessageToolTipManager {
        ...
        @Override
        public void update() {
            String details = getMessageType() == 0 ? null : MessageManager.createDetails(getChildrenMessages());
            if (messageRegion != null)
                messageRegion.updateToolTip(details);
            if (getMessageType() > 0 && (details == null || details.length() == 0))
                details = getMessage();
    ...
    private class FormHeadingLayout extends Layout implements ILayoutExtension {
        ...
        private Point layout(Composite composite, boolean move, int x, int y, int width, int height, boolean flushCache) {
            ...
            if (totalFlexWidth != SWT.DEFAULT) {
                ...
                if (hasToolBar() && getToolBarAlignment() == SWT.TOP || hasMessageRegion())
...
*/
            ...
            if (!move) {
                ...
                if (msize != null || (tbsize != null && getToolBarAlignment() == SWT.TOP))
                    width1 += SPACING;
                size.x = width1;
                if (clsize != null) {
                    int width2 = clsize.x;
                ...
                titleRegion.setBounds(xloc, // yloc + row1Height / 2 - tsize.y / 2,
                ...
                if (hasMessageRegion()) {
                    ...
                    if (tsize.y > 0) {
                        ...
                        messageOffset = (titleLeadingSpace + titleRegion.getFontBaselineHeight()) - (messageLeadingSpace + messageRegion.getFontBaselineHeight());
                    }
                    messageRegion.getMessageControl().setBounds(xloc, tsize.y > 0 ? (yloc + messageOffset) : (yloc + row1Height / 2 - msize.y / 2), msize.x, msize.y);
                    xloc += msize.x;
                }
                if (toolBarManager != null)
                    toolBarManager.getControl().setVisible(!toolBarManager.isEmpty());
                if (tbsize != null && getToolBarAlignment() == SWT.TOP) {
                    ToolBar tbar = toolBarManager.getControl();
                    tbar.setBounds(x + width - 1 - tbsize.x - HMARGIN, yloc + row1Height - 1 - tbsize.y, tbsize.x, tbsize.y);
                }
                // second row
                xloc = HMARGIN;
                ...
                if (tbsize != null && getToolBarAlignment() == SWT.BOTTOM) {
                    ToolBar tbar = toolBarManager.getControl();
                    tbar.setBounds(x + width - 1 - tbsize.x - HMARGIN, yloc, tbsize.x, tbsize.y);
                    tw = tbsize.x + SPACING;
                }
                if (headClient != null) {
        ...
        private void ensureControlExists() {
            if (needHyperlink()) {
                ...
                if (messageHyperlink == null) {
                    ...
                    messageHyperlink.setHref(messages);
                    for (IHyperlinkListener element : listeners) messageHyperlink.addHyperlinkListener(element);
...
*/
    ...
    public final Point computeSize(int wHint, int hHint, boolean changed) {
        return ((FormHeadingLayout) getLayout()).computeSize(this, wHint, hHint, changed);
    }
...
*/
    ...
    private void updateGradientImage() {
        ...
        if (gradientInfo != null) {
            gradientImage = FormImages.getInstance().getGradient(gradientInfo.gradientColors, gradientInfo.percents, gradientInfo.vertical ? rect.height : rect.width, gradientInfo.vertical, getColor(COLOR_BASE_BG), getDisplay());
    ...
    public IMessage[] getChildrenMessages() {
        return messageRegion != null ? messageRegion.getChildrenMessages() : NULL_MESSAGE_ARRAY;
    }
