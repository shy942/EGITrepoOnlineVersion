...
*/
     CommandLegacyWrapper(final Command command, final BindingManager bindingManager) {
        if (command == null) {
            ...
            "The wrapped command cannot be <code>null</code>.");
        }
        if (bindingManager == null) {
            throw new NullPointerException(//$NON-NLS-1$
            "A binding manager is required to wrap a command");
        }
    ...
    public final Object execute(Map parameterValuesByName) throws ExecutionException, NotHandledException {
        try {
            ...
            return command.execute(new ExecutionEvent(command, (parameterValuesByName == null) ? Collections.EMPTY_MAP : parameterValuesByName, null, service.getCurrentState()));
    ...
    public final List getKeySequenceBindings() {
        ...
        for (int i = 0; i < activeBindingsCount; i++) {
            ...
            if (triggerSequence instanceof org.eclipse.jface.bindings.keys.KeySequence) {
                legacyBindings.add(new KeySequenceBinding(KeySequence.getInstance((org.eclipse.jface.bindings.keys.KeySequence) triggerSequence), 0));
            }
        }
