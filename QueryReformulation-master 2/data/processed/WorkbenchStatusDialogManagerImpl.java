...
public class WorkbenchStatusDialogManagerImpl {
    ...
    static final QualifiedName HINT = new QualifiedName(IStatusAdapterConstants.PROPERTY_PREFIX, //$NON-NLS-1$
    "hint");
...
*/
    public boolean shouldAccept(StatusAdapter statusAdapter) {
        ...
        boolean handleOKStatuses = ((Boolean) dialogState.get(IStatusDialogConstants.HANDLE_OK_STATUSES)).booleanValue();
...
*/
    public  WorkbenchStatusDialogManagerImpl(int displayMask, String dialogTitle) {
        Assert.isNotNull(Display.getCurrent(), //$NON-NLS-1$
        "WorkbenchStatusDialogManager must be instantiated in UI thread");
...
*/
    public Map initDialogState(Map dialogState, int displayMask, String dialogTitle) {
        ...
        dialogState.put(IStatusDialogConstants.TITLE, dialogTitle == null ? JFaceResources.getString(//$NON-NLS-1$
        "Problem_Occurred") : dialogTitle);
    ...
    private void doAddStatusAdapter(final StatusAdapter statusAdapter, final boolean modal) {
        ...
        if (isDialogClosed()) {
            ...
            if (shouldPrompt(statusAdapter)) {
                ...
                // handled
                StatusManager.getManager().fireNotification(INotificationTypes.HANDLED, (StatusAdapter[]) getErrors().toArray(new StatusAdapter[] {}));
                if (dialog == null) {
                    ...
                    boolean showSupport = ((Boolean) dialogState.get(IStatusDialogConstants.SHOW_SUPPORT)).booleanValue();
                    if (showSupport) {
                        dialog.openTray();
...
*/
    ...
    public void setStatusListLabelProvider(ITableLabelProvider labelProvider) {
        //$NON-NLS-1$
        Assert.isLegal(labelProvider != null, "Label Provider cannot be null");
...
*/
    private Collection getErrors() {
        return (Collection) dialogState.get(IStatusDialogConstants.STATUS_ADAPTERS);
    }
