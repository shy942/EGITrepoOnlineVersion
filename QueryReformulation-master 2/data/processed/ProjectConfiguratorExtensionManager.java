...
public class ProjectConfiguratorExtensionManager {
    ...
    //$NON-NLS-1$
    private static final String EXTENSION_POINT_ID = IDEWorkbenchPlugin.IDE_WORKBENCH + ".projectConfigurators";
            ...
            if (contributingBundle.getState() == Bundle.ACTIVE) {
                ...
                if (activeWhenElements.length == 0) {
                    ...
                    if (activeWhenChildren.length == 1) {
                        ...
                        "Could not evaluate xpression for " + extension.getContributor().getName() + //$NON-NLS-1$
                        ": there must be exactly one child of 'activeWhen'");
                    }
                } else {
                    IDEWorkbenchPlugin.log(//$NON-NLS-1$
                    "Only one 'activeWhen' is authorized on extension point " + EXTENSION_POINT_ID + //$NON-NLS-1$
                    ", for extension contributed by " + extension.getContributor().getName());
                }
            }
            if (addIt) {
