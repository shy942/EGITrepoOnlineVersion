            ...
            if (action.isEnabled()) {
                ...
                if (callback != null) {
                    if (result == null || result.equals(Boolean.TRUE)) {
                        ...
                        callback.postExecuteFailure(action, new ExecutionException(action.getText() + //$NON-NLS-1$
                        " returned failure."));
                    }
                }
                ...
                if (trace) {
                    System.out.println((System.currentTimeMillis() - ms) + " ms to run action: " + //$NON-NLS-1$
                    action.getText());
                }
            } else {
                if (callback != null) {
                    callback.notEnabled(action, new NotEnabledException(action.getText() + //$NON-NLS-1$
                    " is not enabled."));
                }
            }
        }
...
*/
    ...
    public void update(String propertyName) {
        if (widget != null) {
            ...
            boolean checkChanged = (action.getStyle() == IAction.AS_CHECK_BOX || action.getStyle() == IAction.AS_RADIO_BUTTON) && (propertyName == null || propertyName.equals(IAction.CHECKED));
                ...
                if (tooltipTextChanged || textChanged) {
                    ...
                    if ((callback != null) && (commandId != null) && (toolTip != null)) {
                        ...
                        if (acceleratorText != null && acceleratorText.length() != 0) {
                            toolTip = JFaceResources.format(//$NON-NLS-1$
                            "Toolbar_Tooltip_Accelerator", toolTip, acceleratorText);
                        }
                    }
                    // different
                        ...
                        for (int i = 0; i < triggerSequences.length; i++) {
                            ...
                            if (triggers.length == 1) {
                                ...
                                if (trigger instanceof KeyStroke) {
                                    ...
                                    if ((currentKeyStroke.getModifierKeys() == (lookup.getCtrl() | lookup.getShift())) && ((currentNaturalKey >= '0' && currentNaturalKey <= '9') || (currentNaturalKey >= 'A' && currentNaturalKey <= 'F') || (currentNaturalKey == 'U'))) {
        ...
        final Listener passThrough =  event -> {
            if (!event.widget.isDisposed()) {
                ...
                if (!realItem.isDisposed()) {
                    ...
                    if (event.type == SWT.Selection && ((style & (SWT.TOGGLE | SWT.CHECK | SWT.RADIO)) != 0) && realItem instanceof MenuItem) {
