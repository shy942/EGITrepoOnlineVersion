...
*/
    ...
    public ViewerSorter findSorterForParent(Object aParent) {
        CommonSorterDescriptor[] descriptors = CommonSorterDescriptorManager.getInstance().findApplicableSorters(contentService, aParent);
    ...
    public Map findAvailableSorters(INavigatorContentDescriptor theSource) {
        ...
        for (int i = 0; i < descriptors.length; i++) {
            if (descriptors[i].getId() != null && descriptors[i].getId().length() > 0)
                ...
                //$NON-NLS-1$
                sorters.put(theSource.getId() + ".sorter." + (++count), getSorter(descriptors[i]));
        }
        return sorters;
    }
