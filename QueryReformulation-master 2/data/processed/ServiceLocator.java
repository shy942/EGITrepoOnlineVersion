...
*/
    ...
    public final void dispose() {
        ...
        if (servicesToDispose.size() > 0) {
            ...
            if (servicesToDispose.size() > 0) {
                WorkbenchPlugin.log(StatusUtil.newStatus(IStatus.WARNING, String.format(//$NON-NLS-1$
                "Services: %s register themselves while disposing (skipping dispose of such services).", servicesToDispose), null));
            }
        }
...
*/
    ...
    private void registerService(Class<?> api, Object service, boolean saveInContext) {
        ...
        if (!api.isInstance(service)) {
            ...
            "The service does not implement the given interface");
        }
