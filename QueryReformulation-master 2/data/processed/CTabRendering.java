...
public class CTabRendering extends CTabFolderRenderer implements ICTabRendering {
        ...
        switch(part) {
            case PART_BODY:
                if (state == SWT.FILL) {
                    ...
                    y = onBottom ? y - paddingTop - marginHeight - borderTop - (cornerSize / 4) : y - paddingTop - marginHeight - tabHeight - borderTop - (cornerSize / 4);
                    ...
                    if (parent.getMinimized()) {
                        ...
                        // tabHeight
                        // - borderTop;
                        y = onBottom ? y - marginHeight - borderTop - (cornerSize / 4) : y - marginHeight - tabHeight - borderTop - (cornerSize / 4);
    ...
    void drawSelectedTab(int itemIndex, GC gc, Rectangle bounds, int state) {
        ...
        if (selectedTabFillColors.length == 1) {
            ...
            backgroundPattern = new Pattern(gc.getDevice(), 0, 0, 0, bounds.height + 1, selectedTabFillColors[0], selectedTabFillColors[1]);
    ...
    void drawUnselectedTab(int itemIndex, GC gc, Rectangle bounds, int state) {
        if ((state & SWT.HOT) != 0) {
            ...
            gc.setClipping(points[0], onBottom ? bounds.y - header : bounds.y, parent.getSize().x - (shadowEnabled ? SIDE_DROP_WIDTH : 0 + INNER_KEYLINE + OUTER_KEYLINE), bounds.y + bounds.height);
    ...
    private void drawUnselectedTabBackground(GC gc, Rectangle partHeaderBounds, int state, boolean vertical, Color defaultBackground) {
        if (unselectedTabsColors == null) {
            boolean selected = (state & SWT.SELECTED) != 0;
            unselectedTabsColors = selected ? parentWrapper.getSelectionGradientColors() : parentWrapper.getGradientColors();
            unselectedTabsPercents = selected ? parentWrapper.getSelectionGradientPercents() : parentWrapper.getGradientPercents();
        }
        if (unselectedTabsColors == null) {
            unselectedTabsColors = new Color[] { gc.getDevice().getSystemColor(SWT.COLOR_WHITE) };
            unselectedTabsPercents = new int[] { 100 };
        }
        rendererWrapper.drawBackground(gc, partHeaderBounds.x, partHeaderBounds.y - 1, partHeaderBounds.width, partHeaderBounds.height, defaultBackground, unselectedTabsColors, unselectedTabsPercents, vertical);
    }
    ...
    private void drawTabBackground(GC gc, Rectangle partHeaderBounds, int state, boolean vertical, Color defaultBackground) {
        ...
        rendererWrapper.drawBackground(gc, partHeaderBounds.x, partHeaderBounds.height - 1, partHeaderBounds.width, parent.getBounds().height, defaultBackground, colors, percents, vertical);
    }
    ...
    private static class CTabFolderRendererWrapper extends ReflectionSupport<CTabFolderRenderer> {
        ...
        public void drawBackground(GC gc, int x, int y, int width, int height, Color defaultBackground, Color[] colors, int[] percents, boolean vertical) {
            if (drawBackgroundMethod == null) {
                drawBackgroundMethod = getMethod(//$NON-NLS-1$
                "drawBackground", new Class<?>[] { GC.class, int[].class, int.class, int.class, int.class, int.class, Color.class, Image.class, Color[].class, int[].class, boolean.class });
            }
            executeMethod(drawBackgroundMethod, new Object[] { gc, null, x, y, width, height, defaultBackground, null, colors, percents, vertical });
        }
    }
    ...
    private static class CTabFolderWrapper extends ReflectionSupport<CTabFolder> {
        ...
        public boolean isSelectionGradientVertical() {
            if (selectionGradientVerticalField == null) {
                //$NON-NLS-1$
                selectionGradientVerticalField = getField("selectionGradientVertical");
            }
        ...
        public boolean isGradientVertical() {
            if (gradientVerticalField == null) {
                //$NON-NLS-1$
                gradientVerticalField = getField("gradientVertical");
            }
        ...
        public Color[] getSelectionGradientColors() {
            if (selectionGradientColorsField == null) {
                //$NON-NLS-1$
                selectionGradientColorsField = getField("selectionGradientColorsField");
            }
        ...
        public int[] getSelectionGradientPercents() {
            if (selectionGradientPercentsField == null) {
                //$NON-NLS-1$
                selectionGradientPercentsField = getField("selectionGradientPercents");
            }
        ...
        public Color[] getGradientColors() {
            if (gradientColorsField == null) {
                //$NON-NLS-1$
                gradientColorsField = getField("gradientColors");
            }
        ...
        public int[] getGradientPercents() {
            if (gradientPercentsField == null) {
                //$NON-NLS-1$
                gradientPercentsField = getField("gradientPercents");
            }
