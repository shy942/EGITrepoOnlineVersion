...
*/
    ...
    private CTabFolder2Adapter CTFButtonListener = new CTabFolder2Adapter() {
        ...
        private MUIElement getElementToChange(CTabFolderEvent event) {
            ...
            while (parentElement != null && !(parentElement instanceof MArea)) parentElement = parentElement.getParent();
    ...
    private void subscribeTopicWidget(@UIEventTopic(UIEvents.UIElement.TOPIC_WIDGET) Event event) {
        ...
        ctf.removeCTabFolder2Listener(CTFButtonListener);
        ctf.addCTabFolder2Listener(CTFButtonListener);
        // Prevent multiple instances
        ctf.removeMouseListener(CTFDblClickListener);
...
*/
    ...
    private void subscribeTopicPerspReset(@UIEventTopic(UIEvents.UILifeCycle.PERSPECTIVE_RESET) Event event) {
        ...
        List<MUIElement> minimizedElements = modelService.findElements(resetPersp, null, MUIElement.class, Arrays.asList(IPresentationEngine.MINIMIZED));
...
*/
    ...
    private void subscribeTopicPerspOpened(@UIEventTopic(UIEvents.UILifeCycle.PERSPECTIVE_OPENED) Event event) {
        ...
        // Find any minimized stacks and show their trim
        List<MUIElement> minimizedElements = modelService.findElements(openedPersp, null, MUIElement.class, Arrays.asList(IPresentationEngine.MINIMIZED));
        for (MUIElement element : minimizedElements) {
            createTrim(element);
        }
    }
...
*/
    ...
    void restore(MUIElement element) {
        MWindow window = modelService.getTopLevelWindowFor(element);
        String trimId = TrimStackIdHelper.createTrimStackId(element, modelService.getPerspectiveFor(element), getWindowFor(element));
        ...
        List<MUIElement> curMax = modelService.findElements(window, null, MUIElement.class, maximizeTag, EModelService.PRESENTATION);
        if (curMax.size() > 0) {
...
*/
    private List<MUIElement> getElementsToMinimize(MUIElement element) {
        ...
        if ((loc & EModelService.OUTSIDE_PERSPECTIVE) != 0) {
            ...
            List<MPartStack> partStacksToMinimize = findValidElementsToMinimize(element, win, win, null, MPartStack.class, EModelService.OUTSIDE_PERSPECTIVE, false);
            ...
            List<MPlaceholder> placeholderToMinimize = findValidElementsToMinimize(element, win, win, ID_EDITOR_AREA, MPlaceholder.class, EModelService.OUTSIDE_PERSPECTIVE, true);
            elementsToMinimize.addAll(placeholderToMinimize);
        } else {
            List<MPartStack> partStacksToMinimize = findValidElementsToMinimize(element, win, persp == null ? win : persp, null, MPartStack.class, EModelService.PRESENTATION, false);
            elementsToMinimize.addAll(partStacksToMinimize);
            ...
            List<MPlaceholder> standaloneViews = modelService.findElements(persp == null ? win : persp, null, MPlaceholder.class, standaloneTag, EModelService.PRESENTATION);
            ...
            // Find the editor 'area'
            List<MPlaceholder> placeholderToMinimize = findValidElementsToMinimize(element, win, win, ID_EDITOR_AREA, MPlaceholder.class, EModelService.PRESENTATION, true);
            elementsToMinimize.addAll(placeholderToMinimize);
        }
...
*/
    private MWindow getWindowFor(MUIElement element) {
        ...
        while (parent != null && !(parent instanceof MWindow)) parent = parent.getParent();
