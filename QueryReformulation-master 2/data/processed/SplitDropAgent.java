...
*/
    ...
    public boolean canDrop(MUIElement dragElement, DnDInfo info) {
        ...
        if (dropStack == null) {
            if (!(info.curElement instanceof MStackElement) && !dndManager.getModelService().isLastEditorStack(info.curElement))
                return false;
            // Detect placeholders
    ...
    public void dragEnter(MUIElement dragElement, DnDInfo info) {
        ...
        if (outerRelTo instanceof MPartSashContainer) {
            while (outerRelTo != null && !(outerRelTo.getWidget() instanceof Composite)) outerRelTo = outerRelTo.getParent();
        }
        ...
        if (outerRelTo instanceof MArea) {
            ...
            if (relToParent instanceof MArea) {
                ...
                // PerspectiveStack
                outerRelTo = relToParent.getParent();
            } else {
                outerRelTo = null;
            }
        } else {
...
*/
    private void showFeedback(int location) {
        ...
        clearFeedback();
        feedback = new SplitFeedbackOverlay(dropCTF.getShell(), feedbackBounds, side, pct, getEnclosed(), getModified());
