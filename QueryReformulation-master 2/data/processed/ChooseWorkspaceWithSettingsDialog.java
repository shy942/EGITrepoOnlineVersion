...
*/
    private void createSettingsControls(Composite workArea) {
        ...
        final ExpandableComposite expandable = toolkit.createExpandableComposite(form.getBody(), ExpandableComposite.TWISTIE);
...
*/
    private boolean createButtons(FormToolkit toolkit, Composite sectionClient) {
        ...
        for (int i = 0; i < settings.length; i++) {
            ...
            final Button button = toolkit.createButton(sectionClient, settings[i].getAttribute(ATT_NAME), SWT.CHECK);
    ...
    private IStatus transferSettings(final IConfigurationElement element, final IPath path) {
        ...
        SafeRunner.run(new ISafeRunnable() {
            ...
            public void run() throws Exception {
                try {
                    SettingsTransfer transfer = (SettingsTransfer) WorkbenchPlugin.createExtension(element, ATT_CLASS);
            ...
            public void handleException(Throwable exception) {
                exceptions[0] = StatusUtil.newStatus(IStatus.ERROR, NLS.bind(IDEWorkbenchMessages.ChooseWorkspaceWithSettingsDialog_ClassCreationFailed, element.getAttribute(ATT_CLASS)), exception);
            }
        });
