...
* @since 3.2
*/
    private static final String unescape(final String escapedText) throws SerializationException {
                ...
                if (++i < escapedText.length()) {
                    ...
                    switch(c) {
                        ...
                        case ESCAPE_CHAR:
                            ...
                            break;
                        default:
                            throw new SerializationException(//$NON-NLS-1$ //$NON-NLS-2$
                            "Invalid character '" + c + "' in escape sequence");
                    }
                } else {
                    throw new SerializationException(//$NON-NLS-1$
                    "Unexpected termination of escape sequence");
                }
            }
        }
        if (buffer == null) {
...
*/
    public final ParameterizedCommand deserialize(final String serializedParameterizedCommand) throws NotDefinedException, SerializationException {
        final int lparenPosition = unescapedIndexOf(serializedParameterizedCommand, PARAMETER_START_CHAR);
        ...
        if (lparenPosition == -1) {
            ...
            if (serializedParameterizedCommand.charAt(serializedParameterizedCommand.length() - 1) != PARAMETER_END_CHAR) {
                throw new SerializationException(//$NON-NLS-1$
                "Parentheses must be balanced in serialized ParameterizedCommand");
            }
            serializedParameters = serializedParameterizedCommand.substring(// skip PARAMETER_START_CHAR
            lparenPosition + 1, // skip
            serializedParameterizedCommand.length() - 1);
        // PARAMETER_END_CHAR
        }
        ...
        final Parameterization[] parameterizations = getParameterizations(serializedParameters, parameters);
...
*/
    public final String getHelpContextId(final Command command) throws NotDefinedException {
        ...
        if (!command.isDefined()) {
            ...
            "The command is not defined. " + command.getId());
        }
        // Check the handler.
...
* @since 3.2
*/
    private final Parameterization[] getParameterizations(String serializedParameters, final IParameter[] parameters) throws SerializationException {
        ...
        // split off each param by looking for ','
        int commaPosition;
        do {
