...
*/
    public  TableWrapData(int align, int valign, int rowspan, int colspan) {
        if (align != LEFT && align != CENTER && align != RIGHT && align != FILL && align != FILL_GRAB)
            //$NON-NLS-1$
            SWT.error(SWT.ERROR_INVALID_ARGUMENT, null, " \"align\"");
        if (valign != TOP && valign != MIDDLE && valign != BOTTOM && valign != FILL && valign != FILL_GRAB)
            //$NON-NLS-1$
            SWT.error(SWT.ERROR_INVALID_ARGUMENT, null, " \"valign\"");
        if (rowspan < 1)
            //$NON-NLS-1$
            SWT.error(SWT.ERROR_INVALID_ARGUMENT, null, " \"rowspan\"");
        if (colspan < 1)
            //$NON-NLS-1$
            SWT.error(SWT.ERROR_INVALID_ARGUMENT, null, " \"colspan\"");
        if (align == FILL_GRAB) {
            this.align = FILL;
            grabHorizontal = true;
        } else
