...
*/
    ...
    public void setModel(MItem item) {
        ...
        if (getModel().getCommand() == null) {
            if (logger != null) {
                //$NON-NLS-1$ //$NON-NLS-2$
                logger.error("Element " + getModel().getElementId() + " invalid, no command defined.");
            }
        }
    }
...
*/
    private void generateCommand() {
        if (getModel().getCommand() != null && getModel().getWbCommand() == null) {
            ...
            if (cmdId == null) {
                ...
                "Unable to generate parameterized command for " + getModel() + //$NON-NLS-1$
                ". ElementId is not allowed to be null.");
                return;
            }
            ...
            //$NON-NLS-1$
            Activator.trace(Policy.DEBUG_MENUS, "command: " + parmCmd, null);
            if (parmCmd == null) {
                Activator.log(IStatus.ERROR, //$NON-NLS-1$
                "Unable to generate parameterized command for " + getModel() + " with " + //$NON-NLS-1$
                parameters);
                return;
            }
    ...
    private IEclipseContext getStaticContext(Event event) {
        if (infoContext == null) {
            ...
            ContributionsAnalyzer.populateModelInterfaces(getModel(), infoContext, getModel().getClass().getInterfaces());
        }
        if (event == null) {
