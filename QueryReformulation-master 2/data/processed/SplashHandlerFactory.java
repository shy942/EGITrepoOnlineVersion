    ...
    public static AbstractSplashHandler findSplashHandlerFor(IProduct product) {
        ...
        for (int i = 0; i < extensions.length; i++) {
            ...
            for (int j = 0; j < children.length; j++) {
                AbstractSplashHandler handler = processElement(children[j], idToSplash, targetId, product);
                if (handler != null)
                    return handler;
            }
        }
    ...
    private static AbstractSplashHandler create(final IConfigurationElement splashElement) {
        ...
        SafeRunner.run(new SafeRunnable() {
            ...
            public void handleException(Throwable e) {
                WorkbenchPlugin.log("Problem creating splash implementation", //$NON-NLS-1$
                e);
            }
        });
        return handler[0];
