...
*/
    protected PropertySheet findPropertySheet(ExecutionEvent event, PropertyShowInContext context) throws PartInitException, ExecutionException {
        IWorkbenchPage page = HandlerUtil.getActivePartChecked(event).getSite().getPage();
        ...
        if (HandlerUtil.getActivePart(event) instanceof PropertySheet) {
            ...
            for (IViewReference viewReference : refs) {
                if (IPageLayout.ID_PROP_SHEET.equals(viewReference.getId())) {
                    ...
                    if (!sheet.isPinned() || (sheet.isPinned() && sheet.getShowInContext().equals(context))) {
        ...
        return (PropertySheet) page.showView(IPageLayout.ID_PROP_SHEET, secondaryId, IWorkbenchPage.VIEW_ACTIVATE);
    }
}
