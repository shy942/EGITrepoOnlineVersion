...
*/
    public  LegacyContextListenerWrapper(final org.eclipse.ui.contexts.IContextListener listener, final ContextManager contextManager, final IContext context) {
        if (listener == null) {
            throw new NullPointerException(//$NON-NLS-1$
            "Cannot create a listener wrapper on a null listener");
        }
        if (contextManager == null) {
            throw new NullPointerException(//$NON-NLS-1$
            "Cannot create a listener wrapper with a null context manager");
        }
        if (context == null) {
            throw new NullPointerException(//$NON-NLS-1$
            "Cannot create a listener wrapper with a null context");
        }
    ...
    public final void contextManagerChanged(final ContextManagerEvent event) {
        ...
        if (event.isActiveContextsChanged()) {
            ...
            if ((previousContexts != null) && (previousContexts.contains(contextId)) && ((currentContexts == null) || (currentContexts.contains(contextId)))) {
                enabledChanged = true;
            } else if ((currentContexts != null) && (currentContexts.contains(contextId)) && ((previousContexts == null) || (previousContexts.contains(contextId)))) {
                enabledChanged = true;
            } else {
        ...
        wrappedListener.contextChanged(new org.eclipse.ui.contexts.ContextEvent(context, false, enabledChanged, false, false));
    }
