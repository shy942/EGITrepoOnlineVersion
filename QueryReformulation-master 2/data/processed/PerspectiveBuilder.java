...
public class PerspectiveBuilder {
    ...
    private void addTrimBars() {
        ...
        //$NON-NLS-1$
        perspective.getPersistedState().put("trims", sb.toString());
    }
    ...
    private MPartStack addDefaultFastViewStack() {
        ...
        if (views.size() > 0) {
            ...
            List<MPartSashContainer> list = modelService.findElements(perspective, null, MPartSashContainer.class, null);
    ...
    private static IExtension[] getPerspectiveExtensions() {
        IExtensionPoint point = Platform.getExtensionRegistry().getExtensionPoint(PlatformUI.PLUGIN_ID, IWorkbenchRegistryConstants.PL_PERSPECTIVE_EXTENSIONS);
        if (point == null) {
            return null;
        }
    ...
    private static ArrayList<String> getExtensionShowInPartFromRegistry(IExtension extension, String targetId) {
        ...
        for (int j = 0; j < configElements.length; j++) {
            ...
            if (type.equals(IWorkbenchRegistryConstants.TAG_PERSPECTIVE_EXTENSION)) {
                ...
                if (targetId.equals(id) || "*".equals(id)) {
                    //$NON-NLS-1$
