...
public class WorkbenchSiteProgressServiceTest extends UITestCase {
    ...
    public void testWaitCursorConcurrentJobs() throws Exception {
        ...
        try {
            ...
            Cursor cursor = ((Control) ((PartSite) site).getModel().getWidget()).getCursor();
            logTime("after getCursor:  ");
            // jobWithoutCursor is scheduled to run first -
            assertNull(cursor);
            // no cursor now
            while (jobWithCursor.getState() != Job.RUNNING) {
                Thread.sleep(100);
            ...
            logTime("after getCursor2: ");
            // both running now - cursor should be set
            assertNotNull(cursor);
        } finally {
            jobWithCursor.cancel();
            logTime("after cancel:     ");
            ...
            while (jobWithCursor.getState() == Job.RUNNING || jobWithoutCursor.getState() == Job.RUNNING) {
                Thread.sleep(100);
            }
            logTime("after done:       ");
            processEvents();
