...
*/
    public static MessageDialogWithToggle open(int kind, Shell parent, String title, String message, String toggleMessage, boolean toggleState, IPreferenceStore store, String key, int style) {
        MessageDialogWithToggle dialog = new MessageDialogWithToggle(parent, // accept the default window icon
        title, // accept the default window icon
        null, message, kind, getButtonLabels(kind), 0, toggleMessage, toggleState);
        style &= SWT.SHEET;
...
*/
    protected void setToggleButton(Button button) {
        if (button == null) {
            ...
            "A message dialog with toggle may not have a null toggle button.");
        }
        if (!button.isDisposed()) {
            final String text;
            if (toggleMessage == null) {
                text = JFaceResources.getString(//$NON-NLS-1$
                "MessageDialogWithToggle.defaultToggleMessage");
            } else {
                text = toggleMessage;
            }
            button.setText(text);
