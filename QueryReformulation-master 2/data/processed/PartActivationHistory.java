...
*/
    private MArea isInArea(MUIElement element) {
        ...
        if (placeholder == null) {
            ...
            return parent instanceof MApplication ? null : parent instanceof MArea ? (MArea) parent : isInArea(parent);
        }
...
*/
    ...
    MPart getNextActivationCandidate(Collection<MPart> validParts, MPart part) {
        ...
        if (area != null) {
            ...
            candidate = findActivationCandidate(modelService.findElements(area, null, MPart.class, null), part);
            if (candidate != null) {
                return candidate;
            }
        }
    ...
    void forget(MWindow window, MPart part, boolean full) {
        if (full) {
            ...
            for (MPlaceholder placeholder : modelService.findElements(window, null, MPlaceholder.class, null)) {
    ...
    MPart getActivationCandidate(MPerspective perspective) {
        for (MPart candidate : generalActivationHistory) {
            if (partService.isInContainer(perspective, candidate) && isValid(perspective, candidate)) {
                return candidate;
            }
        }
        List<String> activeTag = new ArrayList();
        activeTag.add(EPartService.ACTIVE_ON_CLOSE_TAG);
        List<MPart> activeCandidates = modelService.findElements(perspective, null, MPart.class, activeTag);
    ...
    private MPart getSiblingActivationCandidate(MPart part) {
        ...
        MUIElement candidate = getSiblingSelectionCandidate(part, placeholder == null ? part : placeholder);
    ...
    private MUIElement getSiblingSelectionCandidate(MPart part, MUIElement element) {
        ...
        for (MPart previouslyActivatedPart : generalActivationHistory) {
            if (previouslyActivatedPart != part && isValid(previouslyActivatedPart)) {
                ...
                if (placeholder != null && placeholder.isToBeRendered() && siblings.contains(placeholder)) {
                    return placeholder;
                }
            }
        }
