...
*
*/
    private class PrivateInterface implements Runnable, IChangeListener, IStaleListener {
    ...
    final List<E> doGetList() {
        if (dirty) {
            ...
            // dependencies list
            IObservable[] newDependencies = ObservableTracker.runAndMonitor(privateInterface, privateInterface, null);
        ...
        if (!dirty) {
            ...
            fireListChange(new ListDiff<E>() {
                ...
                public ListDiffEntry<E>[] getDifferences() {
                    if (differences == null)
                        return Diffs.computeListDiff(oldList, getList()).getDifferences();
                    return differences.toArray(new ListDiffEntry[differences.size()]);
                }
            });
        }
    }
