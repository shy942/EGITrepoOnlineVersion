...
public abstract class HeadlessApplicationTest extends HeadlessApplicationElementTest {
    ...
    private void processPartContributions(IEclipseContext context, Resource resource) {
        ...
        for (int i = 0; i < parts.length; i++) {
            ...
            part.setIconURI(//$NON-NLS-1$
            "platform:/plugin/" + parts[i].getContributor().getName() + //$NON-NLS-1$
            "/" + //$NON-NLS-1$
            parts[i].getAttribute("icon"));
            part.setContributionURI(//$NON-NLS-1$
            "bundleclass://" + parts[i].getContributor().getName() + //$NON-NLS-1$
            "/" + //$NON-NLS-1$
