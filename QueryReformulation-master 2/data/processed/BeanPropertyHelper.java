    ...
    public static void writeProperty(Object source, PropertyDescriptor propertyDescriptor, Object value) {
        try {
            ...
            if (null == writeMethod) {
                ...
                "Missing public setter method for " + propertyDescriptor.getName() + //$NON-NLS-1$
                " property");
            }
            ...
            Policy.getLog().log(new Status(IStatus.WARNING, Policy.JFACE_DATABINDING, IStatus.OK, "Could not change value of " + source + "." + propertyDescriptor.getName(), e));
        }
    }
...
*/
    public static Object readProperty(Object source, PropertyDescriptor propertyDescriptor) {
        try {
            ...
            if (readMethod == null) {
                throw new IllegalArgumentException(propertyDescriptor.getName() + //$NON-NLS-1$
                " property does not have a read method.");
            }
            ...
            Policy.getLog().log(new Status(IStatus.WARNING, Policy.JFACE_DATABINDING, IStatus.OK, "Could not read value of " + source + "." + propertyDescriptor.getName(), e));
            return null;
        }
    }
    ...
    public static PropertyDescriptor getPropertyDescriptor(Class<?> beanClass, String propertyName) {
        if (!beanClass.isInterface()) {
            ...
            try {
                ...
                if (pds.size() > 0) {
                    propertyDescriptors = pds.toArray(new PropertyDescriptor[pds.size()]);
        ...
        "Could not find property with name " + propertyName + " in class " + beanClass);
    }
