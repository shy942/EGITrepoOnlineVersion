        ...
        for (IOperationApprover approver : approvers) {
            ...
            if (!approval.isOK()) {
                if (DEBUG_OPERATION_HISTORY_APPROVAL) {
                    ...
                    "Redo not approved by " + approver + "for operation " + //$NON-NLS-1$
                    operation + " approved by " + //$NON-NLS-1$
                    approval);
                }
                return approval;
            }
        ...
        for (IOperationApprover approver : approvers) {
            ...
            if (!approval.isOK()) {
                if (DEBUG_OPERATION_HISTORY_APPROVAL) {
                    ...
                    Tracing.printTrace(//$NON-NLS-1$
                    "OPERATIONHISTORY", //$NON-NLS-1$
                    "Undo not approved by " + approver + "for operation " + //$NON-NLS-1$
                    operation + " with status " + //$NON-NLS-1$
                    approval);
                }
                return approval;
            }
            ...
            if (tmp instanceof IOperationApprover2) {
                ...
                if (!approval.isOK()) {
                    if (DEBUG_OPERATION_HISTORY_APPROVAL) {
                        ...
                        Tracing.printTrace(//$NON-NLS-1$
                        "OPERATIONHISTORY", //$NON-NLS-1$
                        "Execute not approved by " + approver + "for operation " + //$NON-NLS-1$
                        operation + " with status " + //$NON-NLS-1$
                        approval);
                    }
                    return approval;
            ...
            SafeRunner.run(new ISafeRunnable() {
                ...
                public void handleException(Throwable exception) {
                    if (DEBUG_OPERATION_HISTORY_UNEXPECTED) {
                        ...
                        Tracing.printTrace(//$NON-NLS-1$
                        "OPERATIONHISTORY", //$NON-NLS-1$
                        "Exception during notification callback " + exception);
                    }
                }
        ...
        synchronized (openCompositeLock) {
            if (openComposite != null && openComposite != operation) {
                ...
                if (DEBUG_OPERATION_HISTORY_UNEXPECTED) {
                    ...
                    Tracing.printTrace(//$NON-NLS-1$
                    "OPERATIONHISTORY", //$NON-NLS-1$
                    "Open operation called while another operation is open.  old: " + openComposite + "; new:  " + //$NON-NLS-1$
                    operation);
                }
                throw new IllegalStateException(//$NON-NLS-1$
                "Cannot open an operation while one is already open");
            }
            openComposite = operation;
        }
        if (DEBUG_OPERATION_HISTORY_OPENOPERATION) {
            //$NON-NLS-1$ //$NON-NLS-2$
            Tracing.printTrace("OPERATIONHISTORY", "Opening operation " + openComposite);
        }
        if (mode == EXECUTE) {
    ...
    public void closeOperation(boolean operationOK, boolean addToHistory, int mode) {
        ...
        synchronized (openCompositeLock) {
            ...
            if (openComposite != null) {
                if (DEBUG_OPERATION_HISTORY_OPENOPERATION) {
                    //$NON-NLS-1$ //$NON-NLS-2$
                    Tracing.printTrace("OPERATIONHISTORY", "Closing operation " + openComposite);
                }
                endedComposite = openComposite;
                openComposite = null;
            }
