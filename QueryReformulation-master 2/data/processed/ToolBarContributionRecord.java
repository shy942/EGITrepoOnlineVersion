...
*/
    ...
    public boolean computeVisibility(HashSet<ToolBarContributionRecord> recentlyUpdated, MToolBarElement item, ExpressionContext exprContext) {
        ...
        if (currentVisibility && item.getVisibleWhen() instanceof MCoreExpression) {
            boolean val = ContributionsAnalyzer.isVisible((MCoreExpression) item.getVisibleWhen(), exprContext);
    ...
    public void collectInfo(ExpressionInfo info) {
        ContributionsAnalyzer.collectInfo(info, toolbarContribution.getVisibleWhen());
        for (MToolBarElement item : generatedElements) {
    ...
    private boolean requiresVisibilityCheck(MToolBarElement toolBarElement) {
        return toolBarElement.getVisibleWhen() != null || toolBarElement.getPersistedState().get(MenuManagerRenderer.VISIBILITY_IDENTIFIER) != null;
    }
    ...
    public boolean mergeIntoModel() {
        ...
        for (MToolBarElement copy : copyElements) {
            ...
            if (copy instanceof MToolBarSeparator) {
                MToolBarSeparator shared = findExistingSeparator(copy.getElementId());
                if (shared == null) {
            ...
            if (copy instanceof MToolBarSeparator) {
                ArrayList<ToolBarContributionRecord> array = renderer.getList(copy);
                array.add(this);
            }
        }
        return true;
...
*/
    private List<MToolBarElement> mergeFactoryIntoModel() {
        ...
        factoryDispose = (Runnable) ((IContextFunction) obj).compute(staticContext, null);
    ...
    private IEclipseContext getStaticContext() {
        if (infoContext == null) {
            ...
            ContributionsAnalyzer.populateModelInterfaces(toolbarModel, infoContext, toolbarModel.getClass().getInterfaces());
    ...
    MToolBarSeparator findExistingSeparator(String id) {
        ...
        for (MToolBarElement item : toolbarModel.getChildren()) {
            if (item instanceof MToolBarSeparator && id.equals(item.getElementId())) {
    ...
    public void dispose() {
        ...
        for (MToolBarElement shared : sharedElements) {
            ArrayList<ToolBarContributionRecord> array = renderer.getList(shared);
            array.remove(this);
            if (array.isEmpty()) {
        ...
        while (idx < size) {
            if (id.equals(model.getChildren().get(idx).getElementId())) {
                if ("after".equals(modifier)) {
                    ...
                    while (idx < size && !(model.getChildren().get(idx) instanceof MToolBarSeparator && model.getChildren().get(idx).getElementId() != null)) {
                        idx++;
                    }
                }
                return idx;
