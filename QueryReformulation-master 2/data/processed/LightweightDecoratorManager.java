...
*/
        ...
        public void handleException(Throwable exception) {
            ...
            if (decorator == null) {
                ...
                message = NLS.bind(WorkbenchMessages.DecoratorWillBeDisabled, decorator.getName());
            }
...
*/
    public boolean removeDecorator(LightweightDecoratorDefinition decorator) {
        ...
        if (idx != -1) {
            LightweightDecoratorDefinition[] oldDefs = lightweightDefinitions;
            Util.arrayCopyWithRemoval(oldDefs, lightweightDefinitions = new LightweightDecoratorDefinition[lightweightDefinitions.length - 1], idx);
            // no reset - handled in the DecoratorManager
            String[] types = getTargetTypes(decorator);
            for (int i = 0; i < types.length; i++) {
...
*/
    LightweightDecoratorDefinition[] getDecoratorsFor(Object element) {
        ...
        if (!contributors.isEmpty()) {
            Collection decorators = DecoratorManager.getDecoratorsFor(element, (DecoratorDefinition[]) contributors.toArray(new DecoratorDefinition[contributors.size()]));
