...
*/
    ...
    private static BindingManager loadModelBackend(IServiceLocator locator) {
        ...
        try {
            ...
            StatusManager.getManager().handle(new Status(IStatus.WARNING, WorkbenchPlugin.PI_WORKBENCH, "Keys page found an undefined scheme", e));
        }
        ...
        addPropertyChangeListener(new IPropertyChangeListener() {
            ...
            public void propertyChange(PropertyChangeEvent event) {
                if (BindingElement.PROP_TRIGGER.equals(event.getProperty())) {
                    updateTrigger((BindingElement) event.getSource(), (KeySequence) event.getOldValue(), (KeySequence) event.getNewValue());
                }
            }
        });
    }
    ...
    private void addSetModelObjectListener() {
        addPropertyChangeListener(new IPropertyChangeListener() {
            ...
            @Override
            public void propertyChange(PropertyChangeEvent event) {
                if (event.getSource() instanceof BindingElement && ModelElement.PROP_MODEL_OBJECT.equals(event.getProperty())) {
                    if (event.getNewValue() != null) {
                        ...
                        if (oldValue instanceof Binding && newValue instanceof Binding) {
                            conflictModel.updateConflictsFor(element, ((Binding) oldValue).getTriggerSequence(), ((Binding) newValue).getTriggerSequence(), false);
                        } else {
    ...
    public void exportCSV(Shell shell) {
        ...
        final SafeRunnable runnable = new SafeRunnable() {
            ...
            public final void run() throws IOException {
                ...
                try {
                    fileWriter = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(filePath), //$NON-NLS-1$
                    "UTF-8"));
