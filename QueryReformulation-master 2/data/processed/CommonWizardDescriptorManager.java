...
*/
    ...
    private void addCommonWizardDescriptor(CommonWizardDescriptor aDesc) {
        ...
        synchronized (commonWizardDescriptors) {
            Set<CommonWizardDescriptor> descriptors = commonWizardDescriptors.get(aDesc.getType());
            if (descriptors == null) {
...
*/
    public String[] getEnabledCommonWizardDescriptorIds(Object anElement, String aType, INavigatorContentService aContentService) {
        ...
        for (Iterator commonWizardDescriptorsItr = commonDescriptors.iterator(); commonWizardDescriptorsItr.hasNext(); ) {
            ...
            if (isVisible(aContentService, descriptor) && descriptor.isEnabledFor(anElement)) {
...
*/
    private boolean isVisible(INavigatorContentService aContentService, CommonWizardDescriptor descriptor) {
        return !WorkbenchActivityHelper.filterItem(descriptor) && (aContentService == null || (descriptor.getId() == null || (aContentService.isVisible(descriptor.getId()) && aContentService.isActive(descriptor.getId()))));
    }
            ...
            if (TAG_NAVIGATOR_CONTENT.equals(anElement.getName())) {
                ...
                for (int i = 0; i < commonWizards.length; i++) {
                    ...
                    SafeRunner.run(new NavigatorSafeRunnable(element) {
                        ...
                        @Override
                        public void run() throws Exception {
                            addCommonWizardDescriptor(new CommonWizardDescriptor(element, contentExtensionId));
                            retValue[0] = true;
                        }
                    });
                    if (!retValue[0])
