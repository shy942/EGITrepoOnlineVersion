...
*/
    ...
    static {
        JFaceResources.getImageRegistry().put(STOP_IMAGE_KEY, WorkbenchImages.getWorkbenchImageDescriptor(//$NON-NLS-1$
        "elcl16/progress_stop.png"));
        JFaceResources.getImageRegistry().put(DISABLED_STOP_IMAGE_KEY, WorkbenchImages.getWorkbenchImageDescriptor(//$NON-NLS-1$
        "dlcl16/progress_stop.png"));
        JFaceResources.getImageRegistry().put(DEFAULT_JOB_KEY, WorkbenchImages.getWorkbenchImageDescriptor(//$NON-NLS-1$
        "progress/progress_task.png"));
        JFaceResources.getImageRegistry().put(CLEAR_FINISHED_JOB_KEY, WorkbenchImages.getWorkbenchImageDescriptor(//$NON-NLS-1$
        "elcl16/progress_rem.png"));
        JFaceResources.getImageRegistry().put(DISABLED_CLEAR_FINISHED_JOB_KEY, WorkbenchImages.getWorkbenchImageDescriptor(//$NON-NLS-1$
        "dlcl16/progress_rem.png"));
...
*/
    public String getJobNameAndStatus(JobInfo jobInfo) {
        ...
        switch(job.getState()) {
            ...
            case // Only happens for kept jobs
            Job.NONE:
            ...
            if (taskInfo != null) {
                ...
                if (subTaskString != null) {
                    if (taskString == null || taskString.length() == 0) {
                        ...
                        taskString = NLS.bind(ProgressMessages.JobInfo_DoneNoProgressMessage, taskString, subTaskString);
                    }
                }
                if (taskString != null) {
                ...
                // Check for the finished job state
                Job job = jobInfo.getJob();
...
*/
    private void updateText(String taskString, Link link) {
        ...
        link.setText(link.getData(TRIGGER_KEY) == null ? taskString : NLS.bind("<a>{0}</a>", //$NON-NLS-1$
        taskString));
    }
