...
*/
            ...
            if (fontProperties != null) {
                ...
                if (widget instanceof CTabItem) {
                    ...
                    if (listener == null) {
                        ...
                        parent.setData(CSS_CTABITEM_SELECTED_FONT_LISTENER_KEY, listener);
                    } else {
                        // update our engine
    ...
    public void applyCSSPropertyFontFamily(Object element, CSSValue value, String pseudo, CSSEngine engine) throws Exception {
        ...
        CSS2FontProperties fontProperties = CSSSWTFontHelper.getCSS2FontProperties(widget, engine.getCSSElementContext(widget));
        if (fontProperties != null) {
            applyCSSPropertyFontFamily(fontProperties, value, pseudo, engine);
            return;
        }
    ...
    public void applyCSSPropertyFontSize(Object element, CSSValue value, String pseudo, CSSEngine engine) throws Exception {
        ...
        CSS2FontProperties fontProperties = CSSSWTFontHelper.getCSS2FontProperties(widget, engine.getCSSElementContext(widget));
        if (fontProperties != null) {
            applyCSSPropertyFontSize(fontProperties, value, pseudo, engine);
            return;
        }
    ...
    public void applyCSSPropertyFontWeight(Object element, CSSValue value, String pseudo, CSSEngine engine) throws Exception {
        ...
        CSS2FontProperties fontProperties = CSSSWTFontHelper.getCSS2FontProperties(widget, engine.getCSSElementContext(widget));
        if (fontProperties != null) {
            applyCSSPropertyFontWeight(fontProperties, value, pseudo, engine);
            return;
        }
    ...
    public void applyCSSPropertyFontStyle(Object element, CSSValue value, String pseudo, CSSEngine engine) throws Exception {
        ...
        CSS2FontProperties fontProperties = CSSSWTFontHelper.getCSS2FontProperties(widget, engine.getCSSElementContext(widget));
        if (fontProperties != null) {
            applyCSSPropertyFontStyle(fontProperties, value, pseudo, engine);
            return;
        }
...
*/
        private void applyStyles(CSSStyleDeclaration styleDeclaration, String pseudo, Item item) {
            ...
            CSS2FontProperties fontProperties = CSSSWTFontHelper.getCSS2FontProperties(item, engine.getCSSElementContext(item));
            ...
            if (fontProperties != null) {
                ...
                for (String fontAttribute : fontAttributes) {
                    CSSValue value = styleDeclaration.getPropertyCSSValue(fontAttribute);
                    if (value != null) {
                        try {
                            // we have a value, so apply it to the properties
                            CSSPropertyFontSWTHandler.super.applyCSSProperty(itemElement, fontAttribute, value, pseudo, engine);
                        } catch (Exception e) {
                ...
                try {
                    // set the font
                    Font font = (Font) engine.convert(fontProperties, Font.class, item);
                    setFont(item, font);
                } catch (Exception e) {
                    engine.handleExceptions(e);
                }
...
*/
        private void styleUnselected(Item[] items) {
            for (Item item : items) {
                CSSStyleDeclaration unselectedStyle = engine.getViewCSS().getComputedStyle(engine.getElement(item), null);
