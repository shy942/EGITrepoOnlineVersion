...
*/
     ContextAuthority(final ContextManager contextManager, final IContextService contextService) {
        if (contextManager == null) {
            ...
            "The context authority needs a context manager");
        }
        if (contextService == null) {
            throw new NullPointerException(//$NON-NLS-1$
            "The context authority needs an evaluation context");
        }
...
*            disposed.
*/
    private final void checkWindowType(final Shell newShell, final Shell oldShell) {
        /*
...
*/
        if ((newShell != null) && (!newShell.isDisposed())) {
            ...
            if ((newShell.getParent() != null) && (registeredWindows.get(newShell) == null)) {
                ...
                final IContextActivation dialogWindowActivation = new ContextActivation(IContextService.CONTEXT_ID_DIALOG_AND_WINDOW, expression, contextService);
                activateContext(dialogWindowActivation);
                newActivations.add(dialogWindowActivation);
                final IContextActivation dialogActivation = new ContextActivation(IContextService.CONTEXT_ID_DIALOG, expression, contextService);
        ...
        if (value instanceof Collection) {
            ...
            if (contextActivations.contains(activation)) {
                ...
                if (contextActivations.isEmpty()) {
                    ...
                    final IContextActivation remainingActivation = (IContextActivation) contextActivations.iterator().next();
...
*/
    public final int getShellType(final Shell shell) {
        ...
        if (activations != null) {
            ...
            "A registered shell should have at least one submission matching TYPE_WINDOW or TYPE_DIALOG");
            // not reachable
            return IContextService.TYPE_NONE;
...
*/
    public final boolean registerShell(final Shell shell, final int type) {
        ...
        if (DEBUG) {
            //$NON-NLS-1$
            final StringBuffer buffer = new StringBuffer("register shell '");
            buffer.append(shell);
            //$NON-NLS-1$
            buffer.append("' as ");
            switch(type) {
        ...
        switch(type) {
            case IContextService.TYPE_DIALOG:
                ...
                dialogWindowActivation = new ContextActivation(IContextService.CONTEXT_ID_DIALOG_AND_WINDOW, expression, contextService);
                activateContext(dialogWindowActivation);
            ...
            case IContextService.TYPE_WINDOW:
                ...
                activateContext(dialogWindowActivation);
                activations.add(dialogWindowActivation);
                final IContextActivation windowActivation = new ContextActivation(IContextService.CONTEXT_ID_WINDOW, expression, contextService);
                activateContext(windowActivation);
                activations.add(windowActivation);
                break;
            default:
                throw new IllegalArgumentException(//$NON-NLS-1$
                "The type is not recognized: " + type);
        }
...
*/
        ...
        if (DEBUG_PERFORMANCE) {
            ...
            if (size > 0) {
                Tracing.printTrace(TRACING_COMPONENT, size + " activations recomputed in " + elapsedTime + //$NON-NLS-1$ //$NON-NLS-2$
                "ms");
            }
        }
    }
...
*            otherwise.
*/
    private final void updateContext(final String contextId, final boolean active) {
        if (active) {
...
*/
    @Override
    protected final void updateEvaluationContext(final String name, final Object value) {
        /*
