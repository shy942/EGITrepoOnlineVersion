...
*/
        ...
        for (final IPartListener listener : listeners) {
            SafeRunner.run(new ISafeRunnable() {
                ...
                public void handleException(Throwable throwable) {
                    //$NON-NLS-1$
                    logger.error(throwable, "An exception occurred while notifying part listeners");
                }
            });
        }
    }
    ...
    private <T> List<T> getParts(Class<T> cls, String id) {
        return modelService.findElements(workbenchWindow, id, cls, null, EModelService.OUTSIDE_PERSPECTIVE | EModelService.IN_ACTIVE_PERSPECTIVE | EModelService.IN_SHARED_AREA);
    }
    ...
    private boolean isMinimized(MUIElement elt) {
        ...
        return (tags.contains(IPresentationEngine.MINIMIZED) || tags.contains(IPresentationEngine.MINIMIZED_BY_ZOOM)) && !tags.contains(IPresentationEngine.ACTIVE);
    }
    ...
    private boolean isInActivePerspective(MUIElement element) {
        ...
        if (persp == null) {
            List<MUIElement> allPerspectiveElements = modelService.findElements(workbenchWindow, null, MUIElement.class, null, EModelService.PRESENTATION);
    ...
    public boolean isPartOrPlaceholderInPerspective(String elementId, MPerspective perspective) {
        ...
        if (!findElements.isEmpty()) {
            ...
            if (workbenchWindow.getSharedElements().contains(part)) {
                List<MPlaceholder> placeholders = modelService.findElements(perspective, elementId, MPlaceholder.class, null);
    ...
    public java.util.Optional<MPerspective> switchPerspective(String perspectiveId) {
        ...
        //$NON-NLS-1$ //$NON-NLS-2$
        logger.error("Perspective with ID " + perspectiveId + " not found in the current window.");
...
*/
    private MPart addPart(MPart providedPart, MPart localPart) {
        ...
        if (descriptor == null) {
            ...
            if (category == null) {
                ...
                if ("org.eclipse.e4.primaryDataStack".equals(category)) {
                    //$NON-NLS-1$
                    ...
                    @SuppressWarnings("rawtypes") List<MElementContainer> containers = modelService.findElements(getContainer(), null, MElementContainer.class, Collections.singletonList(category), EModelService.PRESENTATION);
    ...
    private void adjustPlaceholder(MPart part) {
        if (isShared(part)) {
            ...
            if (placeholder == null || // shifting that placeholder to the current container during the add operation
    ...
    private void addToLastContainer(String category, MPart part) {
        ...
        if (colonIndex >= 0) {
            ...
            //$NON-NLS-1$
            descId += ":*";
            List<MPlaceholder> phList = modelService.findElements(workbenchWindow, descId, MPlaceholder.class, null, EModelService.PRESENTATION);
            if (phList.size() > 0) {
        ...
        @SuppressWarnings("unchecked") MElementContainer<MUIElement> lastContainer = (MElementContainer<MUIElement>) getLastContainer();
        MPlaceholder placeholder = part.getCurSharedRef();
        if (placeholder == null) {
    ...
    private MElementContainer<? extends MUIElement> getLastContainer() {
        ...
        @SuppressWarnings("unchecked") List<MUIElement> children = (List<MUIElement>) searchRoot.getChildren();
        if (children.size() == 0) {
...
*/
    private MElementContainer<MUIElement> getParent(MUIElement element) {
        ...
        if (parent == null) {
            ...
            if (placeholder == null) {
                @SuppressWarnings("unchecked") MElementContainer<MUIElement> container = (MElementContainer<MUIElement>) getContainer();
        ...
        for (MUIElement child : container.getChildren()) {
            if (child == element) {
                ...
                if (ref == element) {
                    ...
                    @SuppressWarnings("unchecked") MElementContainer<MUIElement> ref2 = (MElementContainer<MUIElement>) ref;
                ...
                @SuppressWarnings("unchecked") MElementContainer<MUIElement> child2 = (MElementContainer<MUIElement>) child;
    ...
    public void hidePart(MPart part, boolean force) {
        ...
        //$NON-NLS-1$
        boolean needNewSel = thePersp == null || !thePersp.getTags().contains("PerspClosing");
        if (needNewSel) {
            if (parent.getSelectedElement() == toBeRemoved) {
                ...
                candidate = candidate == null ? null : candidate.getCurSharedRef() == null ? candidate : candidate.getCurSharedRef();
    ...
    public boolean savePart(MPart part, boolean confirm) {
        ...
        try {
            ...
            log("Failed to persist contents of part", "Failed to persist contents of part ({0})", part.getElementId(), e);
            return false;
        } catch (RuntimeException e) {
            log("Failed to persist contents of part via DI", "Failed to persist contents of part ({0}) via DI", part.getElementId(), e);
            return false;
        }
        return true;
    }
    ...
    public Collection<MInputPart> getInputParts(String inputUri) {
        //$NON-NLS-1$
        Assert.isNotNull(inputUri, "Input uri must not be null");
...
*/
    private MElementContainer<? extends MUIElement> getContainer() {
        MElementContainer<? extends MUIElement> outerContainer = (workbenchWindow != null) ? workbenchWindow : application;
