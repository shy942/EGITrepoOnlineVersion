...
*/
    ...
    private void writeSchemeToModel(Scheme activeScheme) {
        ...
        //$NON-NLS-1$
        tags.add(EBindingService.ACTIVE_SCHEME_TAG + ":" + activeScheme.getId());
    }
...
*/
    ...
    private static boolean isSameBinding(MKeyBinding existingBinding, MCommand cmd, Binding binding) {
        ...
        if (schemeId != null && !schemeId.equals(BindingPersistence.getDefaultSchemeId())) {
            if (//$NON-NLS-1$
            !modelTags.contains(EBindingService.SCHEME_ID_ATTR_TAG + ":" + schemeId))
                return false;
        }
        String locale = binding.getLocale();
        if (locale != null) {
            if (//$NON-NLS-1$
            !modelTags.contains(EBindingService.LOCALE_ATTR_TAG + ":" + locale))
                return false;
        }
        String platform = binding.getPlatform();
        if (platform != null) {
            if (//$NON-NLS-1$
            !modelTags.contains(EBindingService.PLATFORM_ATTR_TAG + ":" + platform))
                return false;
        }
        if (binding.getType() == Binding.USER) {
            if (//$NON-NLS-1$
            !modelTags.contains(EBindingService.TYPE_ATTR_TAG + ":user"))
                return false;
        }
        return true;
    }
    ...
    public static MKeyBinding createORupdateMKeyBinding(MApplication application, MBindingTable table, Binding binding) {
        ...
        if (keyBinding == null) {
            ...
            for (Object obj : parmCmd.getParameterMap().entrySet()) {
                @SuppressWarnings({ "unchecked" }) Map.Entry<String, String> entry = (Map.Entry<String, String>) obj;
            ...
            if (binding.getSchemeId() != null && !binding.getSchemeId().equals(BindingPersistence.getDefaultSchemeId())) {
                //$NON-NLS-1$
                tags.add(EBindingService.SCHEME_ID_ATTR_TAG + ":" + binding.getSchemeId());
            }
            if (binding.getLocale() != null) {
                //$NON-NLS-1$
                tags.add(EBindingService.LOCALE_ATTR_TAG + ":" + binding.getLocale());
            }
            if (binding.getPlatform() != null) {
                //$NON-NLS-1$
                tags.add(EBindingService.PLATFORM_ATTR_TAG + ":" + binding.getPlatform());
            }
            // just add the 'type' tag if it's a user binding
            if (binding.getType() == Binding.USER) {
                //$NON-NLS-1$
                tags.add(EBindingService.TYPE_ATTR_TAG + ":user");
            }
        }
    ...
    private MKeyBinding findMKeyBinding(MBindingTable table, Binding binding) {
        ...
        String bindingSchemeId = binding.getSchemeId() == null ? IBindingService.DEFAULT_DEFAULT_ACTIVE_SCHEME_ID : binding.getSchemeId();
        if (binding.getParameterizedCommand() != null) {
            ...
            for (MKeyBinding curr : mBindings) {
                ...
                // check equality
                if (curr.getKeySequence().equals(binding.getTriggerSequence().toString()) && curr.getCommand() != null && curr.getCommand().getElementId().equals(commandId)) {
...
*/
    public final void removeBinding(final Binding binding) {
        ...
        } else // if we're removing a system binding, then find the model binding, add
