...
public class WorkbenchStatusReporter extends StatusReporter {
    ...
    private void openDialog(final IStatus status, boolean shouldBlock, Object... information) {
        ...
        final Status exceptionStatus = new Status(status.getSeverity(), status.getPlugin(), status.getException() == null ? status.getMessage() : status.getException().toString(), status.getException());
        ...
        dialog = new ErrorDialog(myShell, WorkbenchSWTMessages.InternalError, status.getMessage(), status.getException() != null ? exceptionStatus : status, ERROR | WARNING | INFO) {
    ...
    protected String getPluginId() {
        return bundleContext == null ? "unknown" : bundleContext.getBundle().getSymbolicName();
    }
}
