...
public class AreaRenderer extends SWTPartRenderer {
    ...
    private void ensureCTF(MArea areaModel) {
        ...
        List<MPartStack> stacks = modelService.findElements(areaModel, null, MPartStack.class, null);
        MPartStack curStack = null;
        for (MPartStack stack : stacks) {
            if (stack.isToBeRendered() && stack.getWidget() instanceof CTabFolder) {
                CTabFolder stackCTF = (CTabFolder) stack.getWidget();
                if (stackCTF.getMinimizeVisible() || stackCTF.getMaximizeVisible()) {
                    curStack = stack;
                    break;
                }
            }
