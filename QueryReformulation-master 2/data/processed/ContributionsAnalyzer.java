...
public final class ContributionsAnalyzer {
    ...
    public static void XXXgatherToolBarContributions(final MToolBar toolbarModel, final List<MToolBarContribution> toolbarContributionList, final String id, final ArrayList<MToolBarContribution> toContribute) {
    ...
    public static void gatherToolBarContributions(final MToolBar toolbarModel, final List<MToolBarContribution> toolbarContributionList, final String id, final ArrayList<MToolBarContribution> toContribute, final ExpressionContext eContext) {
        if (id == null || id.length() == 0) {
            return;
        }
        for (MToolBarContribution toolBarContribution : toolbarContributionList) {
    ...
    public static void XXXgatherMenuContributions(final MMenu menuModel, final List<MMenuContribution> menuContributionList, final String id, final ArrayList<MMenuContribution> toContribute, final ExpressionContext eContext, boolean includePopups) {
        if (id == null || id.length() == 0) {
            return;
        }
    ...
    public static void gatherMenuContributions(final MMenu menuModel, final List<MMenuContribution> menuContributionList, final String id, final ArrayList<MMenuContribution> toContribute, final ExpressionContext eContext, boolean includePopups) {
        if (id == null || id.length() == 0) {
            return;
        }
    ...
    static boolean isFiltered(MMenu menuModel, MMenuContribution menuContribution, boolean includePopups) {
        if (includePopups || menuModel.getTags().contains(ContributionsAnalyzer.MC_POPUP)) {
            return !menuContribution.getTags().contains(ContributionsAnalyzer.MC_POPUP) && menuContribution.getTags().contains(ContributionsAnalyzer.MC_MENU);
        }
        if (menuModel.getTags().contains(ContributionsAnalyzer.MC_MENU)) {
            return !menuContribution.getTags().contains(ContributionsAnalyzer.MC_MENU) && menuContribution.getTags().contains(ContributionsAnalyzer.MC_POPUP);
        }
        if (!includePopups) {
            // menu
    ...
    public static void addMenuContributions(final MMenu menuModel, final ArrayList<MMenuContribution> toContribute, final ArrayList<MMenuElement> menuContributionsToRemove) {
    ...
    public static boolean processAddition(final MTrimBar trimBar, MTrimContribution contribution, List<MTrimElement> contributions, HashSet<String> existingToolbarIds) {
    ...
    static class Key {
        ...
        public boolean equals(Object obj) {
            ...
            return Util.equals(parentId, other.parentId) && Util.equals(position, other.position) && getSchemeTag() == other.getSchemeTag() && Util.equals(exp1, exp2) && Util.equals(factory, other.factory);
        }
        ...
        public String toString() {
            ...
            getClass().getName() + " " + parentId + "--" + position + "--" + getSchemeTag() + "--" + //$NON-NLS-1$//$NON-NLS-2$
            vexp;
        }
    }
    ...
    public static void mergeToolBarContributions(ArrayList<MToolBarContribution> contributions, ArrayList<MToolBarContribution> result) {
        ...
        //$NON-NLS-1$
        trace("mergeContributions size: " + contributions.size(), null);
        ...
        //$NON-NLS-1$
        trace("mergeContributions: final size: " + result.size(), null);
    }
    ...
    private static boolean containsMatching(List<MMenuElement> children, MMenuElement me) {
        for (MMenuElement element : children) {
            if (Util.equals(me.getElementId(), element.getElementId()) && element.getClass().isInstance(me) && (element instanceof MMenuSeparator || element instanceof MMenu)) {
                return true;
            }
        }
        return false;
    ...
    private static boolean containsMatching(List<MToolBarElement> children, MToolBarElement me) {
        for (MToolBarElement element : children) {
            if (Util.equals(me.getElementId(), element.getElementId()) && element.getClass().isInstance(me) && (element instanceof MToolBarSeparator || element instanceof MToolBar)) {
                return true;
            }
        }
        return false;
    ...
    public static void populateModelInterfaces(Object modelObject, IEclipseContext context, Class<?>[] interfaces) {
        for (Class<?> intf : interfaces) {
            Activator.trace(Policy.DEBUG_CONTEXTS, //$NON-NLS-1$ //$NON-NLS-2$
            "Adding " + intf.getName() + " for " + modelObject.getClass().getName(), null);
