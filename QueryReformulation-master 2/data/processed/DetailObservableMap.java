...
*/
    public  DetailObservableMap(IObservableFactory<? super M, IObservableMap<K, V>> detailFactory, IObservableValue<M> master, Object keyType, Object valueType) {
        ...
        //$NON-NLS-1$
        Assert.isTrue(!master.isDisposed(), "Master observable is disposed");
        this.master = master;
    ...
    private void updateDetailMap() {
        ...
        if (masterValue == null) {
            ...
            if (detailKeyType != null) {
                ...
                Assert.isTrue(detailKeyType.equals(innerKeyType), //$NON-NLS-1$
                "Cannot change key type in a nested observable map");
            }
            if (detailValueType != null) {
                Object innerValueType = detailMap.getValueType();
                Assert.isTrue(detailValueType.equals(innerValueType), //$NON-NLS-1$
                "Cannot change value type in a nested observable map");
            }
