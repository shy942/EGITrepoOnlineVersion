...
public class DynamicTestsView extends ViewPart implements ITabbedPropertySheetPageContributor {
    ...
    class DynamicTestsViewLabelProvider extends LabelProvider {
        ...
        public Image getImage(Object obj) {
            DynamicTestsElement element = ((DynamicTestsTreeNode) obj).getDynamicTestsElement();
...
*/
    private void initToolBar() {
        //$NON-NLS-1$
        final String staticText = "Use static contributions from plugin.xml";
        //$NON-NLS-1$
        final String dynamicSectionsText = "Use dynamic section contributions from code";
        //$NON-NLS-1$
        final String dynamicTabsText = "Use dynamic tab (and section) contributions from code";
        ImageDescriptor imageDescriptor = Activator.getImageDescriptor(//$NON-NLS-1$
        "icons/sample.gif");
        if (imageDescriptor == null) {
            imageDescriptor = PlatformUI.getWorkbench().getSharedImages().getImageDescriptor(ISharedImages.IMG_OBJS_ERROR_TSK);
        }
        staticAction = new Action(staticText, IAction.AS_CHECK_BOX) {
            ...
            public void run() {
                if (isChecked()) {
                    getViewSite().getActionBars().getStatusLineManager().setMessage(staticText);
                    dynamicSectionsAction.setChecked(false);
                    dynamicTabsAction.setChecked(false);
        ...
        dynamicSectionsAction = new Action(dynamicSectionsText, IAction.AS_CHECK_BOX) {

            @Override
            public void run() {
                if (isChecked()) {
                    getViewSite().getActionBars().getStatusLineManager().setMessage(dynamicSectionsText);
                    staticAction.setChecked(false);
                    dynamicTabsAction.setChecked(false);
                    viewer.setSelection(StructuredSelection.EMPTY);
                }
        ...
        dynamicTabsAction = new Action(dynamicTabsText, IAction.AS_CHECK_BOX) {
            ...
            public void run() {
                if (isChecked()) {
                    getViewSite().getActionBars().getStatusLineManager().setMessage(dynamicTabsText);
                    staticAction.setChecked(false);
                    dynamicSectionsAction.setChecked(false);
                    viewer.setSelection(StructuredSelection.EMPTY);
                }
