    ...
    private void subscribeTopicTagsChanged(@UIEventTopic(UIEvents.ApplicationElement.TOPIC_TAGS) Event event) {
        ...
        if (UIEvents.isADD(event)) {
            if (UIEvents.contains(event, UIEvents.EventTags.NEW_VALUE, IPresentationEngine.SPLIT_HORIZONTAL)) {
                splitPart(part, true);
            } else if (UIEvents.contains(event, UIEvents.EventTags.NEW_VALUE, IPresentationEngine.SPLIT_VERTICAL)) {
...
*/
    public static MCompositePart findContainingCompositePart(MPart part) {
        ...
        while (curParent != null && !(curParent instanceof MCompositePart)) curParent = curParent.getParent();
    ...
    private void unsplitPart(MCompositePart compositePart) {
        ...
        if (innerElements.size() < 3)
            return;
        // '0' is the composite part
        MPart originalEditor = innerElements.get(1);
    ...
    private MCompositePart createCompositePart(MPart originalPart) {
        ...
        //$NON-NLS-1$
        compPart.setContributionURI("bundleclass://org.eclipse.e4.ui.workbench.addons.swt/org.eclipse.e4.ui.workbench.addons.splitteraddon.SplitHost");
