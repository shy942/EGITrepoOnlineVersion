...
public class DnDTest extends NavigatorTestBase {
    ...
    public void testBasicDragDrop() {
        ...
        _viewer.setSelection(new StructuredSelection(_p1.getFolder("f1").getFile("file1.txt")));
        ...
        assertEquals(_p1.getFolder("f1").getFile("file2.txt"), items[_p1Ind].getItem(0).getItem(0).getData());
        assertEquals(_p1.getFolder("f2").getFile("file1.txt"), items[_p1Ind].getItem(1).getItem(0).getData());
    ...
    public void testSetDragOperation() {
        _contentService.bindExtensions(new String[] { TEST_CONTENT_DROP_COPY }, false);
        _contentService.getActivationService().activateExtensions(new String[] { TEST_CONTENT_DROP_COPY }, false);
        _viewer.expandToLevel(_p1, 3);
        // Need to set the selection because the Dnd stuff is not doing it
        _viewer.setSelection(new StructuredSelection(_p1.getFolder("f1").getFile("file1.txt")));
        ...
        assertEquals(_p1.getFolder("f1").getFile("file1.txt"), items[_p1Ind].getItem(firstFolder).getItem(0).getData());
        assertEquals(_p1.getFolder("f1").getFile("file2.txt"), items[_p1Ind].getItem(firstFolder).getItem(1).getData());
        // This line fails to see the firstFolder+1 unless all of that
        // refreshing crap above is in
        assertEquals(_p1.getFolder("f2").getFile("file1.txt"), items[_p1Ind].getItem(firstFolder + 1).getItem(0).getData());
