    ...
    private class ThemeContentProvider implements ITreeContentProvider {
        ...
        public Object[] getChildren(Object parentElement) {
            ...
            for (int i = 0; i < defs.length; i++) {
                if (id.equals(defs[i].getDefaultsTo()) && ColorsAndFontsPreferencePage.equals(((ICategorizedThemeElementDefinition) def).getCategoryId(), ((ICategorizedThemeElementDefinition) defs[i]).getCategoryId())) {
                    list.add(defs[i]);
                }
            }
            return list.toArray();
            ...
            }
            {
                ...
                for (int i = 0; i < colorDefinitions.length; i++) {
                    ...
                    if ((catId == null && categoryId == null) || (catId != null && categoryId != null && categoryId.equals(catId))) {
                        if (colorDefinitions[i].getDefaultsTo() != null && parentIsInSameCategory(colorDefinitions[i])) {
                            continue;
                        }
...
*/
        private boolean isDefault(IThemeElementDefinition def) {
            if (def instanceof FontDefinition) {
                ...
                return defaultFontID != null && Arrays.equals(fontRegistry.getFontData(def.getId()), fontRegistry.getFontData(defaultFontID));
            }
            if (def instanceof ColorDefinition) {
...
*/
    ...
    public void applyData(Object data) {
        ...
        } else if (command.startsWith("selectColor:")) {
            //$NON-NLS-1$
...
*/
    ...
    protected Control createContents(Composite parent) {
        ...
        //$NON-NLS-1$
        fontChangeButton = createButton(controlColumn, RESOURCE_BUNDLE.getString("openChange"));
        fontSystemButton = createButton(controlColumn, WorkbenchMessages.FontsPreference_useSystemFont);
        //$NON-NLS-1$
        fontResetButton = createButton(controlColumn, RESOURCE_BUNDLE.getString("reset"));
        createSeparator(controlColumn);
        //$NON-NLS-1$
        editDefaultButton = createButton(controlColumn, RESOURCE_BUNDLE.getString("editDefault"));
        //$NON-NLS-1$
        goToDefaultButton = createButton(controlColumn, RESOURCE_BUNDLE.getString("goToDefault"));
        createSeparator(controlColumn);
        //$NON-NLS-1$
        expandAllButton = createButton(controlColumn, RESOURCE_BUNDLE.getString("expandAll"));
    ...
    private void clearPreviews() {
        ...
        for (Iterator i = previewSet.iterator(); i.hasNext(); ) {
            ...
            try {
                ...
                WorkbenchPlugin.log(RESOURCE_BUNDLE.getString("errorDisposePreviewLog"), StatusUtil.newStatus(IStatus.ERROR, e.getMessage(), e));
            }
        }
        previewSet.clear();
    }
    ...
    protected boolean resetFont(FontDefinition definition, boolean force) {
        if (force || !isDefault(definition)) {
            ...
            if (definition.isOverridden()) {
                ...
                newFD = PreferenceConverter.getDefaultFontDataArray(getPreferenceStore(), createPreferenceKey(definition));
            }
            if (newFD != null) {
...
*/
    ...
    private void updateTreeSelection(ISelection selection) {
        ...
        if (category != null) {
            // check if there is a preview for it
            ...
            if (previewControl == null) {
                try {
                    ...
                    WorkbenchPlugin.log(RESOURCE_BUNDLE.getString("errorCreatePreviewLog"), StatusUtil.newStatus(IStatus.ERROR, e.getMessage(), e));
                }
            }
        }
        if (previewControl == null) {
            // there is no preview for this theme, use default preview
            if (element instanceof ColorDefinition)
...
*/
    ...
    private void paintColorSample(GC gc) {
        ...
        String messageBottom = MessageFormat.format(fontColorString, new Object[] { Integer.valueOf(rgb.red), Integer.valueOf(rgb.green), Integer.valueOf(rgb.blue) });
    ...
    private void refreshAllLabels() {
        // refresh all the labels in the tree
        tree.getViewer().refresh();
    }
    ...
    private String fomatDescription(ThemeElementDefinition definition) {
        ...
        RESOURCE_BUNDLE.getString("definitionNotAvailInTheme"), new Object[] { description }).trim();
    }
    ...
    private void refreshElement(ThemeElementDefinition definition) {
        ...
        updateTreeSelection(tree.getViewer().getSelection());
        Object newValue = definition instanceof ColorDefinition ? ((ColorDefinition) definition).getValue() : ((FontDefinition) definition).getValue();
...
* @param fontRegistry
*/
        public  CascadingThemeExt(ITheme currentTheme, CascadingColorRegistry colorRegistry, CascadingFontRegistry fontRegistry) {
