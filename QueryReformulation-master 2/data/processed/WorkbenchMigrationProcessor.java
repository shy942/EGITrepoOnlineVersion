...
public class WorkbenchMigrationProcessor {
    ...
    public void migrate() {
        ...
        IModelBuilderFactory builderFactory = ContextInjectionFactory.make(ModelBuilderFactoryImpl.class, builderContext);
        builderContext.set(IModelBuilderFactory.class, builderFactory);
        ApplicationBuilder modelBuilder = builderFactory.createApplicationBuilder(new WorkbenchMementoReader(workbenchMemento));
    ...
    private IMemento loadMemento() {
        ...
        try {
            //$NON-NLS-1$
            reader = new BufferedReader(new InputStreamReader(new FileInputStream(legacyWorkbenchFile), "utf-8"));
            memento = XMLMemento.createReadRoot(reader);
        } catch (IOException e) {
            WorkbenchPlugin.log("Failed to load " + legacyWorkbenchFile.getAbsolutePath(), e);
    ...
    private void moveStickyViews(IStickyViewDescriptor[] stickyViews, MWindow window) {
        ...
        for (MPartStack stickyFolder : modelService.findElements(application, StickyViewDescriptor.STICKY_FOLDER_RIGHT, MPartStack.class, null)) {
    ...
    private void removeStickyViewFromPerspectives(IStickyViewDescriptor stickyView, MWindow window) {
        for (MPlaceholder placeholder : modelService.findElements(window, stickyView.getId(), MPlaceholder.class, null)) {
