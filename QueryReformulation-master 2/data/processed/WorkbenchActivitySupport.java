...
*/
            private void log(Exception e) {
                StatusUtil.newStatus(IStatus.ERROR, "Could not update contribution managers", //$NON-NLS-1$
                e);
            }
        });
    }
...
*/
    private ImageBindingRegistry getActivityImageBindingRegistry() {
        if (activityImageBindingRegistry == null) {
            ...
            PlatformUI.getWorkbench().getExtensionTracker().registerHandler(activityImageBindingRegistry, ExtensionTracker.createExtensionPointFilter(getActivitySupportExtensionPoint()));
        }
...
*/
    private ImageBindingRegistry getCategoryImageBindingRegistry() {
        if (categoryImageBindingRegistry == null) {
            ...
            PlatformUI.getWorkbench().getExtensionTracker().registerHandler(categoryImageBindingRegistry, ExtensionTracker.createExtensionPointFilter(getActivitySupportExtensionPoint()));
        }
...
*/
    public ITriggerPointAdvisor getTriggerPointAdvisor() {
        ...
        if (product != null) {
            TriggerPointAdvisorDescriptor descriptor = TriggerPointAdvisorRegistry.getInstance().getAdvisorForProduct(product.getId());
            if (descriptor != null) {
                try {
