...
public class BindingProcessingAddon {
    ...
    private void activateContexts(Object me) {
        ...
        if (me instanceof MElementContainer) {
            @SuppressWarnings("unchecked") List<MUIElement> children = ((MElementContainer<MUIElement>) me).getChildren();
    ...
    private void defineBindingTables() {
        Activator.trace(Policy.DEBUG_CMDS, "Initialize binding tables from model", //$NON-NLS-1$
        null);
...
*/
    private void defineBindingTable(MBindingTable bindingTable) {
        Assert.isNotNull(bindingTable.getBindingContext(), //$NON-NLS-1$
        "Binding context referred to by the binding table \"" + bindingTable.getElementId() + "\"");
        Assert.isNotNull(bindingTable.getBindingContext().getElementId(), //$NON-NLS-1$
        "Element ID of binding table \"" + bindingTable.getElementId() + "\".");
...
*/
    ...
    private Binding createBinding(Context bindingContext, MCommand cmdModel, List<MParameter> modelParms, String keySequence, MKeyBinding binding) {
        ...
        if (binding.getTransientData().get(EBindingService.MODEL_TO_BINDING_KEY) != null) {
            try {
                ...
                System.err.println("Invalid type stored in transient data with the key " + EBindingService.MODEL_TO_BINDING_KEY);
                return null;
            }
        }
        if (cmdModel == null) {
            //$NON-NLS-1$
            Activator.log(IStatus.ERROR, "binding with no command: " + binding);
            return null;
        }
        ...
        if (cmd == null) {
            ...
            "Failed to find command for binding: " + binding);
            ...
            try {
                ...
                Activator.trace(Policy.DEBUG_MENUS, "failed to create: " + binding, e);
                return null;
            }
        }
        return keyBinding;
    ...
    private void updateBinding(MKeyBinding binding, boolean add, Object eObj) {
        ...
        if (table == null) {
            Activator.log(IStatus.ERROR, //$NON-NLS-1$
            "Trying to create \'" + binding + "\' without binding table " + //$NON-NLS-1$
        ...
        additionHandler = new EventHandler() {
            ...
            public void handleEvent(Event event) {
                ...
                if (elementObj instanceof MApplication) {
                    ...
                    if (UIEvents.isSET(event)) {
                        ...
                        if (UIEvents.isADD(event)) {
                            ...
                            if (UIEvents.EventTypes.REMOVE_MANY.equals(event.getProperty(UIEvents.EventTags.TYPE))) {
                                @SuppressWarnings("unchecked") Collection<MParameter> parms = (Collection<MParameter>) oldObj;
                    ...
                    } else // if we've updated the tags for an MKeyBinding
                    if (UIEvents.ApplicationElement.TAGS.equals(attrName)) {
                        List<String> tags = binding.getTags();
                        // remove it from the runtime binding tables
                        if (tags.contains(EBindingService.DELETED_BINDING_TAG)) {
                            updateBinding(binding, false, elementObj);
                        } else // else we're adding the binding to the runtime tables
                        {
                            updateBinding(binding, true, elementObj);
                        }
                    }
