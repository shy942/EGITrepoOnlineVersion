...
public class ZoomTestCase extends UITestCase {
    ...
    protected void doSetUp() throws Exception {
        ...
        apiStore.setValue(IWorkbenchPreferenceConstants.ENABLE_ANIMATIONS, false);
        // These tests are hard-wired to the pre-3.3 zoom behaviour
        // Run them anyway to ensure that we preserve the 3.0 mechanism
        oldMinMaxState = apiStore.getBoolean(IWorkbenchPreferenceConstants.ENABLE_NEW_MIN_MAX);
        ...
        try {
            ...
            //$NON-NLS-1$
            file1 = FileUtil.createFile("Test1.txt", project);
            //$NON-NLS-1$
            file2 = FileUtil.createFile("Test2.txt", project);
            //$NON-NLS-1$
            file3 = FileUtil.createFile("Test3.txt", project);
...
*/
    protected void assertActive(IWorkbenchPart part) {
        ...
        Assert.assertTrue("Unexpected active part: expected " + partName(part) + " and found " + partName(activePart), activePart == part);
    ...
    protected void assertActiveEditor(IEditorPart part) {
        ...
        Assert.assertTrue("Unexpected active editor: expected " + partName(part) + " and found " + partName(activeEditor), activeEditor == part);
    }
