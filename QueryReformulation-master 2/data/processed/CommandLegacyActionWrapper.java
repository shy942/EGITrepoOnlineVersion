...
*/
    public  CommandLegacyActionWrapper(final String id, final ParameterizedCommand command, final String style, final IServiceLocator serviceLocator) {
        if (command == null) {
            throw new NullPointerException(//$NON-NLS-1$
            "An action proxy can't be created without a command");
        }
        if (serviceLocator == null) {
            throw new NullPointerException(//$NON-NLS-1$
            "An action proxy can't be created without a service locator");
        }
...
*/
    ...
    public final void runWithEvent(final Event event) {
        final Command baseCommand = command.getCommand();
        final ExecutionEvent executionEvent = new ExecutionEvent(command.getCommand(), command.getParameterMap(), event, null);
        try {
