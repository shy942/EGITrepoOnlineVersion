...
*/
    ...
    class CyclicPathException extends Exception {
        ...
        protected  CyclicPathException(TreePathCompiler compiler, Object invalidSegment, boolean asChild) {
            super(//$NON-NLS-1$
            "Cannot add " + invalidSegment + " to the list of segments in " + //$NON-NLS-1$
            compiler + //$NON-NLS-1$ //$NON-NLS-2$
            (asChild ? " as a child." : " as a parent."));
        }
    }
...
*/
    ...
    private Set findPaths(TreePathCompiler aPathCompiler) {
        Set /* <Object> */
        parents = findParents(aPathCompiler.getFirstSegment());
        Set /* <TreePathCompiler> */
        parentPaths = new LinkedHashSet();
        Set /* <TreePathCompiler> */
        foundPaths = Collections.EMPTY_SET;
        if (parents.size() > 0) {
