...
class TestUtil {
    ...
    public static void printMenuOut(int level, MElementContainer<MMenuElement> container) {
        for (MMenuElement child : container.getChildren()) {
            printTabs(level);
            System.out.print(child.getClass().getSimpleName() + ": " + child.getElementId());
    ...
    public static MApplication setupRenderer(IEclipseContext appContext) {
        ...
        ((Notifier) application).eAdapters().add(new UIEventPublisher(appContext));
        return application;
    }
    ...
    public static void setupRendererServices(IEclipseContext appContext) {
        ...
        appContext.set(CSSRenderingUtils.class, ContextInjectionFactory.make(CSSRenderingUtils.class, appContext));
    }
