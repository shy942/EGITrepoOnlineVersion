...
public class DetachedDropAgent extends DropAgent {
    ...
    public boolean canDrop(MUIElement dragElement, DnDInfo info) {
        if (info.curElement != null)
            return false;
        if (dragElement instanceof MPart || dragElement instanceof MPlaceholder || dragElement instanceof MPartStack)
            return true;
        return false;
    }
    ...
    public boolean drop(MUIElement dragElement, DnDInfo info) {
        ...
        modelService.detach((MPartSashContainerElement) dragElement, rectangle.x, rectangle.y, rectangle.width, rectangle.height);
