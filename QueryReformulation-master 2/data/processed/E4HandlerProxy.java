...
public class E4HandlerProxy implements IHandler2, IHandlerListener, IElementUpdater {
    ...
    @Execute
    public Object execute(IEclipseContext context, @Optional @Named(HandlerServiceImpl.PARM_MAP) Map parms, @Optional Event trigger, @Optional IEvaluationContext staticContext) throws ExecutionException, NotHandledException {
        Activator.trace(Policy.DEBUG_CMDS, //$NON-NLS-1$ //$NON-NLS-2$
        "execute " + command + " and " + handler + " with: " + context, //$NON-NLS-1$
        null);
        IEvaluationContext appContext = staticContext;
        if (appContext == null) {
            appContext = new ExpressionContext(context);
        }
        ExecutionEvent event = new ExecutionEvent(command, parms == null ? Collections.EMPTY_MAP : parms, trigger, appContext);
    ...
    public Object execute(ExecutionEvent event) throws ExecutionException {
        if (logExecute) {
            ...
            "org.eclipse.ui", "Called handled proxy execute(*) directly" + command, //$NON-NLS-1$
            new Exception());
    ...
    public void setEnabled(Object evaluationContext) {
        if (logSetEnabled) {
            ...
            Status(//$NON-NLS-1$
            IStatus.WARNING, //$NON-NLS-1$
            "org.eclipse.ui", "Called handled proxy setEnabled(*) directly" + command, //$NON-NLS-1$
            new Exception());
            WorkbenchPlugin.log(status);
        }
    }
