...
*/
    private static final void addReverseLookup(final Map map, final Object key, final Object value) {
        if (map == null) {
            return;
        }
...
*/
    public  BindingManager(final ContextManager contextManager, final CommandManager commandManager) {
        if (contextManager == null) {
            ...
            "A binding manager requires a context manager");
        }
        if (commandManager == null) {
            throw new NullPointerException(//$NON-NLS-1$
            "A binding manager requires a command manager");
        }
...
*/
    public final void addBindingManagerListener(final IBindingManagerListener listener) {
...
*            computed).
*/
    private final void computeBindings(final Map activeContextTree, final Map bindingsByTrigger, final Map triggersByCommandId, final Map conflictsByTrigger) {
        /*
...
*/
        ...
        MultiStatus conflicts = new //$NON-NLS-1$
        MultiStatus(//$NON-NLS-1$
        "org.eclipse.jface", //$NON-NLS-1$
        0, //$NON-NLS-1$
        "Keybinding conflicts occurred.  They may interfere with normal accelerator operation.", null);
        /*
            ...
            if (activeContextTree == null) {
                ...
                if (match instanceof Binding) {
                    ...
                    while (matchItr.hasNext()) {
                        addReverseLookup(triggersByCommandId, ((Binding) matchItr.next()).getParameterizedCommand(), trigger);
                    }
                }
            } else {
                ...
                if (match instanceof Binding) {
                    ...
                    if (winner == null) {
                        ...
                        if (triggerConflicts.add(trigger)) {
                            ...
                            try {
                                //$NON-NLS-1$
                                buffer.write("A conflict occurred for ");
                            ...
                            conflicts.add(new Status(IStatus.WARNING, //$NON-NLS-1$
                            "org.eclipse.jface", sw.toString()));
                        }
                        if (DEBUG) {
                            //$NON-NLS-1$
                            Tracing.printTrace(//$NON-NLS-1$
                            "BINDINGS", //$NON-NLS-1$
                            "A conflict occurred for " + trigger);
                            //$NON-NLS-1$ //$NON-NLS-2$
                            Tracing.printTrace("BINDINGS", "    " + match);
                        }
                    } else {
        ...
        while (contextIdItr.hasNext()) {
            ...
            try {
                ...
                if (context == null || !bindingErrors.contains(context.getId())) {
                    ...
                    Policy.getLog().log(new Status(IStatus.ERROR, Policy.JFACE, IStatus.OK, "Undefined context while filtering dialog/window contexts", e));
                }
            }
        }
...
* @since 3.2
*/
    public final TriggerSequence[] getActiveBindingsDisregardingContextFor(final ParameterizedCommand parameterizedCommand) {
        final Object object = getActiveBindingsDisregardingContextByParameterizedCommand().get(parameterizedCommand);
...
*         never be <code>null</code>, but it may be empty.
*/
    public final TriggerSequence[] getActiveBindingsFor(final ParameterizedCommand parameterizedCommand) {
...
*/
    public final TriggerSequence[] getActiveBindingsFor(final String commandId) {
        final ParameterizedCommand parameterizedCommand = new ParameterizedCommand(commandManager.getCommand(commandId), null);
    ...
    private final String[] getSchemeIds(String schemeId) {
        ...
        while (schemeId != null) {
            ...
            try {
                ...
                Policy.getLog().log(new Status(IStatus.ERROR, Policy.JFACE, IStatus.OK, "Failed ascending scheme parents", e));
                return new String[0];
            }
        }
        return (String[]) strings.toArray(new String[strings.size()]);
...
*/
    private final void recomputeBindings() {
        ...
        // Build a cached binding set for that state.
        final CachedBindingSet bindingCache = new CachedBindingSet(activeContextTree, locales, platforms, activeSchemeIds);
        /*
* Check if the cached binding set already exists. If so, simply set the
* active bindings and return.
*/
        ...
        if (existingCache.isInitialized()) {
            ...
            setActiveBindings(existingCache.getBindingsByTrigger(), existingCache.getTriggersByCommandId(), existingCache.getPrefixTable(), existingCache.getConflictsByTrigger());
            return;
        }
        // There is no cached entry for this.
        if (DEBUG) {
...
*/
    public final void removeBindingManagerListener(final IBindingManagerListener listener) {
...
*
*/
    public final void removeBindings(final TriggerSequence sequence, final String schemeId, final String contextId, final String locale, final String platform, final String windowManager, final int type) {
...
*/
    private final Binding[] removeDeletions(final Binding[] bindings) {
        ...
        if (DEBUG) {
            ...
            "There are " + deletions.size() + //$NON-NLS-1$
            " deletion markers");
        }
    ...
    public final void schemeChanged(final SchemeEvent schemeEvent) {
        if (schemeEvent.isDefinedChanged()) {
            ...
            if (isListenerAttached()) {
                fireBindingManagerChanged(new BindingManagerEvent(this, false, null, activeSchemeChanged, scheme, schemeIdAdded, false, false));
            }
        }
    }
...
*            solution.
*/
    private final void setActiveBindings(final Map activeBindings, final Map activeBindingsByCommandId, final Map prefixTable, final Map conflicts) {
        ...
        fireBindingManagerChanged(new BindingManagerEvent(this, true, previousBindingsByParameterizedCommand, false, null, false, false, false));
    }
...
*/
    public final void setActiveScheme(final Scheme scheme) throws NotDefinedException {
        if (scheme == null) {
            //$NON-NLS-1$
            throw new NullPointerException("Cannot activate a null scheme");
        }
        if ((scheme == null) || (!scheme.isDefined())) {
            throw new NotDefinedException(//$NON-NLS-1$
            "Cannot activate an undefined scheme. " + scheme.getId());
        }
