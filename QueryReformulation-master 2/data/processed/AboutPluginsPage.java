            ...
            if (columnIndex == 0) {
                if (element instanceof AboutBundleData) {
                    ...
                    if (data.isSignedDetermined()) {
                        return WorkbenchImages.getImage(data.isSigned() ? IWorkbenchGraphicConstants.IMG_OBJ_SIGNED_YES : IWorkbenchGraphicConstants.IMG_OBJ_SIGNED_NO);
                    }
                    synchronized (resolveQueue) {
...
*/
    protected void handleSigningInfoPressed() {
        if (signingArea == null) {
            ...
            AboutBundleData bundleInfo = (AboutBundleData) ((IStructuredSelection) vendorInfo.getSelection()).getFirstElement();
...
*/
    protected void createTable(Composite parent) {
        ...
        int[] columnWidths = { // signature
        convertHorizontalDLUsToPixels(30), convertHorizontalDLUsToPixels(120), convertHorizontalDLUsToPixels(120), convertHorizontalDLUsToPixels(70), convertHorizontalDLUsToPixels(130) };
        // create table headers
        for (int i = 0; i < columnTitles.length; i++) {
            TableColumn column = new TableColumn(vendorInfo.getTable(), SWT.NULL);
            if (i == PLUGIN_NAME_COLUMN_INDEX) {
                // prime initial sorting
...
*/
    private boolean selectionHasInfo(AboutBundleData bundleInfo) {
        ...
        if (infoURL == null && WorkbenchPlugin.DEBUG) {
            ...
            "Problem reading plugin info for: " + bundleInfo.getName());
        }
        return infoURL != null;
    }
...
*/
    protected void handleMoreInfoPressed() {
        ...
        if (!AboutUtils.openBrowser(getShell(), getMoreInfoURL(bundleInfo, true))) {
            String message = NLS.bind(WorkbenchMessages.AboutPluginsDialog_unableToOpenFile, PLUGININFO, bundleInfo.getId());
            StatusUtil.handleStatus(WorkbenchMessages.AboutPluginsDialog_errorTitle + ": " + message, StatusManager.SHOW, //$NON-NLS-1$
            getShell());
        }
    }
...
class BundlePatternFilter extends ViewerFilter {
    ...
    public boolean select(Viewer viewer, Object parentElement, Object element) {
        ...
        if (element instanceof AboutBundleData) {
            AboutBundleData data = (AboutBundleData) element;
            return matcher.match(data.getName()) || matcher.match(data.getProviderName()) || matcher.match(data.getId());
        }
        return true;
    }
}
