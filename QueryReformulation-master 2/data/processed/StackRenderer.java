...
*/
    ...
    List<CTabItem> getItemsToSet(MPart part) {
        ...
        if (partParent instanceof MPartStack) {
            ...
            List<MPlaceholder> partRefs = modelService.findElements(topWin, part.getElementId(), MPlaceholder.class, null);
    ...
    @Inject
    @Optional
    private void subscribeTopicTransientDataChanged(@UIEventTopic(UIEvents.ApplicationElement.TOPIC_TRANSIENTDATA) org.osgi.service.event.Event event) {
...
*/
    ...
    private void subscribeTopicChildrenChanged(@UIEventTopic(UIEvents.ElementContainer.TOPIC_CHILDREN) Event event) {
        ...
        if (container instanceof MPart) {
            ...
            if (parent instanceof MPartStack && parent.getSelectedElement() == container && parent.getRenderer() == StackRenderer.this) {
    ...
    @Inject
    @Optional
    private void subscribeTopicClosablePlaceholderChanged(@UIEventTopic(UIEvents.Placeholder.TOPIC_CLOSEABLE) Event event) {
        updateClosableTab(event);
    }
...
*/
    ...
    private void subscribeTopicActivateChanged(@UIEventTopic(UIEvents.UILifeCycle.ACTIVATE) Event event) {
        ...
        List<MUIElement> activeElements = modelService.findElements(modelService.getTopLevelWindowFor(newActivePart), null, MUIElement.class, tags);
    ...
    @Inject
    @Optional
    private void subscribeTopicSelectedelementChanged(@UIEventTopic(UIEvents.ElementContainer.TOPIC_SELECTEDELEMENT) Event event) {
        if (tabStateHandler == null) {
            tabStateHandler = new TabStateHandler();
        }
        tabStateHandler.handleEvent(event);
...
*/
    ...
    public void hookControllerLogic(final MUIElement me) {
        ...
        if (!(me instanceof MElementContainer<?>))
            return;
        @SuppressWarnings("unchecked") final MElementContainer<MUIElement> stack = (MElementContainer<MUIElement>) me;
    ...
    public void showAvailableItems(MElementContainer<?> stack, CTabFolder ctf) {
        ...
        final BasicPartList editorList = new BasicPartList(ctf.getShell(), SWT.ON_TOP, SWT.V_SCROLL | SWT.H_SCROLL, ctxt.get(EPartService.class), stack, this, getMRUValueFromPreferences());
        editorList.setInput();
...
*/
    ...
    private List<MPart> getCloseableSiblingParts(MPart part, List<MUIElement> children, final int start, final int end) {
    ...
    private boolean hasVisibleMenuItems(MMenu viewMenu, MPart part) {
        ...
        for (MMenuElement menuElement : viewMenu.getChildren()) {
            if (menuElement.isToBeRendered() && menuElement.isVisible()) {
                if (OpaqueElementUtil.isOpaqueMenuItem(menuElement) || OpaqueElementUtil.isOpaqueMenuSeparator(menuElement)) {
...
*/
    ...
    public class TabStateHandler implements EventHandler {
        ...
        public void handleEvent(Event event) {
            ...
            } else if (// part gets active
            newValue instanceof MPlaceholder && part.getTags().contains(CSSConstants.CSS_HIGHLIGHTED_CLASS)) {
        ...
        public boolean validateValues(Object oldValue, Object newValue) {
            ...
            isTagAdded(CSSConstants.CSS_BUSY_CLASS, oldValue, newValue) || // part gets busy
            isTagRemoved(CSSConstants.CSS_BUSY_CLASS, oldValue, newValue) || // part gets idle
