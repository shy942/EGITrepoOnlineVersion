...
*/
    public  ValidatedObservableList(final IObservableList target, final IObservableValue validationStatus) {
    ...
    public void add(int index, Object element) {
        ...
        ListDiff diff = Diffs.createListDiff(Diffs.createListDiffEntry(index, true, element));
    ...
    public boolean addAll(int index, Collection c) {
        ...
        for (int i = 0; i < elements.length; i++) {
            ...
            entries[i] = Diffs.createListDiffEntry(index + i, true, elements[i]);
        }
    ...
    public void clear() {
        ...
        if (isEmpty())
            return;
        ListDiff diff = Diffs.computeListDiff(wrappedList, Collections.EMPTY_LIST);
        wrappedList.clear();
        updateTargetList(diff);
        fireListChange(diff);
    }
    ...
    public Iterator iterator() {
        ...
        return new Iterator() {
            ...
            public void remove() {
                ...
                ListDiff diff = Diffs.createListDiff(Diffs.createListDiffEntry(index, false, last));
                updateTargetList(diff);
                fireListChange(diff);
            }
        };
    ...
    public Object move(int oldIndex, int newIndex) {
        ...
        if (oldIndex >= size)
            throw new IndexOutOfBoundsException(//$NON-NLS-1$ //$NON-NLS-2$
            "oldIndex: " + oldIndex + ", size:" + size);
        if (newIndex >= size)
            throw new IndexOutOfBoundsException(//$NON-NLS-1$ //$NON-NLS-2$
        ...
        ListDiff diff = Diffs.createListDiff(Diffs.createListDiffEntry(oldIndex, false, element), Diffs.createListDiffEntry(newIndex, true, element));
        updateTargetList(diff);
        fireListChange(diff);
        return element;
    }
    ...
    public Object set(int index, Object element) {
        ...
        ListDiff diff = Diffs.createListDiff(Diffs.createListDiffEntry(index, false, oldElement), Diffs.createListDiffEntry(index, true, element));
        updateTargetList(diff);
        fireListChange(diff);
        return oldElement;
    }
