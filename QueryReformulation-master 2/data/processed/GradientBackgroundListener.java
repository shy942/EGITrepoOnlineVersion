...
*/
        if (grad.isRadial()) {
            ...
            BufferedImage image = getBufferedImage(size.x, size.y, colors, CSSSWTColorHelper.getPercents(grad));
            ...
            radialGradient = true;
        } else if (oldImage == null || oldImage.isDisposed() || oldImage.getBounds().height != size.y || radialGradient || event == null) {
            ...
            fillGradient(gc, new Rectangle(0, 0, x, y), colors, CSSSWTColorHelper.getPercents(grad), grad.getVerticalGradient());
            gc.dispose();
            for (Color c : colors) {
                // Dispose colors too.
                c.dispose();
    ...
    private static void fillGradient(GC gc, Rectangle rect, List<Color> gradientColors, int[] gradientPercents, boolean gradientVertical) {
        ...
        if (gradientColors.size() == 1) {
            ...
            for (int i = 0; i < loopCount; ++i) {
                ...
                if (gradientVertical) {
                    final int gradientHeight = (grpercent * rect.height / 100) - (pos - rect.y);
                    gc.fillGradientRectangle(rect.x, pos, rect.width, gradientHeight, true);
                    pos += gradientHeight;
                } else {
                    final int gradientWidth = (grpercent * rect.width / 100) - (pos - rect.x);
...
*/
    private BufferedImage getBufferedImage(int width, int height, List<java.awt.Color> colors, int[] percents) {
        ...
        RadialGradientPaint radialGradientPaint = new RadialGradientPaint(new Point2D.Double(width / 2.0, 0), width, new Point2D.Double(width / 2.0, 0.0), fractions, colorArray, CycleMethod.NO_CYCLE);
