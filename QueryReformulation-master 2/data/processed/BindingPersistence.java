...
*/
    private static final void readActiveScheme(final IConfigurationElement[] configurationElements, final int configurationElementCount, final IMemento preferences, final BindingManager bindingManager) {
        ...
        if ((defaultActiveSchemeId != null && defaultActiveSchemeId.length() > 0) && (!defaultActiveSchemeId.equals(IBindingService.DEFAULT_DEFAULT_ACTIVE_SCHEME_ID))) {
            try {
...
*            <code>null</code>.
*/
    private static final void readBindingsFromPreferences(final IMemento preferences, final BindingManager bindingManager, final CommandManager commandService) {
        ...
        if (preferences != null) {
            ...
            for (int i = preferenceMementoCount - 1; i >= 0; i--) {
                ...
                if (schemeId == null) {
                    schemeId = readRequired(memento, ATT_CONFIGURATION, warningsToLog, //$NON-NLS-1$
                    "Key bindings need a scheme or key configuration");
                    if (schemeId == null) {
                        continue;
                    }
                }
                ...
                if (keySequenceText == null) {
                    keySequenceText = readRequired(memento, ATT_STRING, warningsToLog, //$NON-NLS-1$
                    "Key bindings need a key sequence or string");
                    if (keySequenceText == null) {
                        continue;
                    }
                    ...
                    try {
                        ...
                        addWarning(warningsToLog, "Could not parse", null, commandId, "keySequence", keySequenceText);
                        continue;
                    }
                    if (keySequence.isEmpty() || !keySequence.isComplete()) {
                        addWarning(warningsToLog, //$NON-NLS-1$
                        "Key bindings cannot use an empty or incomplete key sequence", null, commandId, "keySequence", //$NON-NLS-1$
                ...
                final ParameterizedCommand parameterizedCommand = command != null ? readParameters(memento, warningsToLog, command) : null;
                final Binding binding = new KeyBinding(keySequence, parameterizedCommand, schemeId, contextId, locale, platform, null, Binding.USER);
                bindingManager.addBinding(binding);
            }
        }
        // If there were any warnings, then log them now.
        logWarnings(warningsToLog, //$NON-NLS-1$
        "Warnings while parsing the key bindings from the preference store");
    }
...
*            <code>null</code>.
*/
    private static final void readBindingsFromRegistry(final IConfigurationElement[] configurationElements, final int configurationElementCount, final BindingManager bindingManager, final CommandManager commandService) {
...
*/
            ...
            if (commandId != null) {
                ...
                if (!command.isDefined()) {
                    ...
                    "Cannot bind to an undefined command", configurationElement, commandId);
                    continue;
                }
            } else {
                command = null;
            ...
            if (isEmpty(keySequenceText)) {
                ...
                addWarning(warningsToLog, //$NON-NLS-1$
                "Defining a key binding with no key sequence has no effect", configurationElement, commandId);
                continue;
            }
            // Read out the key sequence.
            ...
            ParameterizedCommand parameterizedCommand = readParameterizedCommand(warningsToLog, configurationElement, viewParameter, command);
    ...
    private static List applyModifiers(KeySequence keySequence, String keySequenceText, String platform, IConfigurationElement[] sequenceModifiers, ParameterizedCommand parameterizedCommand, String schemeId, String contextId, String locale, List warningsToLog) {
        ...
        for (int i = 0; i < sequenceModifiers.length; i++) {
            ...
            if (keySequenceText.startsWith(findSequence)) {
                ...
                try {
                    if (platform == null) {
                        addGenericBindings(keySequence, parameterizedCommand, schemeId, contextId, locale, bindings, modifiedSequence, platforms);
                    } else {
                        getBindingForPlatform(keySequence, platform, parameterizedCommand, schemeId, contextId, locale, bindings, modifiedSequence, platforms);
                    }
                } catch (ParseException e) {
                    bindings.clear();
                    addWarning(warningsToLog, "Cannot create modified sequence for key binding", sequenceModifier, parameterizedCommand.getId(), ATT_REPLACE, replaceSequence);
                }
                break;
            }
        }
    ...
    private static void getBindingForPlatform(KeySequence keySequence, String platform, ParameterizedCommand parameterizedCommand, String schemeId, String contextId, String locale, List bindings, String modifiedSequence, String[] platforms) throws ParseException {
        ...
        for (; j < platforms.length; j++) {
            if (platforms[j].equals(SWT.getPlatform())) {
                KeyBinding newBinding = new KeyBinding(KeySequence.getInstance(modifiedSequence), parameterizedCommand, schemeId, contextId, locale, platforms[j], null, Binding.SYSTEM);
        ...
        if (j == platforms.length) {
            ...
            KeyBinding newBinding = new KeyBinding(keySequence, parameterizedCommand, schemeId, contextId, locale, null, null, Binding.SYSTEM);
            bindings.add(newBinding);
        }
    }
    ...
    private static void addGenericBindings(KeySequence keySequence, ParameterizedCommand parameterizedCommand, String schemeId, String contextId, String locale, List bindings, String modifiedSequence, String[] platforms) throws ParseException {
        KeyBinding originalBinding = new KeyBinding(keySequence, parameterizedCommand, schemeId, contextId, locale, null, null, Binding.SYSTEM);
        ...
        if (modifierExists) {
            KeyBinding newBinding = new KeyBinding(KeySequence.getInstance(modifiedSequence), parameterizedCommand, schemeId, contextId, locale, SWT.getPlatform(), null, Binding.SYSTEM);
            KeyBinding deleteBinding = new KeyBinding(keySequence, null, schemeId, contextId, locale, SWT.getPlatform(), null, Binding.SYSTEM);
            bindings.add(newBinding);
    ...
    private static KeySequence readKeySequence(IConfigurationElement configurationElement, List warningsToLog, String commandId, String keySequenceText) {
        ...
        if (keySequenceText.equals(configurationElement.getAttribute(ATT_STRING))) {
            ...
            if (keySequence.isEmpty() || !keySequence.isComplete()) {
                ...
                commandId, //$NON-NLS-1$
                "keySequence", keySequence.toString());
                return null;
            }
        }
        return keySequence;
    ...
    private static ParameterizedCommand readParameterizedCommand(final List warningsToLog, final IConfigurationElement configurationElement, String viewParameter, final Command command) {
        ...
        if ((schemeId == null) || (schemeId.length() == 0)) {
            ...
            if ((schemeId == null) || (schemeId.length() == 0)) {
                ...
                if ((schemeId == null) || (schemeId.length() == 0)) {
                    ...
                    addWarning(//$NON-NLS-1$
                    warningsToLog, //$NON-NLS-1$
                    "Key bindings need a scheme", configurationElement, commandId);
                }
            }
        }
        return schemeId;
...
*/
    private static final void readSchemesFromRegistry(final IConfigurationElement[] configurationElements, final int configurationElementCount, final BindingManager bindingManager) {
        ...
        for (int i = 0; i < configurationElementCount; i++) {
            ...
            final String id = readRequired(configurationElement, ATT_ID, warningsToLog, //$NON-NLS-1$
            "Schemes need an id");
            if (id == null) {
                continue;
            }
        ...
        logWarnings(warningsToLog, //$NON-NLS-1$
        "Warnings while parsing the key bindings from the 'org.eclipse.ui.bindings', 'org.eclipse.ui.acceleratorConfigurations' and 'org.eclipse.ui.commands' extension point");
    }
...
*/
    static final void write(final Scheme activeScheme, final Binding[] bindings) throws IOException {
        ...
        if (DEBUG) {
            Tracing.printTrace("BINDINGS", //$NON-NLS-1$ //$NON-NLS-2$
            "Persisting active scheme '" + activeScheme.getId() + '\'');
            //$NON-NLS-1$ //$NON-NLS-2$
...
*/
    ...
    public boolean bindingsNeedUpdating(final IRegistryChangeEvent event) {
        final IExtensionDelta[] acceleratorConfigurationDeltas = event.getExtensionDeltas(PlatformUI.PLUGIN_ID, IWorkbenchRegistryConstants.PL_ACCELERATOR_CONFIGURATIONS);
        if (acceleratorConfigurationDeltas.length == 0) {
            final IExtensionDelta[] bindingDeltas = event.getExtensionDeltas(PlatformUI.PLUGIN_ID, IWorkbenchRegistryConstants.PL_BINDINGS);
            if (bindingDeltas.length == 0) {
                final IExtensionDelta[] commandDeltas = event.getExtensionDeltas(PlatformUI.PLUGIN_ID, IWorkbenchRegistryConstants.PL_COMMANDS);
                if (commandDeltas.length == 0) {
                    final IExtensionDelta[] acceleratorScopeDeltas = event.getExtensionDeltas(PlatformUI.PLUGIN_ID, IWorkbenchRegistryConstants.PL_ACCELERATOR_SCOPES);
                    if (acceleratorScopeDeltas.length == 0) {
                        final IExtensionDelta[] contextDeltas = event.getExtensionDeltas(PlatformUI.PLUGIN_ID, IWorkbenchRegistryConstants.PL_CONTEXTS);
                        if (contextDeltas.length == 0) {
                            final IExtensionDelta[] actionDefinitionDeltas = event.getExtensionDeltas(PlatformUI.PLUGIN_ID, IWorkbenchRegistryConstants.PL_ACTION_DEFINITIONS);
    ...
    public void reRead() {
        ...
        for (int i = 0; i < bindingsExtensionPoint.length; i++) {
            ...
            if (TAG_KEY.equals(name)) {
                addElementToIndexedArray(configurationElement, indexedConfigurationElements, INDEX_BINDING_DEFINITIONS, bindingDefinitionCount++);
            } else // Check to see if it is a scheme definition.
            if (TAG_SCHEME.equals(name)) {
                addElementToIndexedArray(configurationElement, indexedConfigurationElements, INDEX_SCHEME_DEFINITIONS, schemeDefinitionCount++);
            }
        }
        ...
        for (int i = 0; i < commandsExtensionPoint.length; i++) {
            ...
            // Check if it is an active scheme identifier.
                ...
                addElementToIndexedArray(configurationElement, indexedConfigurationElements, INDEX_ACTIVE_SCHEME, activeSchemeElementCount++);
            }
        }
        /*
...
* definitions.
*/
        final IConfigurationElement[] acceleratorConfigurationsExtensionPoint = registry.getConfigurationElementsFor(EXTENSION_ACCELERATOR_CONFIGURATIONS);
        ...
        readSchemesFromRegistry(indexedConfigurationElements[INDEX_SCHEME_DEFINITIONS], schemeDefinitionCount, bindingManager);
        readActiveScheme(indexedConfigurationElements[INDEX_ACTIVE_SCHEME], activeSchemeElementCount, preferenceMemento, bindingManager);
        readBindingsFromRegistry(indexedConfigurationElements[INDEX_BINDING_DEFINITIONS], bindingDefinitionCount, bindingManager, commandManager);
