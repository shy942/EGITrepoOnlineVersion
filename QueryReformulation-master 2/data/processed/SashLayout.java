...
public class SashLayout extends Layout {
    ...
    protected List<SashRect> getSashRects(int x, int y) {
        ...
        for (SashRect sr : sashes) {
            if (!sr.container.getTags().contains(IPresentationEngine.NO_MOVE) && sr.rect.intersects(target))
                srs.add(sr);
        }
        return srs;
    }
    ...
    private void tileSubNodes(Rectangle bounds, MUIElement node) {
        ...
        for (MUIElement subNode : visibleChildren) {
            ...
            if (prev != null) {
                Rectangle sashRect = sashContainer.isHorizontal() ? new Rectangle(tilePos, bounds.y, sashWidth, bounds.height) : new Rectangle(bounds.x, tilePos, bounds.width, sashWidth);
                sashes.add(new SashRect(sashRect, sashContainer, prev, subNode));
                host.redraw(sashRect.x, sashRect.y, sashRect.width, sashRect.height, false);
                tilePos += sashWidth;
            }
