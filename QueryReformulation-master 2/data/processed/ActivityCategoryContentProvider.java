...
*/
    private IActivity[] getCategoryActivities(ICategory category) {
        Set activityIds = InternalActivityHelper.getActivityIdsForCategory(manager, category);
            ...
            if (!currentCategoryId.equals(categorizedActivity.getCategory().getId())) {
                ...
                for (int index = 0; index < categoryActivities.length; index++) {
                    ...
                    if (currentActivityId.equals(categorizedActivity.getActivity().getId())) {
                        duplicateCategorizedactivities.add(new CategorizedActivity(currentCategory, manager.getActivity(currentActivityId)));
                        // break
                        break;
                    }
                }
    ...
    public Object[] getChildRequiredActivities(String activityId) {
        ...
        for (Iterator i = actvitiyRequirementBindings.iterator(); i.hasNext(); ) {
            ...
            for (int index = 0; index < currentCategoryIds.length; index++) {
                childRequiredActivities.add(new CategorizedActivity(manager.getCategory((String) currentCategoryIds[index]), manager.getActivity(requiredActivityId)));
            }
        }
...
*/
    public Object[] getParentRequiredActivities(String activityId) {
        ...
        for (Iterator i = definedActivities.iterator(); i.hasNext(); ) {
            ...
            for (Iterator j = activityRequirementBindings.iterator(); j.hasNext(); ) {
                ...
                if (currentActivityRequirementBinding.getRequiredActivityId().equals(activityId)) {
                    ...
                    for (int index = 0; index < currentCategoryIds.length; index++) {
                        parentRequiredActivities.add(new CategorizedActivity(manager.getCategory((String) currentCategoryIds[index]), manager.getActivity(currentActivityId)));
                    }
                }
            }
        }
