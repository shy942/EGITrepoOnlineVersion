...
*/
    ...
    protected Control createDialogArea(Composite parent) {
        ...
        if (!gcj && vmName != null && vmName.indexOf("libgcj") != -1) {
            //$NON-NLS-1$
            composite.getDisplay().asyncExec(() -> setMessage(IDEWorkbenchMessages.UnsupportedVM_message, IMessageProvider.WARNING));
        }
...
*/
    private void createRecentWorkspacesComposite(final Composite composite) {
        ...
        ExpandableComposite expandableComposite = toolkit.createExpandableComposite(recentWorkspacesForm.getBody(), ExpandableComposite.TWISTIE);
...
*/
    private Map<String, String> createUniqueWorkspaceNameMap() {
        ...
        List<String[]> splittedWorkspaceNames = Arrays.asList(launchData.getRecentWorkspaces()).stream().filter( s -> s != null && !s.isEmpty()).map( s -> s.split(Pattern.quote(fileSeparator))).collect(Collectors.toList());
        for (int i = 1; !splittedWorkspaceNames.isEmpty(); i++) {
            final int c = i;
            Function<String[], String> stringArraytoName =  s -> String.join(fileSeparator, Arrays.copyOfRange(s, s.length - c, s.length));
            List<String> uniqueNames = splittedWorkspaceNames.stream().map(stringArraytoName).collect(Collectors.groupingBy( s -> s, Collectors.counting())).entrySet().stream().filter( e -> e.getValue() == 1).map( e -> e.getKey()).collect(Collectors.toList());
...
*/
    private String getInitialBrowsePath() {
        ...
        return dir != null ? dir.getAbsolutePath() : System.getProperty(//$NON-NLS-1$
        "user.dir");
    }
