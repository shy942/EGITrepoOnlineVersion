        ...
        try {
            ...
            if (builder.isShowingHierarchy()) {
                ...
                for (int i = 0; i < categories.length; i++) {
                    ...
                    MarkerSortUtil.sortStartingKElement(markerEntryArray, comparator, category.start, category.end, effLimit, monitor);
                }
            } else {
                if (monitor.isCanceled()) {
                    return false;
                ...
                MarkerSortUtil.sortStartingKElement(markerEntryArray, builder.getComparator(), effLimit, monitor);
            }
            if (monitor.isCanceled()) {
                return false;
            }
...
*/
    MarkerCategory[] groupIntoCategories(IProgressMonitor monitor, MarkerEntry[] newMarkers) {
        Map<MarkerGroupingEntry, Integer> boundaryInfoMap = groupMarkerEntries(newMarkers, builder.getCategoryGroup(), newMarkers.length - 1, monitor);
        int start = 0;
        ...
        for (Entry<MarkerGroupingEntry, Integer> entry : boundaryInfoMap.entrySet()) {
            ...
            markerCategories[i++] = new MarkerCategory(this, start, end, builder.getCategoryGroup().getMarkerField().getValue(newMarkers[start]));
            start = end + 1;
        }
...
*/
    private Map<MarkerGroupingEntry, Integer> groupMarkerEntries(MarkerEntry[] entries, MarkerGroup group, int k, IProgressMonitor monitor) {
        TreeMap<MarkerGroupingEntry, List<MarkerEntry>> map = new TreeMap(group.getEntriesComparator());
        for (int i = 0; i <= k; i++) {
        ...
        TreeMap<MarkerGroupingEntry, Integer> result = new TreeMap(group.getEntriesComparator());
        int i = 0;
