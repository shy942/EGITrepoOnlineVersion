...
*            {@link #MATCH_PARTIAL}
*/
     QuickAccessEntry(QuickAccessElement element, QuickAccessProvider provider, int[][] elementMatchRegions, int[][] providerMatchRegions, int matchQuality) {
        ...
        switch(event.index) {
            ...
            case 1:
                ...
                if (element instanceof CommandElement) {
                    ...
                    if (binding != null) {
                        StyledString styledString = StyledCellLabelProvider.styleDecoratedString(label, StyledString.QUALIFIER_STYLER, new StyledString(commandElement.getCommand()));
                        StyleRange[] styleRanges = styledString.getStyleRanges();
                        for (int i = 0; i < styleRanges.length; i++) {
                            textLayout.setStyle(styleRanges[i], styleRanges[i].start, styleRanges[i].start + styleRanges[i].length);
                        }
                    }
                }
