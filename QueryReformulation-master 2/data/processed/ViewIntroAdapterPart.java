...
*/
    ...
    public void init(IViewSite site, IMemento memento) throws PartInitException {
        ...
        try {
            ...
            WorkbenchPlugin.log(IntroMessages.Intro_could_not_create_proxy, new Status(IStatus.ERROR, WorkbenchPlugin.PI_WORKBENCH, IStatus.ERROR, IntroMessages.Intro_could_not_create_proxy, e));
        }
    }
...
*/
    private void setBarVisibility(final boolean visible) {
        ...
        // don't layout unless things have actually changed
        boolean layout = false;
        if (visible) {
            // Restore the last 'saved' state
            boolean coolbarVisible = PrefUtil.getInternalPreferenceStore().getBoolean(IPreferenceConstants.COOLBAR_VISIBLE);
            boolean persBarVisible = PrefUtil.getInternalPreferenceStore().getBoolean(IPreferenceConstants.PERSPECTIVEBAR_VISIBLE);
            layout = (coolbarVisible != window.getCoolBarVisible()) || (persBarVisible != window.getPerspectiveBarVisible());
