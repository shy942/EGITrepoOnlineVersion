...
*/
    ...
    public IStatus proceedRedoing(IUndoableOperation operation, IOperationHistory history, IAdaptable uiInfo) {
        ...
        String message = NLS.bind(WorkbenchMessages.Operations_nonLocalRedoWarning, operation.getLabel(), part.getEditorInput().getName());
    ...
    public IStatus proceedUndoing(IUndoableOperation operation, IOperationHistory history, IAdaptable uiInfo) {
        ...
        String message = NLS.bind(WorkbenchMessages.Operations_nonLocalUndoWarning, operation.getLabel(), part.getEditorInput().getName());
...
*/
    private IStatus proceedWithOperation(IUndoableOperation operation, final String message, final String discardButton, final String title) {
        // nothing we can do.
        ...
        PlatformUI.getWorkbench().getDisplay().syncExec(new Runnable() {
            ...
            @Override
            public void run() {
                MessageDialog dialog = new MessageDialog(part.getSite().getShell(), title, null, message, MessageDialog.QUESTION, 0, IDialogConstants.OK_LABEL, discardButton, // yes is the default
