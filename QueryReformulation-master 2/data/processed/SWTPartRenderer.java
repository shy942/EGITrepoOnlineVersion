...
public abstract class SWTPartRenderer extends AbstractPartRenderer {
    ...
    //$NON-NLS-1$
    private String pinURI = "platform:/plugin/org.eclipse.e4.ui.workbench.renderers.swt/icons/full/ovr16/pinned_ovr.gif";
    ...
    public void setCSSInfo(MUIElement me, Object widget) {
        ...
        final IStylingEngine engine = (IStylingEngine) ctxt.get(IStylingEngine.SERVICE_NAME);
        if (engine == null)
            return;
        // Put all the tags into the class string
        EObject eObj = (EObject) me;
        String cssClassStr = 'M' + eObj.eClass().getName();
        for (String tag : me.getTags()) cssClassStr += ' ' + tag;
    ...
    public String getToolTip(MUILabel element) {
        String overrideTip = (String) ((MUIElement) element).getTransientData().get(IPresentationEngine.OVERRIDE_TITLE_TOOL_TIP_KEY);
...
*/
    private Image adornImage(MUIElement element, Image image) {
        ...
        Image previouslyAdornedImage = (Image) element.getTransientData().get(//$NON-NLS-1$
        "previouslyAdorned");
        ...
        if (element.getTags().contains(IPresentationEngine.ADORNMENT_PIN)) {
            ...
            if (adornedImage != image)
                element.getTransientData().put("previouslyAdorned", //$NON-NLS-1$
                adornedImage);
        }
        return adornedImage;
    }
...
*/
    ...
    public void init(IEclipseContext context) {
        super.init(context);
        resUtils = (ISWTResourceUtilities) context.get(IResourceUtilities.class.getName());
    ...
    protected boolean requiresFocus(MPart element) {
        MUIElement focussed = getModelElement(Display.getDefault().getFocusControl());
        if (focussed == null) {
            return true;
        }
        // we ignore menus
