...
*/
    ...
    protected void configureShell(Shell newShell) {
        ...
        PlatformUI.getWorkbench().getHelpSystem().setHelp(newShell, ITaskListHelpContextIds.FILTERS_DIALOG);
    }
...
*/
    void createAttributesArea(Composite parent) {
        ...
        composite.setLayout(layout);
        composite.setFont(parent.getFont());
        String[] filters = { TaskListMessages.TaskList_contains, TaskListMessages.TaskList_doesNotContain };
        //$NON-NLS-1$
        descriptionGroup = new LabelComboTextGroup(composite, TaskListMessages.TaskList_whereDescription, filters, "", 200);
...
*/
    void createResourceArea(Composite parent) {
        ...
        anyResourceInSameProjectButton = createRadioButton(group, // added by cagatayk@acm.org
        TaskListMessages.TaskList_anyResourceInSameProject);
        selectedResourceButton = createRadioButton(group, TaskListMessages.TaskList_selectedResource);
        selectedResourceAndChildrenButton = createRadioButton(group, TaskListMessages.TaskList_selectedAndChildren);
    ...
    ITreeContentProvider getContentProvider() {
        return new ITreeContentProvider() {
            ...
            public Object[] getElements(Object inputElement) {
                return new Object[] { markerTypesModel.getType(IMarker.PROBLEM), markerTypesModel.getType(IMarker.TASK) };
            }
        ...
        if (markerTypes == null) {
            ...
            for (int i = 0; i < types.length; ++i) {
                ...
                if (type.getLabel().length() > 0) {
                    if (type.isSubtypeOf(markerTypesModel.getType(IMarker.PROBLEM)) || type.isSubtypeOf(markerTypesModel.getType(IMarker.TASK))) {
                        typesList.add(type);
                    }
                }
            }
...
*/
    ...
    void initTypes() {
        severityType = new EnumType(new EnumValue[] { new EnumValue(IMarker.SEVERITY_ERROR, TaskListMessages.TaskList_severity_error), new EnumValue(IMarker.SEVERITY_WARNING, TaskListMessages.TaskList_severity_warning), new EnumValue(IMarker.SEVERITY_INFO, TaskListMessages.TaskList_severity_info) });
        priorityType = new EnumType(new EnumValue[] { new EnumValue(IMarker.PRIORITY_HIGH, TaskListMessages.TaskList_priority_high), new EnumValue(IMarker.PRIORITY_NORMAL, TaskListMessages.TaskList_priority_normal), new EnumValue(IMarker.PRIORITY_LOW, TaskListMessages.TaskList_priority_low) });
        completionType = new EnumType(new EnumValue[] { new EnumValue(1, TaskListMessages.TaskList_status_completed), new EnumValue(0, TaskListMessages.TaskList_status_notCompleted) });
    }
...
*/
    ...
    protected void okPressed() {
        try {
            ...
            MessageBox messageBox = new MessageBox(getShell(), SWT.OK | SWT.APPLICATION_MODAL | SWT.ICON_ERROR);
...
*/
    void updateUIFromFilter(TasksFilter tasksFilter) {
        ...
        anyResourceInSameProjectButton.setSelection(// added by cagatayk@acm.org
        on == TasksFilter.ON_ANY_RESOURCE_OF_SAME_PROJECT);
        ...
        //$NON-NLS-1$
        markerLimit.setText("" + tasksFilter.getMarkerLimit());
