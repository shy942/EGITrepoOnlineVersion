                ...
                if (showHover) {
                    ...
                    if (decorationRectangle.contains(event.x, event.y)) {
                        ...
                        if (moveListeningTarget == null) {
                            ...
                            printRemoveListener(moveListeningTarget, //$NON-NLS-1$
                            "MOUSEMOVE");
    ...
    private void removeControlListeners() {
        ...
        if (DEBUG) {
            if (listenerInstalls > 0) {
                ...
                "REMOVED UNREGISTERED LISTENERS>>>CHECK TRACE ABOVE");
            } else {
                //$NON-NLS-1$
...
*/
    private void printAddListener(Widget widget, String listenerType) {
        ...
        if (DEBUG) {
            ...
            "Added listener>>>" + listenerType + " to>>>" + widget);
        }
    }
...
*/
    private void printRemoveListener(Widget widget, String listenerType) {
        ...
        if (DEBUG) {
            System.out.println(//$NON-NLS-1$//$NON-NLS-2$
            "Removed listener>>>" + listenerType + " from>>>" + widget);
        }
    }
