...
*/
    ...
    private void bindUI() {
        IObservableValue startDateObservable = WidgetProperties.selection().observe(startDate);
        IObservableValue endDateObservable = WidgetProperties.selection().observe(endDate);
        ControlDecorationSupport.create(new DateRangeValidator(startDateObservable, endDateObservable, "Start date must be on or before end date"), SWT.LEFT | SWT.CENTER);
        ...
        ControlDecorationSupport.create(new DateRangeValidator(Observables.constantObservableValue(new Date()), startDateObservable, "Choose a starting date later than today"), SWT.LEFT | SWT.TOP, (Composite) null, decorationUpdater);
    }
