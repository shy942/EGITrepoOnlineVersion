...
*/
    static final String createPreferenceKey(final Command command, final String stateId) {
...
*/
    public  CommandService(final CommandManager commandManager, IEclipseContext context) {
        if (commandManager == null) {
            ...
            "Cannot create a command service with a null manager");
        }
    ...
    @Override
    public final void defineUncategorizedCategory(final String name, final String description) {
        commandManager.defineUncategorizedCategory(name, description);
    }

    @Override
    public final ParameterizedCommand deserialize(final String serializedParameterizedCommand) throws NotDefinedException, SerializationException {
            ...
            for (int j = 0; j < stateIds.length; j++) {
                ...
                if (state instanceof PersistentState) {
                    ...
                    if (persistentState.shouldPersist()) {
                        persistentState.save(PrefUtil.getInternalPreferenceStore(), createPreferenceKey(command, stateId));
                    }
                }
            }
        }
...
*/
        ...
        for (Iterator i = callbackRefs.iterator(); i.hasNext(); ) {
            ...
            ISafeRunnable run = new ISafeRunnable() {
                ...
                public void handleException(Throwable exception) {
                    ...
                    "Failed to update callback: " + callbackRef.getCommandId(), exception);
                }
    ...
    public final IElementReference registerElementForCommand(ParameterizedCommand command, UIElement element) throws NotDefinedException {
        if (!command.getCommand().isDefined()) {
            ...
            "Cannot define a callback for undefined command " + command.getCommand().getId());
        }
        if (element == null) {
            throw new NotDefinedException(//$NON-NLS-1$
            "No callback defined for command " + command.getCommand().getId());
        }
    ...
    public Runnable registerElementForUpdate(ParameterizedCommand parameterizedCommand, final MItem item) {
        UIElement element = new UIElement(context.get(IWorkbench.class)) {
            ...
            public void setDisabledIcon(ImageDescriptor desc) {
                item.getTransientData().put(IPresentationEngine.DISABLED_ICON_IMAGE_KEY, MenuHelper.getIconURI(desc, context));
            }
