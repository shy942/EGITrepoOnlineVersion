...
public class HeadlessContextPresentationEngine implements IPresentationEngine {
    ...
    private static void populateModelInterfaces(MContext contextModel, IEclipseContext context, Class<?>[] interfaces) {
        for (Class<?> intf : interfaces) {
            ...
            "Adding " + intf.getName() + " for " + contextModel.getClass().getName(), null);
        ...
        childHandler = new EventHandler() {
            ...
            public void handleEvent(Event event) {
                if (UIEvents.isADD(event)) {
                    for (Object element : UIEvents.asIterable(event, UIEvents.EventTags.NEW_VALUE)) {
                ...
                if (element instanceof MUIElement) {
                    ...
                    if (parent instanceof MGenericStack) {
                        ...
                        if (parent instanceof MPerspectiveStack) {
                            ...
                            parentContext.get(EPartService.class).switchPerspective(perspective);
                        }
                    }
                }
            }
        };
        eventBroker.subscribe(UIEvents.ElementContainer.TOPIC_SELECTEDELEMENT, activeChildHandler);
        toBeRenderedHandler = new EventHandler() {

            @Override
            public void handleEvent(Event event) {
                MUIElement element = (MUIElement) event.getProperty(UIEvents.EventTags.ELEMENT);
                Boolean value = (Boolean) event.getProperty(UIEvents.EventTags.NEW_VALUE);
                if (value.booleanValue()) {
                    createGui(element, element.getParent(), getParentContext(element));
                } else {
                    removeGui(element);
                }
            }
    ...
    public Object createGui(MUIElement element) {
        ...
        if (placeholder != null) {
            return createGui(element, placeholder.getWidget(), getParentContext(element));
        }
        MUIElement parent = element.getParent();
        if (parent == null) {
    ...
    public void removeGui(MUIElement element) {
        ...
        if (element instanceof MContext) {
            ...
            if (context != null) {
                ...
                if (parentContext != null && parentContext.getActiveChild() == context) {
                    context.deactivate();
                }
            }
    ...
    private void removePlaceholder(MPlaceholder placeholder) {
        MUIElement ref = placeholder.getRef();
        List<MPlaceholder> placeholders = renderedPlaceholders.get(placeholder.getRef());
        ...
        if (placeholders.isEmpty()) {
            ...
            IEclipseContext currentContext = modelService.getContainingContext(placeholder);
            // find another placeholder to put the element under
            for (MPlaceholder other : placeholders) {
                IEclipseContext newParentContext = modelService.getContainingContext(other);
                if (newParentContext != null) {
                    List<MContext> contextElements = modelService.findElements(ref, null, MContext.class, null);
