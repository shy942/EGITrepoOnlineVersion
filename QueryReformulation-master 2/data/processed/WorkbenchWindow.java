...
*/
    ...
    private EventHandler windowWidgetHandler = new EventHandler() {
        ...
        public void handleEvent(Event event) {
            if (event.getProperty(UIEvents.EventTags.ELEMENT) == model && event.getProperty(UIEvents.EventTags.NEW_VALUE) == null) {
...
* </p>
*/
    private static final List<String> QUICK_ACCESS_ELEMENT_IDS = Collections.unmodifiableList(//$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    Arrays.asList("Spacer Glue", "SearchField", "Search-PS Glue"));
...
* @since 3.0
*/
    private static final int FILL_ALL_ACTION_BARS = ActionBarAdvisor.FILL_MENU_BAR | ActionBarAdvisor.FILL_COOL_BAR | ActionBarAdvisor.FILL_STATUS_LINE;
...
*/
        ...
        if (spacerControl == null) {
            ...
            spacerControl.setContributionURI(//$NON-NLS-1$
            "bundleclass://org.eclipse.e4.ui.workbench.renderers.swt/org.eclipse.e4.ui.workbench.renderers.swt.LayoutModifierToolControl");
            ...
            if (!spacerControl.getTags().contains("SHOW_RESTORE_MENU")) {
                //$NON-NLS-1$
                //$NON-NLS-1$
                spacerControl.getTags().add("SHOW_RESTORE_MENU");
            }
        }
        MToolControl switcherControl = (MToolControl) modelService.find("PerspectiveSwitcher", //$NON-NLS-1$
        model);
        if (switcherControl == null && getWindowConfigurer().getShowPerspectiveBar()) {
            ...
            switcherControl.setContributionURI(//$NON-NLS-1$
            ...
            if (elements.remove(element)) {
                ...
                switch(positionInfo.getPosition()) {
                    ...
                    default:
                        ...
                        "Can't position control '" + element.getElementId() + //$NON-NLS-1$
                        "' because of the unknown position type '" + positionInfo.getPosition() + //$NON-NLS-1$
                        "'!");
                }
            }
        }
...
*/
    ...
    private void populateStandardTrim(MTrimBar bottomTrim) {
        ...
        // Heap Status
        MToolControl hsElement = (MToolControl) modelService.find("org.eclipse.ui.HeapStatus", //$NON-NLS-1$
        model);
        if (hsElement == null) {
        ...
        // Progress Bar
        MToolControl pbElement = (MToolControl) modelService.find("org.eclipse.ui.ProgressBar", //$NON-NLS-1$
        model);
        if (pbElement == null) {
    ...
    private void populateTrimContributions(MTrimBar bottomTrim) {
        // Part 1: Add groups
        IConfigurationElement[] exts = extensionRegistry.getConfigurationElementsFor(//$NON-NLS-1$
        ...
        // Hack!! startup seeding
        handledElements.add(items.get(0));
        ...
        while (items.size() > 0 && handledElements.size() > 0) {
            ...
            for (IConfigurationElement item : items) {
                ...
                //$NON-NLS-1$
                IConfigurationElement[] locs = item.getChildren("location");
                for (IConfigurationElement loc : locs) {
                    ...
                    if (bars.length > 0) {
                        ...
                        //$NON-NLS-1$//$NON-NLS-2$
                        boolean isTrim = "trim".equals(bar.getAttribute("type"));
                        if (isTrim) {
                            //$NON-NLS-1$
    ...
    private MToolControl addTrimElement(MTrimBar bottomTrim, IConfigurationElement ice, String id, boolean isBefore, String relTo, String classSpec) {
        ...
        if (classSpec != null) {
            newTrimElement.setContributionURI(//$NON-NLS-1$
            "bundleclass://org.eclipse.ui.workbench/org.eclipse.ui.internal.LegacyTrim");
        }
    ...
    void registerGlobalAction(IAction globalAction) {
        ...
        if (commandId != null) {
            ...
            if (globalAction instanceof CommandAction) {
                ...
                if (actionId != null) {
                    final IActionCommandMappingService mappingService = (IActionCommandMappingService) serviceLocator.getService(IActionCommandMappingService.class);
...
*/
        ...
        if (shell != null) {
            ...
            for (Iterator<Entry<String, ActionHandler>> iterator = handlersByCommandId.entrySet().iterator(); iterator.hasNext(); ) {
...
*/
    ...
    private void hideNonRestorableViews() {
        ...
        for (MPlaceholder ph : phList) {
            ...
            if (regEntry != null && !regEntry.isRestorable() && !("*".equals(secondaryId))) {
                //$NON-NLS-1$
...
*/
    ...
    void liftRestrictions() {
        ...
        IEvaluationService es = (IEvaluationService) serviceLocator.getService(IEvaluationService.class);
...
*/
    private boolean getShowHeapStatus() {
        ...
        PrefUtil.getAPIPreferenceStore().getBoolean(IWorkbenchPreferenceConstants.SHOW_MEMORY_MONITOR) || Boolean.valueOf(Platform.getDebugOption(PlatformUI.PLUGIN_ID + //$NON-NLS-1$
        "/perf/showHeapStatus")).booleanValue();
    }

    public void showHeapStatus(boolean show) {
        //$NON-NLS-1$
        MUIElement hsElement = modelService.find("org.eclipse.ui.HeapStatus", model);
...
*/
    public void setCoolBarVisible(boolean visible) {
        ...
        if (oldValue != coolBarVisible) {
            ...
            updateLayoutDataForContents();
            firePropertyChanged(PROP_COOLBAR_VISIBLE, oldValue ? Boolean.TRUE : Boolean.FALSE, coolBarVisible ? Boolean.TRUE : Boolean.FALSE);
        }
    }
...
*/
    public void setPerspectiveBarVisible(boolean visible) {
        ...
        if (oldValue != perspectiveBarVisible) {
            ...
            updateLayoutDataForContents();
            firePropertyChanged(PROP_PERSPECTIVEBAR_VISIBLE, oldValue ? Boolean.TRUE : Boolean.FALSE, perspectiveBarVisible ? Boolean.TRUE : Boolean.FALSE);
        }
    }
...
*/
    public void setStatusLineVisible(boolean visible) {
        ...
        if (oldValue != statusLineVisible) {
            getModel().getPersistedState().put(IPreferenceConstants.PERSPECTIVEBAR_VISIBLE, Boolean.toString(visible));
            updateLayoutDataForContents();
            firePropertyChanged(PROP_STATUS_LINE_VISIBLE, oldValue ? Boolean.TRUE : Boolean.FALSE, statusLineVisible ? Boolean.TRUE : Boolean.FALSE);
        }
    }
...
*/
    public boolean isToolbarVisible() {
        return (getCoolBarVisible() && getWindowConfigurer().getShowCoolBar()) || (getPerspectiveBarVisible() && getWindowConfigurer().getShowPerspectiveBar());
    }
    ...
    public CoolBarManager getCoolBarManager() {
        //$NON-NLS-1$
        new Exception("Bad call to getCoolBarManager()").printStackTrace();
        return oldCBM;
    }
    ...
    //$NON-NLS-1$
    MenuManager menuManager = new MenuManager("MenuBar", ActionSet.MAIN_MENU);
