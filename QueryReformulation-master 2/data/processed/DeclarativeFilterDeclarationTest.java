...
*/
    public void testAnyErrorFilter() {
        ...
        assertTrue(filterName + "not selecting by severity", filter.getSelectBySeverity());
        assertTrue(filterName + "should be on error", filter.getSeverity() == ProblemFilter.SEVERITY_ERROR);
        assertTrue(filterName + "should be on any", filter.getOnResource() == MarkerFilter.ON_ANY);
    }
...
*/
    public void testSelectedWarning() {
        ...
        assertTrue(filterName + " is enabled ", !filter.isEnabled());
        assertTrue(filterName + "not selecting by severity", filter.getSelectBySeverity());
        assertTrue(filterName + "should be on warning", filter.getSeverity() == ProblemFilter.SEVERITY_WARNING);
        assertTrue(filterName + "should be on selected only", filter.getOnResource() == MarkerFilter.ON_SELECTED_ONLY);
    }
...
*/
    public void testInfoAndChildren() {
        ...
        assertTrue(filterName + " is enabled ", !filter.isEnabled());
        assertTrue(filterName + "not selecting by severity", filter.getSelectBySeverity());
        assertTrue(filterName + "should be on info", filter.getSeverity() == ProblemFilter.SEVERITY_INFO);
        assertTrue(filterName + "should be on selected and children", filter.getOnResource() == MarkerFilter.ON_SELECTED_AND_CHILDREN);
    }
...
*/
    public void testSameContainer() {
        ...
        assertTrue(filterName + " not found ", filter != null);
        assertTrue(filterName + " is enabled ", !filter.isEnabled());
        assertFalse(filterName + "selecting by severity", filter.getSelectBySeverity());
        assertTrue(filterName + "should be on on any in same container", filter.getOnResource() == MarkerFilter.ON_ANY_IN_SAME_CONTAINER);
    }
...
*/
    public void testDescription() {
        ...
        assertTrue(filterName + " not found ", filter != null);
        assertTrue(filterName + " is enabled ", !filter.isEnabled());
        assertTrue(filterName + "does not have description", filter.getDescription().length() > 0);
...
*/
    public void testProblemTypes() {
        ...
        assertTrue(filterName + "should only have one type has " + types.size(), types.size() == 1);
        assertTrue(filterName + "should be enabled for category test", ((MarkerType) types.get(0)).getId().equals("org.eclipse.ui.tests.categoryTestMarker"));
    }
}
