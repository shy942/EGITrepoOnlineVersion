    ...
    public void getSegments(SegmentEvent event) {
        ...
        if (length > 0) {
            try {
                ...
                for (int i = 1; i < event.segments.length - 1; i++) event.segmentsChars[i] = BidiUtils.LRM;
                event.segmentsChars[event.segments.length - 1] = BidiUtils.PDF;
            } catch (RuntimeException ex) {
                if (logExceptions) {
                    Policy.getLog().log(new Status(IStatus.ERROR, Policy.JFACE, "An error occurred while processing \"" + event.lineText + "\" with " + expert, ex));
                    logExceptions = false;
                }
            }
        }
