            ...
            public void run() throws Exception {
                if (currentListener != null) {
                    ...
                    while (iterator.hasNext()) {
                        iterator.next().dragFinished(event);
                    }
                }
            }
        });
...
*/
    ...
    public void dragSetData(final DragSourceEvent event) {
        ...
        // find a listener that can provide the given data type
...
*/
    ...
    public void dragStart(final DragSourceEvent event) {
        ...
        // true if any one of the listeners can handle the drag
        boolean doit = false;
        List<Transfer> transfers = new ArrayList(listeners.size());
        activeListeners.clear();
        for (int i = 0; i < listeners.size(); i++) {
            final TransferDragSourceListener listener = listeners.get(i);
            // restore event.doit
            event.doit = true;
            SafeRunnable.run(new SafeRunnable() {
...
*/
    public Transfer[] getTransfers() {
        ...
        for (int i = 0; i < listeners.size(); i++) {
            TransferDragSourceListener listener = listeners.get(i);
