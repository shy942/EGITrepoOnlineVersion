    ...
    public boolean close() {
        ...
        SafeRunnable runnable = new SafeRunnable() {
            ...
            public void handleException(Throwable e) {
                ...
                //Do not cache a node with problems
                clearSelectedNode();
            }
        };
            ...
            if (page instanceof PreferencePage) {
                ...
                if (store != null && store.needsSaving() && store instanceof IPersistentPreferenceStore) {
                    try {
                        ...
                        String message = JFaceResources.format("PreferenceDialog.saveErrorMessage", page.getTitle(), e.getMessage());
...
*/
    protected void layoutTreeAreaControl(Control control) {
        GridDataFactory.fillDefaults().hint(getLastRightWidth(), convertVerticalDLUsToPixels(120)).grab(false, true).applyTo(control);
    }
...
*/
    void showPageFlippingAbortDialog() {
        MessageDialog.open(MessageDialog.ERROR, getShell(), JFaceResources.getString(//$NON-NLS-1$
        "AbortPageFlippingDialog.title"), JFaceResources.getString("AbortPageFlippingDialog.message"), //$NON-NLS-1$
        SWT.SHEET);
    }
