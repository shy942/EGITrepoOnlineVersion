...
*/
    Control createUpDownBtt(Composite parent) {
        ...
        //$NON-NLS-1$
        downButton.setText(JFaceResources.getString("ConfigureColumnsDialog_down"));
...
*/
    void handleVisibleSelection(ISelection selection) {
        @SuppressWarnings("unchecked") List<T> selVCols = ((IStructuredSelection) selection).toList();
...
*/
    void handleDownButton(Event e) {
        ...
        @SuppressWarnings("unchecked") List<T> selVCols = selection.toList();
        List<T> allVCols = getVisible();
        IColumnUpdater<T> updater = doGetColumnUpdater();
...
*/
    private void updateWidth() {
        try {
            ...
            @SuppressWarnings("unchecked") T data = (T) ((IStructuredSelection) visibleViewer.getSelection()).getFirstElement();
            if (data != null) {
