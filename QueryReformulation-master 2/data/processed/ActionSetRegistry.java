...
*/
    private IExtensionPoint getActionSetPartAssociationExtensionPoint() {
        return Platform.getExtensionRegistry().getExtensionPoint(PlatformUI.PLUGIN_ID, IWorkbenchRegistryConstants.PL_ACTION_SET_PART_ASSOCIATIONS);
    }
...
*/
    private IExtensionPoint getActionSetExtensionPoint() {
        return Platform.getExtensionRegistry().getExtensionPoint(PlatformUI.PLUGIN_ID, IWorkbenchRegistryConstants.PL_ACTION_SETS);
    }
...
*/
    private void addActionSet(ActionSetDescriptor desc) {
        ...
        if (!actionSetContext.isDefined()) {
            actionSetContext.define(desc.getLabel(), desc.getDescription(), //$NON-NLS-1$
    ...
    public IActionSetDescriptor[] getActionSetsFor(String partId) {
        ...
        for (Iterator i = actionSetIds.iterator(); i.hasNext(); ) {
            ...
            if (actionSet != null) {
                ...
                "Unable to associate action set with part: " + partId + ". Action set " + actionSetId + //$NON-NLS-2$ //$NON-NLS-1$
                " not found.");
            }
        }
...
*/
    private void readFromRegistry() {
        ...
        for (int i = 0; i < extensions.length; i++) {
            addActionSets(PlatformUI.getWorkbench().getExtensionTracker(), extensions[i]);
        }
        extensions = getActionSetPartAssociationExtensionPoint().getExtensions();
        for (int i = 0; i < extensions.length; i++) {
            addActionSetPartAssociations(PlatformUI.getWorkbench().getExtensionTracker(), extensions[i]);
        }
    }
            ...
            if (element.getName().equals(IWorkbenchRegistryConstants.TAG_ACTION_SET_PART_ASSOCIATION)) {
                ...
                for (int j = 0; j < children.length; j++) {
                    ...
                    if (child.getName().equals(IWorkbenchRegistryConstants.TAG_PART)) {
                        ...
                        WorkbenchPlugin.log(//$NON-NLS-1$
                        "Unable to process element: " + child.getName() + //$NON-NLS-1$
                        " in action set part associations extension: " + extension.getUniqueIdentifier());
                    }
                }
            }
        }
