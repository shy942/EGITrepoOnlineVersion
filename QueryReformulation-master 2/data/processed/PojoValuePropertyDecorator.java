...
*/
    public  PojoValuePropertyDecorator(IValueProperty delegate, PropertyDescriptor propertyDescriptor) {
    ...
    public IBeanValueProperty value(IBeanValueProperty property) {
        return new PojoValuePropertyDecorator(super.value(property), property.getPropertyDescriptor());
    }
    ...
    public IBeanMapProperty map(String propertyName, Class keyType, Class valueType) {
        Class beanClass = (Class) delegate.getValueType();
        return map(PojoProperties.map(beanClass, propertyName, keyType, valueType));
    }
