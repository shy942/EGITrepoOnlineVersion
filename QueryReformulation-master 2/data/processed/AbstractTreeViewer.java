    ...
    private void createAddedElements(Widget widget, Object[] elements) {
        ...
        elementloop: for (int i = 0; i < elements.length; i++) {
            ...
            if (indexInItems == items.length) {
                ...
                while (insertionIndexInItems < items.length && internalCompare(comparator, parentPath, element, items[insertionIndexInItems].getData()) == 0) {
                    // any)
                ...
                if (tis != null) {
                    for (int i1 = 0; i1 < tis.length; i1++) {
                        if (tis[i1].getData() != null) {
                            disassociate(tis[i1]);
                            Assert.isTrue(tis[i1].getData() == null, //$NON-NLS-1$
...
*/
    protected void doUpdateItem(final Item item, Object element) {
        ...
        if (// If no columns are created then fake one
        columnCount == 0)
            columnCount = 1;
...
*/
    protected Widget internalExpand(Object elementOrPath, boolean expand) {
        ...
        if (w == null) {
            if (equals(elementOrPath, getRoot())) {
                // stop at root
                return null;
            }
    ...
    protected void internalRemove(Object[] elementsOrPaths) {
        ...
        for (int i = 0; i < elementsOrPaths.length; ++i) {
            ...
            if (childItems.length > 0) {
                ...
                if (parent != null && !equals(parent, getRoot()) && !(parent instanceof TreePath && ((TreePath) parent).getSegmentCount() == 0)) {
...
*/
    ...
    protected void assertContentProviderType(IContentProvider provider) {
        Assert.isTrue(provider instanceof ITreeContentProvider || provider instanceof ITreePathContentProvider, //$NON-NLS-1$
        "Instances of AbstractTreeViewer must have a content provider " + //$NON-NLS-1$
        "of type ITreeContentProvider or ITreePathContentProvider");
    }
...
*/
    ...
    public ITreeSelection getStructuredSelection() throws ClassCastException {
        ...
        //$NON-NLS-1$
        throw new ClassCastException("AbstractTreeViewer should return an instance of ITreeSelection from its getSelection() method.");
    }
...
* @since 3.3
*/
    protected final boolean internalIsInputOrEmptyPath(final Object elementOrTreePath) {
