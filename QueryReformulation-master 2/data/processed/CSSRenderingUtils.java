...
public class CSSRenderingUtils {
    ...
    public CSSValue getCSSValue(Control styleControl, String className, String attributeName) {
        ...
        ControlElement tempEment = (ControlElement) csseng.getElement(styleControl);
        if (tempEment == null) {
            return null;
        }
        // super hack
        ...
        CSSStyleDeclaration styleDeclarations = csseng.getViewCSS().getComputedStyle(tempEment, //$NON-NLS-1$
        "");
...
*/
    public Image createImage(Control styleControl, String classId, String attName, Integer[] frameInts) {
        ...
        CSSStyleDeclaration styleDeclarations = csseng.getViewCSS().getComputedStyle(tempEment, "");
        if (styleDeclarations == null)
            return null;
        //$NON-NLS-1$
        ...
        if (imagePath == null)
            return null;
        if (imagePath != null && imagePath.getCssValueType() == CSSValue.CSS_PRIMITIVE_VALUE) {
            // System.out.println("here" + imageString);
            try {
                image = (Image) csseng.convert(imagePath, Image.class, styleControl.getDisplay());
                if (image != null && frameInts != null) {
                    CSSValue value = styleDeclarations.getPropertyCSSValue(//$NON-NLS-1$
                    "frame-cuts");
                    if (value.getCssValueType() == CSSValue.CSS_VALUE_LIST) {
                        ...
                        for (int i = 0; i < valueList.getLength(); i++) {
                            ...
                            if ((val.getCssValueType() == CSSValue.CSS_PRIMITIVE_VALUE) && ((CSSPrimitiveValue) val).getPrimitiveType() == CSSPrimitiveValue.CSS_PX) {
                                frameInts[i] = (int) ((CSSPrimitiveValue) val).getFloatValue(CSSPrimitiveValue.CSS_PX);
                            } else {
                                return null;
                            }
                        }
    ...
    private void addHandleImageDisposedListener(ImageBasedFrame imageBasedFrame, final Control toFrame, final String classId, final boolean vertical) {
    ...
    private void addFrameImageDisposedListener(ImageBasedFrame imageBasedFrame, final Control toFrame, final String classId, final boolean vertical) {
        final Listener listener = new Listener() {
